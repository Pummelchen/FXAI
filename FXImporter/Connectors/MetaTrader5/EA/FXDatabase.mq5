...
#property description "FXDatabase localhost history bridge. Swift owns validation, storage, checkpoints, verification, and repair."
...
int OnInit()
{
   ...
   Print("FXDatabase EA initialized. Waiting to connect to Swift at ", SwiftHost, ":", SwiftPort);
...
bool EnsureConnected()
{
   ...
   {
      Print("FXDatabase bridge SocketCreate failed. error=", GetLastError());
      return false;
   }
   ...
   {
      Print("FXDatabase bridge connect failed. error=", GetLastError(), " target=", SwiftHost, ":", SwiftPort);
      CloseSocket();
      return false;
   }

   Print("FXDatabase bridge connected to Swift at ", SwiftHost, ":", SwiftPort);
   g_connected = true;
   return true;
}
...
bool ProcessOneFrame()
{
   ...
   {
      Print("FXDatabase bridge failed to read frame header. error=", GetLastError());
      CloseSocket();
      return false;
   }
   ...
   {
      Print("FXDatabase bridge invalid frame length: ", bodyLength);
      CloseSocket();
      return false;
   }
   ...
   {
      Print("FXDatabase bridge partial frame read. expected=", bodyLength, " error=", GetLastError());
      CloseSocket();
      return false;
   }
...
void HandleRequest(const string json)
{
   ...
   if(command == "HELLO")
      SendOK(requestId, command, "{\"bridge_name\":\"FXDatabase\",\"bridge_version\":\"0.1\",\"schema_version\":1}");
...
bool SendAll(uchar &frame[])
{
   ...
   while(sent < total && !IsStopped())
   {
      ...
      if(written <= 0)
      {
         Print("FXDatabase bridge SocketSend failed. sent=", sent, " remaining=", remaining, " error=", GetLastError());
         CloseSocket();
         return false;
      }
      sent += written;
