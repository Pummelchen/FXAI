...
private struct FXStupidBroker {
    ...
    mutating func open(symbol: String, direction: FXStupidDirection, lots: Double, price: Int64, digits: Int) throws -> Bool {
        ...
        let normalizedLots = Self.normalizeLot(lots)
    ...
    private mutating func close(position: FXStupidPosition, price: Int64, digits _: Int?) throws {
        let gross = profit(position: position, price: price)
        balance += gross
    ...
    private static func normalizeLot(_ lots: Double) -> Double {
        guard lots.isFinite, lots > 0 else { return 0 }
        return (lots * 100).rounded() / 100
    }
}
