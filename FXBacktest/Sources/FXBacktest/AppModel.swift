...
final class AppModel: ObservableObject, @unchecked Sendable {
    ...
    func runOptimization() {
        ...
        updateStatus("Starting pure M1 OHLC backtest for \(marketUniverse.symbols.joined(separator: ","))...")
        ...
        runTask = Task {
            ...
            do {
                ...
                updateStatus("Running \(plugin.descriptor.displayName) on \(settings.target.rawValue.uppercased()) with pure M1 OHLC close prices...")
    ...
    private func startPersistenceIfEnabled(
        ...
        do {
            ...
            let run = BacktestStoredRun(
                ...
                brokerSourceId: brokerSourceId(for: marketUniverse),
    ...
    private func brokerSourceId(for universe: OhlcMarketUniverse) -> String {
        let brokerIds = Set(universe.seriesBySymbol.values.map(\.metadata.brokerSourceId))
        if brokerIds.count == 1, let brokerSourceId = brokerIds.first {
            return brokerSourceId
        }
        return "mixed"
    }
...
extension AppModel {
    ...
    private func saveResultsFromTerminal(_ arguments: ArraySlice<String>) async throws {
        ...
        let settings = lastRunSettings ?? BacktestRunSettings(
            target: executionTarget,
            maxWorkers: maxWorkers,
            chunkSize: chunkSize,
        ...
        let run = BacktestStoredRun(
            ...
            engine: settings.target,
            brokerSourceId: brokerSourceId(for: activeUniverse),
