...
final class FXStupidPluginTests: XCTestCase {
    ...
    func testFXStupidUsesConfiguredContractSizeWithPureOhlcPrices() throws {
        let plugin = FXStupid()
        let market = try monotonicMarket(symbol: "EURUSD")
        let sweep = try ParameterSweep.singlePass(definitions: plugin.parameterDefinitions)
        let parameters = try sweep.parameterVector(at: 0)
        let fullSizeContext = BacktestContext(settings: BacktestRunSettings(contractSize: 100_000), digits: market.metadata.digits)
        let halfSizeContext = BacktestContext(settings: BacktestRunSettings(contractSize: 50_000), digits: market.metadata.digits)

        let fullSizeResult = try plugin.runPass(market: market, parameters: parameters, context: fullSizeContext)
        let halfSizeResult = try plugin.runPass(market: market, parameters: parameters, context: halfSizeContext)

        XCTAssertGreaterThan(fullSizeResult.netProfit, 0)
        XCTAssertLessThan(halfSizeResult.netProfit, fullSizeResult.netProfit)
    }
