...
public struct MicrostructurePairState: Codable, Hashable, Sendable {
    ...
    public init(
        ...
        spreadCurrent: Double = 0.0,
        spreadZscore60s: Double = 0.0,
        ...
        sessionSpreadBehaviorScore: Double = 0.0,
        ...
        self.spreadCurrent = max(0.0, spreadCurrent)
        self.spreadZscore60s = fxClamp(spreadZscore60s, -8.0, 8.0)
        ...
        self.sessionSpreadBehaviorScore = fxClamp(sessionSpreadBehaviorScore, 0.0, 1.0)
...
public enum MicrostructureTools {
    ...
    public static func parseSnapshot(tsv: String, pairID: String) -> MicrostructurePairState {
        ...
        for line in tsv.components(separatedBy: .newlines) where !line.isEmpty {
            ...
            if kind == "pair", target == targetPairID {
                ...
                case "spread_current":
                    state.spreadCurrent = Double(value) ?? 0.0
                case "spread_zscore_60s":
                    state.spreadZscore60s = Double(value) ?? 0.0
                case "tick_rate_60s":
                ...
                case "session_spread_behavior_score":
                    state.sessionSpreadBehaviorScore = Double(value) ?? 0.0
                case "trade_gate":
    ...
    private static func normalizedAvailableState(
        ...
        output.spreadCurrent = max(output.spreadCurrent, 0.0)
        output.spreadZscore60s = fxClamp(output.spreadZscore60s, -8.0, 8.0)
        ...
        output.sessionSpreadBehaviorScore = fxClamp(output.sessionSpreadBehaviorScore, 0.0, 1.0)
