...
- `Tools/OfflineLab/offline_lab/rates_engine*.py`
  ...
  Existing shared-subsystem pattern for FXDatabase service export, FILE_COMMON artifacts, local status mirroring, replay, and service installation.
...
- `Tools/OfflineLab/offline_lab/market_universe.py`
  ...
  Existing broader FXDatabase context-symbol conventions. The new subsystem borrows those proxy expectations for DXY, yields, volatility, and equity/commodity proxies.
...
- Instead of depending on external paid feeds or a separate non-FXDatabase market-data connector, phase 1 uses:
  - existing rates-engine outputs
  - FXDatabase-accessible indicator/context symbols
