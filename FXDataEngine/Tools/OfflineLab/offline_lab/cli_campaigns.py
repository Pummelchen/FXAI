...
def run_dataset_campaign(conn: libsql.Connection, dataset: dict, profile_name: str, args, out_dir: Path, baseline_summary: dict, base_args) -> list[dict]:
    ...
    campaign = extend_campaign(
        testlab.build_optimization_campaign(
            ...
            walkforward_available_bars=int(dataset.get("bars", 0) or 0),
            walkforward_test_years=float(getattr(args, "wf_test_years", 0.0) or 0.25),
            walkforward_folds=int(getattr(args, "wf_folds", 6) or 6),
            walkforward_bars_per_year=int(getattr(args, "wf_bars_per_year", testlab.FX_M1_BARS_PER_TRADING_YEAR) or testlab.FX_M1_BARS_PER_TRADING_YEAR),
