...
- `Lifecycle` and `Warmup`: partially ported. Lifecycle context-symbol category/scoring helpers, Warmup bucket scoring, and portfolio-objective proxy math are in Swift; remaining provider-backed context discovery, warmup loops, plugin pretraining, lifecycle bootstrap/compliance/reset behavior, and agent orchestration still need conversion.
