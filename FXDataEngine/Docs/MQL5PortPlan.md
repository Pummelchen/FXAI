...
- `Runtime/Trade`: partially ported. Pure symbol/currency exposure helpers, Pair Network config/exposure math, and first risk-policy admission/sizing DTOs are in FXDataEngine; remaining broker position/order scanning, lifecycle execution, peer discovery, broker lot normalization, and live exposure caps must be split between FXBacktest simulation and future live-agent contracts.
