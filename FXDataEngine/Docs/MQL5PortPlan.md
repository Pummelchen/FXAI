...
- `Runtime/Trade`: partially ported. Pure symbol/currency exposure helpers, Pair Network config/exposure math, first risk-policy admission/sizing DTOs, and lifecycle recommendation math are in FXDataEngine; remaining broker position/order scanning, lifecycle execution, peer discovery, broker lot normalization, and live exposure caps must be split between FXBacktest simulation and future live-agent contracts.
...
- Done: Risk policy foundational slice: `runtime_trade_risk.mqh` and `runtime_trade_exposure.mqh` kill-switch reason priority, prepared-state admission gates, factor/system-health/macro floors, conviction blending, estimated risk-points, prepared portfolio-pressure scoring/gating, risk-budget lot cap, and fixed/conviction/vol-target lot math before broker normalization are ported as deterministic Swift helpers. Service overlay gates, portfolio exposure cap calculation from live positions/orders, symbol lot-step normalization, money-per-point discovery, account equity reads, and order filling resolution remain FXBacktest/live-agent work.
