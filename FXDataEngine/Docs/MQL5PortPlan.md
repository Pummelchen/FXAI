...
- Done: Pair Network pure helper slice: `runtime_trade_pair_network.mqh` defaults, config/status TSV parsing, safe-token runtime paths, decision labels, currency profiles, base-minus-quote factor exposure, structural-overlap scoring, and quality-score blending are ported without live peer discovery or order/position APIs. Runtime artifact writing and live peer collection remain FXBacktest/live-agent work.
