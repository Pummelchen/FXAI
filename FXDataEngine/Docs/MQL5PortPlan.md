...
- Done: Risk policy foundational slice: `runtime_trade_risk.mqh` kill-switch reason priority, prepared-state admission gates, factor/system-health/macro floors, conviction blending, risk-budget lot cap, and fixed/conviction/vol-target lot math before broker normalization are ported as deterministic Swift helpers. Service overlay gates, portfolio exposure caps, symbol lot-step normalization, money-per-point discovery, account equity reads, and order filling resolution remain FXBacktest/live-agent work.
