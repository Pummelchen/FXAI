...
- `Runtime` stages: partially ported. Time-context helpers and router model-cap pressure logic are available as deterministic Swift contracts; broader feature/model/transfer/policy/finalize orchestration still needs plugin mockability before real PyTorch/TensorFlow backends.
...
- Done: runtime time/router micro-slice: `runtime_time_service.mqh` reset/build/offset conversion/summary behavior is ported to prepared Swift time inputs, and `runtime_router_stage.mqh` performance-budget model capping is ported against `RuntimePerformanceState`.
