...
- Done: Risk policy foundational slice: `runtime_trade_risk.mqh` and `runtime_trade_exposure.mqh` kill-switch reason priority, prepared-state admission gates, factor/system-health/macro floors, conviction blending, estimated risk-points, prepared portfolio-pressure scoring/gating, supervisor/service/command overlay blocks, overlay lot scaling, prepared exposure-cap application, risk-budget lot cap, and fixed/conviction/vol-target lot math before broker normalization are ported as deterministic Swift helpers. Service caution lot scales, live exposure collection from positions/orders, symbol lot-step normalization, money-per-point discovery, account equity reads, and order filling resolution remain FXBacktest/live-agent work.
