...
def _top_level_plugin_sources() -> list[Path]:
    api_text = _read("FXDataEngine/API/api.mqh")
    include_paths = re.findall(r'#include "\.\.\\\.\.\\(FXPlugins\\[^"]+\.mqh)"', api_text)
...
def test_plugin_contract_suite_exercises_lifecycle_predict_state_and_synthetic_series_paths():
    suite = _read("FXDataEngine/Tests/PluginContracts/plugin_contract_suite.mqh")
    required_tokens = [
        "registry.Initialize()",
        "registry.Release()",
