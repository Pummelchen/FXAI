...
#define FXAI_CAT_MAX_TREES 96
#define FXAI_CAT_MAX_DEPTH 5
#define FXAI_CAT_MAX_LEAVES (1 << FXAI_CAT_MAX_DEPTH)
#define FXAI_CAT_BINS 20
#define FXAI_CAT_BUFFER 4096
#define FXAI_CAT_MIN_BUFFER 192
#define FXAI_CAT_BUILD_EVERY 64
#define FXAI_CAT_MIN_DATA 10
#define FXAI_CAT_MIN_CHILD_HESS 0.05
#define FXAI_CAT_GAMMA 0.01
#define FXAI_CAT_CAL_BINS 10
...
private:
   ...
   double m_cal3_temp;
   double m_cal3_bias[FXAI_CAT_CLASS_COUNT];
                         ...
                         const double &x[]) const
   {
      ...
      for(int d=0; d<tree.depth; d++)
      {
         ...
         if(f < 1 || f >= FXAI_AI_WEIGHTS) break;
                      ...
                      double &gain_out) const
   {
      ...
      if(feature < 1 || feature >= FXAI_AI_WEIGHTS) return false;
      ...
      for(int old_leaf=0; old_leaf<old_leaf_count; old_leaf++)
      {
         ...
         if(child_cnt[left_leaf] < FXAI_CAT_MIN_DATA || child_cnt[right_leaf] < FXAI_CAT_MIN_DATA)
            return false;
         ...
         gain += 0.5 * (child_score - parent_score) - FXAI_CAT_GAMMA;
      }
   ...
   void BuildLeafValues(FXAICatTree &tree,
                        const int n,
                        const int &leaf_idx[],
                        const double &g[][FXAI_CAT_CLASS_COUNT],
                        const double &h[][FXAI_CAT_CLASS_COUNT],
                        ...
                        const double lambda) const
   {
      ...
      double leaf_g[FXAI_CAT_MAX_LEAVES][FXAI_CAT_CLASS_COUNT];
      double leaf_h[FXAI_CAT_MAX_LEAVES][FXAI_CAT_CLASS_COUNT];
      double leaf_mv_sum[FXAI_CAT_MAX_LEAVES];
      int    leaf_cnt[FXAI_CAT_MAX_LEAVES];
      ...
      for(int l=0; l<leaf_count; l++)
      {
         ...
         for(int c=0; c<FXAI_CAT_CLASS_COUNT; c++)
         {
            ...
            if(leaf_cnt[l] > 0 && leaf_h[l][c] > 1e-9)
               v = eta * FXAI_ClipSym(leaf_g[l][c] / (leaf_h[l][c] + lambda), 5.0);
            tree.leaf_value[l][c] = v;
            mean_val += v;
         }
         mean_val /= (double)FXAI_CAT_CLASS_COUNT;
         for(int c=0; c<FXAI_CAT_CLASS_COUNT; c++)
            tree.leaf_value[l][c] -= mean_val;
      }

      for(int l=leaf_count; l<FXAI_CAT_MAX_LEAVES; l++)
      {
         tree.leaf_count[l] = 0;
         tree.leaf_move_mean[l] = 0.0;
         for(int c=0; c<FXAI_CAT_CLASS_COUNT; c++)
   ...
   bool BuildOneTree(const FXAIAIHyperParams &hp)
   {
      ...
      double eta = FXAI_Clamp(hp.xgb_lr, 0.0005, 0.3000);
      double lambda = FXAI_Clamp(hp.xgb_l2, 0.0001, 10.0000);
      if(m_drift_cooldown > 0)
      {
         eta = FXAI_Clamp(eta * 0.70, 0.0005, 0.3000);
         lambda = FXAI_Clamp(lambda * 1.25, 0.0001, 10.0000);
      }
      ...
      for(int i=0; i<n; i++)
      {
         ...
         double probs[FXAI_CAT_CLASS_COUNT];
         ModelMargins(xloc, margins);
         Softmax3(margins, probs);

         double wi = FXAI_Clamp(w_all[i], 0.10, 6.00);
         for(int c=0; c<FXAI_CAT_CLASS_COUNT; c++)
         {
            double target = (c == y_all[i] ? 1.0 : 0.0);
            g[i][c] = (target - probs[c]) * wi;
            h[i][c] = FXAI_Clamp(probs[c] * (1.0 - probs[c]) * wi, 0.01, 4.00);
         }
      }
      ...
      for(int c=0; c<FXAI_CAT_CLASS_COUNT; c++)
      {
         ...
         if(H > 1e-9)
            m_bias[c] += 0.20 * eta * FXAI_ClipSym(G / (H + lambda), 4.0);
      }
      ...
      double minv[FXAI_AI_WEIGHTS];
      double maxv[FXAI_AI_WEIGHTS];
      int valid_cnt[FXAI_AI_WEIGHTS];
      ...
      for(int d=0; d<FXAI_CAT_MAX_DEPTH; d++)
      {
         ...
         for(int f=1; f<FXAI_AI_WEIGHTS; f++)
         {
            if(valid_cnt[f] < 2 * FXAI_CAT_MIN_DATA) continue;
            double range = maxv[f] - minv[f];
            if(range < 1e-12) continue;

            for(int b=1; b<FXAI_CAT_BINS; b++)
            {
               double thr = minv[f] + range * ((double)b / (double)FXAI_CAT_BINS);
...
public:
   ...
   virtual void EnsureInitialized(const FXAIAIHyperParams &hp)
   {
      ...
      m_bias[(int)FXAI_LABEL_SKIP] = 0.12;
   }
...
protected:
                               ...
                               const double move_points)
   {
      ...
      double cal_lr = FXAI_Clamp(0.01 + 0.10 * FXAI_Clamp(h.xgb_lr, 0.0005, 0.3000), 0.0005, 0.0300);
