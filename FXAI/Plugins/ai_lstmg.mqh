...
private:
   ...
   double m_wi[FXAI_AI_MLP_HIDDEN][FXAI_AI_WEIGHTS];
   double m_wf[FXAI_AI_MLP_HIDDEN][FXAI_AI_WEIGHTS];
   double m_wo[FXAI_AI_MLP_HIDDEN][FXAI_AI_WEIGHTS];
   double m_wg[FXAI_AI_MLP_HIDDEN][FXAI_AI_WEIGHTS];

   double m_ui[FXAI_AI_MLP_HIDDEN][FXAI_AI_MLP_HIDDEN];
   double m_uf[FXAI_AI_MLP_HIDDEN][FXAI_AI_MLP_HIDDEN];
   double m_uo[FXAI_AI_MLP_HIDDEN][FXAI_AI_MLP_HIDDEN];
   double m_ug[FXAI_AI_MLP_HIDDEN][FXAI_AI_MLP_HIDDEN];

   double m_pi[FXAI_AI_MLP_HIDDEN];
   double m_pf[FXAI_AI_MLP_HIDDEN];
   double m_po[FXAI_AI_MLP_HIDDEN];
   ...
   double m_w_cls[FXAI_LSTMG_CLASS_COUNT][FXAI_AI_MLP_HIDDEN];
   double m_b_cls[FXAI_LSTMG_CLASS_COUNT];
   ...
   double m_ema_wi[FXAI_AI_MLP_HIDDEN][FXAI_AI_WEIGHTS];
   double m_ema_wf[FXAI_AI_MLP_HIDDEN][FXAI_AI_WEIGHTS];
   double m_ema_wo[FXAI_AI_MLP_HIDDEN][FXAI_AI_WEIGHTS];
   double m_ema_wg[FXAI_AI_MLP_HIDDEN][FXAI_AI_WEIGHTS];

   double m_ema_ui[FXAI_AI_MLP_HIDDEN][FXAI_AI_MLP_HIDDEN];
   double m_ema_uf[FXAI_AI_MLP_HIDDEN][FXAI_AI_MLP_HIDDEN];
   double m_ema_uo[FXAI_AI_MLP_HIDDEN][FXAI_AI_MLP_HIDDEN];
   double m_ema_ug[FXAI_AI_MLP_HIDDEN][FXAI_AI_MLP_HIDDEN];

   double m_ema_pi[FXAI_AI_MLP_HIDDEN];
   double m_ema_pf[FXAI_AI_MLP_HIDDEN];
   double m_ema_po[FXAI_AI_MLP_HIDDEN];
   ...
   double m_ema_w_cls[FXAI_LSTMG_CLASS_COUNT][FXAI_AI_MLP_HIDDEN];
   double m_ema_b_cls[FXAI_LSTMG_CLASS_COUNT];
   ...
   double m_batch_x[FXAI_LSTMG_TBPTT][FXAI_AI_WEIGHTS];
   int    m_batch_cls[FXAI_LSTMG_TBPTT];
   double m_batch_move[FXAI_LSTMG_TBPTT];
   double m_batch_cost[FXAI_LSTMG_TBPTT];
   double m_batch_w[FXAI_LSTMG_TBPTT];
   double m_batch_reset[FXAI_LSTMG_TBPTT];

   // Optimizer moments (grouped AdamW-style scaling).
   double m_opt_m[FXAI_LSTMG_OPT_GROUPS];
   double m_opt_v[FXAI_LSTMG_OPT_GROUPS];

   // Native multiclass calibrator.
   double m_cal3_temp;
   double m_cal3_bias[FXAI_LSTMG_CLASS_COUNT];
   double m_cal3_iso_pos[FXAI_LSTMG_CLASS_COUNT][FXAI_LSTMG_CAL_BINS];
   double m_cal3_iso_cnt[FXAI_LSTMG_CLASS_COUNT][FXAI_LSTMG_CAL_BINS];
   int    m_cal3_steps;
   ...
   double m_cls_count[4][FXAI_LSTMG_CLASS_COUNT];
   ...
   {
      double keep = 1.0 - FXAI_LSTMG_DROP_RATE;
      if(keep <= 0.05) keep = 0.05;
      double u = Hash01(m_step + 7, salt, idx);
      if(u < FXAI_LSTMG_DROP_RATE) return 0.0;
      return 1.0 / keep;
   }
                      ...
                      const double base_lr)
   {
      int g = group_idx;
      if(g < 0) g = 0;
      if(g >= FXAI_LSTMG_OPT_GROUPS) g = FXAI_LSTMG_OPT_GROUPS - 1;
                   ...
                   double &p_cal[]) const
   {
      ...
      double p_iso[FXAI_LSTMG_CLASS_COUNT];
      for(int c=0; c<FXAI_LSTMG_CLASS_COUNT; c++)
      {
         double total = 0.0;
         for(int b=0; b<FXAI_LSTMG_CAL_BINS; b++) total += m_cal3_iso_cnt[c][b];
         if(total < 30.0)
         {
            p_iso[c] = p_cal[c];
            continue;
         ...
         double mono[FXAI_LSTMG_CAL_BINS];
         double prev = 0.01;
         ...
         int bi = (int)MathFloor(p_cal[c] * (double)FXAI_LSTMG_CAL_BINS);
         if(bi < 0) bi = 0;
         if(bi >= FXAI_LSTMG_CAL_BINS) bi = FXAI_LSTMG_CAL_BINS - 1;
         p_iso[c] = mono[bi];
      }
                          ...
                          const double lr)
   {
      ...
      double p_cal[FXAI_LSTMG_CLASS_COUNT];
      Softmax3(logits, p_cal);
                            ...
                            const double base_lr)
   {
      ...
      double err = FXAI_ClipSym(y - p, 40.0);

      double w = FXAI_Clamp(sample_w, 0.25, 6.0);
      double lr = FXAI_Clamp(0.08 * base_lr * w, 0.00005, 0.01000);
      double reg = 0.0005;

      m_ev_a += lr * (err * x - reg * (m_ev_a - 1.0));
      m_ev_b += lr * err;

      m_ev_a = FXAI_Clamp(m_ev_a, 0.10, 4.00);
      m_ev_b = FXAI_Clamp(m_ev_b, -25.0, 25.0);
      m_ev_steps++;
                         ...
                         double &inv_std) const
   {
      ...
      inv_std = 1.0 / MathSqrt(var + FXAI_LSTMG_LN_EPS);
      for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
      {
   ...
   void InitWeights(void)
   {
      ...
      for(int c=0; c<FXAI_LSTMG_CLASS_COUNT; c++) m_b_cls[c] = 0.0;
                   ...
                   double &h_new[]) const
   {
      ...
      for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
      {
         ig[h] = FXAI_Sigmoid(zi_ln[h]);
         fg[h] = FXAI_Sigmoid(zf_ln[h]);
         gg[h] = FXAI_Tanh(zg_ln[h]);
                    ...
                    const double reset_flag)
   {
      ...
      for(int t=1; t<FXAI_LSTMG_TBPTT; t++)
      {
      ...
      int last = FXAI_LSTMG_TBPTT - 1;
      for(int i=0; i<FXAI_AI_WEIGHTS; i++) m_batch_x[last][i] = x[i];
                             ...
                             const int cls) const
   {
      ...
      int c = cls;
      if(c < 0) c = 0;
      if(c >= FXAI_LSTMG_CLASS_COUNT) c = FXAI_LABEL_SKIP;

      double cnt = m_cls_count[s][c] + 1.0;
      double tot = m_sess_total[s] + (double)FXAI_LSTMG_CLASS_COUNT;
      double ideal = tot / (double)FXAI_LSTMG_CLASS_COUNT;
      double w = MathSqrt(ideal / cnt);
      return FXAI_Clamp(w, 0.55, 2.50);
   }
                        ...
                        const double min_move_points) const
   {
      ...
      {
         if(y == 1) cls = (int)FXAI_LABEL_BUY;
         else if(y == 0) cls = (int)FXAI_LABEL_SELL;
         else cls = (move_points >= 0.0 ? (int)FXAI_LABEL_BUY : (int)FXAI_LABEL_SELL);
      }
   ...
   bool RunSelfTests(void)
   {
      ...
      double h0[FXAI_AI_MLP_HIDDEN];
      double c0[FXAI_AI_MLP_HIDDEN];
      for(int i=0; i<FXAI_AI_WEIGHTS; i++) x[i] = 0.05 * MathSin((double)(i + 1) * 0.31);
      for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
      {
      ...
      double drop1[FXAI_AI_MLP_HIDDEN], drop2[FXAI_AI_MLP_HIDDEN];
      double zone1[FXAI_AI_MLP_HIDDEN], zone2[FXAI_AI_MLP_HIDDEN];
      double zih1[FXAI_AI_MLP_HIDDEN], zfh1[FXAI_AI_MLP_HIDDEN], zoh1[FXAI_AI_MLP_HIDDEN], zgh1[FXAI_AI_MLP_HIDDEN];
      double zih2[FXAI_AI_MLP_HIDDEN], zfh2[FXAI_AI_MLP_HIDDEN], zoh2[FXAI_AI_MLP_HIDDEN], zgh2[FXAI_AI_MLP_HIDDEN];
      double ii1, if1, io1, ig1, ii2, if2, io2, ig2;
      double i1[FXAI_AI_MLP_HIDDEN], f1[FXAI_AI_MLP_HIDDEN], o1[FXAI_AI_MLP_HIDDEN], g1[FXAI_AI_MLP_HIDDEN], c1[FXAI_AI_MLP_HIDDEN], h1[FXAI_AI_MLP_HIDDEN];
      double i2[FXAI_AI_MLP_HIDDEN], f2[FXAI_AI_MLP_HIDDEN], o2[FXAI_AI_MLP_HIDDEN], g2[FXAI_AI_MLP_HIDDEN], c2[FXAI_AI_MLP_HIDDEN], h2[FXAI_AI_MLP_HIDDEN];
      ...
      double logits[FXAI_LSTMG_CLASS_COUNT];
      double probs[FXAI_LSTMG_CLASS_COUNT];
      double mu, lv, q25, q75;
      ComputeHeads(h1, false, logits, probs, mu, lv, q25, q75);

      int cls = (int)FXAI_LABEL_BUY;
      double analytic = (1.0 - probs[cls]) * h1[0];

      double eps = 1e-4;
      double orig = m_w_cls[cls][0];
      m_w_cls[cls][0] = orig + eps;
      double l1[FXAI_LSTMG_CLASS_COUNT], p1[FXAI_LSTMG_CLASS_COUNT];
      ComputeHeads(h1, false, l1, p1, mu, lv, q25, q75);
      double lp1 = MathLog(FXAI_Clamp(p1[cls], 1e-8, 1.0));

      m_w_cls[cls][0] = orig - eps;
      double l2[FXAI_LSTMG_CLASS_COUNT], p2[FXAI_LSTMG_CLASS_COUNT];
      ComputeHeads(h1, false, l2, p2, mu, lv, q25, q75);
      double lp2 = MathLog(FXAI_Clamp(p2[cls], 1e-8, 1.0));
      ...
      double x1[FXAI_AI_WEIGHTS], x2[FXAI_AI_WEIGHTS];
      double hs[FXAI_AI_MLP_HIDDEN], cs[FXAI_AI_MLP_HIDDEN];
      for(int i=0; i<FXAI_AI_WEIGHTS; i++)
      {
      ...
      double h_after1[FXAI_AI_MLP_HIDDEN], c_after1[FXAI_AI_MLP_HIDDEN];
      ForwardOne(x1, hs, cs, false, false, 5,
                 drop1, zone1, zih1, zfh1, zoh1, zgh1, ii1, if1, io1, ig1,
                 i1, f1, o1, g1, c_after1, h_after1);

      double h_noreset[FXAI_AI_MLP_HIDDEN], c_noreset[FXAI_AI_MLP_HIDDEN];
      ForwardOne(x2, h_after1, c_after1, false, false, 6,
                 drop1, zone1, zih1, zfh1, zoh1, zgh1, ii1, if1, io1, ig1,
                 i1, f1, o1, g1, c_noreset, h_noreset);

      double hz[FXAI_AI_MLP_HIDDEN], cz[FXAI_AI_MLP_HIDDEN];
      for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++) { hz[h] = 0.0; cz[h] = 0.0; }
      double h_reset[FXAI_AI_MLP_HIDDEN], c_reset[FXAI_AI_MLP_HIDDEN];
      ForwardOne(x2, hz, cz, false, false, 6,
                 drop1, zone1, zih1, zfh1, zoh1, zgh1, ii1, if1, io1, ig1,
                 i1, f1, o1, g1, c_reset, h_reset);

      // "Masked reset" behavior should match zero-state pass exactly.
      double h_mask[FXAI_AI_MLP_HIDDEN], c_mask[FXAI_AI_MLP_HIDDEN];
      ForwardOne(x2, hz, cz, false, false, 6,
                 drop2, zone2, zih2, zfh2, zoh2, zgh2, ii2, if2, io2, ig2,
      ...
      m_selftest_ok = ok;
      if(!ok)
         Print("FXAI lstmg self-test warning: one or more checks failed");
      return ok;
   }
   ...
   void TrainBatch(const FXAIAIHyperParams &hp)
   {
      ...
      double lr0 = FXAI_Clamp(hp.lr, 0.00005, 0.30000);
      ...
      double base_lr = FXAI_Clamp(lr0 * warm * invs * (0.65 + 0.35 * cosine), 0.00001, 0.05000);
      double l2 = FXAI_Clamp(hp.l2, 0.0000, 0.2000);

      double h_prev[FXAI_LSTMG_TBPTT][FXAI_AI_MLP_HIDDEN];
      double c_prev[FXAI_LSTMG_TBPTT][FXAI_AI_MLP_HIDDEN];

      double drop_mask[FXAI_LSTMG_TBPTT][FXAI_AI_MLP_HIDDEN];
      double zone_mask[FXAI_LSTMG_TBPTT][FXAI_AI_MLP_HIDDEN];

      double zi_hat[FXAI_LSTMG_TBPTT][FXAI_AI_MLP_HIDDEN];
      double zf_hat[FXAI_LSTMG_TBPTT][FXAI_AI_MLP_HIDDEN];
      double zo_hat[FXAI_LSTMG_TBPTT][FXAI_AI_MLP_HIDDEN];
      double zg_hat[FXAI_LSTMG_TBPTT][FXAI_AI_MLP_HIDDEN];

      double inv_i[FXAI_LSTMG_TBPTT];
      double inv_f[FXAI_LSTMG_TBPTT];
      double inv_o[FXAI_LSTMG_TBPTT];
      double inv_g[FXAI_LSTMG_TBPTT];

      double ig[FXAI_LSTMG_TBPTT][FXAI_AI_MLP_HIDDEN];
      double fg[FXAI_LSTMG_TBPTT][FXAI_AI_MLP_HIDDEN];
      double og[FXAI_LSTMG_TBPTT][FXAI_AI_MLP_HIDDEN];
      double gg[FXAI_LSTMG_TBPTT][FXAI_AI_MLP_HIDDEN];

      double c_t[FXAI_LSTMG_TBPTT][FXAI_AI_MLP_HIDDEN];
      double h_t[FXAI_LSTMG_TBPTT][FXAI_AI_MLP_HIDDEN];

      double logits[FXAI_LSTMG_TBPTT][FXAI_LSTMG_CLASS_COUNT];
      double probs[FXAI_LSTMG_TBPTT][FXAI_LSTMG_CLASS_COUNT];
      double mu[FXAI_LSTMG_TBPTT];
      double logv[FXAI_LSTMG_TBPTT];
      double q25[FXAI_LSTMG_TBPTT];
      double q75[FXAI_LSTMG_TBPTT];
      ...
      for(int t=0; t<len; t++)
      {
         ...
         double drop_loc[FXAI_AI_MLP_HIDDEN];
         double zone_loc[FXAI_AI_MLP_HIDDEN];
         double zi_loc[FXAI_AI_MLP_HIDDEN];
         double zf_loc[FXAI_AI_MLP_HIDDEN];
         double zo_loc[FXAI_AI_MLP_HIDDEN];
         double zg_loc[FXAI_AI_MLP_HIDDEN];
         ...
         double lo[FXAI_LSTMG_CLASS_COUNT];
         double pr[FXAI_LSTMG_CLASS_COUNT];
      ...
      // Gradients.
      double g_wi[FXAI_AI_MLP_HIDDEN][FXAI_AI_WEIGHTS];
      double g_wf[FXAI_AI_MLP_HIDDEN][FXAI_AI_WEIGHTS];
      double g_wo[FXAI_AI_MLP_HIDDEN][FXAI_AI_WEIGHTS];
      double g_wg[FXAI_AI_MLP_HIDDEN][FXAI_AI_WEIGHTS];

      double g_ui[FXAI_AI_MLP_HIDDEN][FXAI_AI_MLP_HIDDEN];
      double g_uf[FXAI_AI_MLP_HIDDEN][FXAI_AI_MLP_HIDDEN];
      double g_uo[FXAI_AI_MLP_HIDDEN][FXAI_AI_MLP_HIDDEN];
      double g_ug[FXAI_AI_MLP_HIDDEN][FXAI_AI_MLP_HIDDEN];

      double g_pi[FXAI_AI_MLP_HIDDEN];
      double g_pf[FXAI_AI_MLP_HIDDEN];
      double g_po[FXAI_AI_MLP_HIDDEN];
      ...
      double g_w_cls[FXAI_LSTMG_CLASS_COUNT][FXAI_AI_MLP_HIDDEN];
      double g_b_cls[FXAI_LSTMG_CLASS_COUNT];
      ...
      for(int c=0; c<FXAI_LSTMG_CLASS_COUNT; c++) g_b_cls[c] = 0.0;
      ...
      for(int t=len - 1; t>=0; t--)
      {
         ...
         double err_cls[FXAI_LSTMG_CLASS_COUNT];
         for(int c=0; c<FXAI_LSTMG_CLASS_COUNT; c++)
         {
            double target = (c == cls ? 1.0 : 0.0);
         ...
         double move_scale = FXAI_Clamp(0.65 * sw, 0.10, 8.00);
         ...
         double dh_total[FXAI_AI_MLP_HIDDEN];
         double dzo_ln[FXAI_AI_MLP_HIDDEN];
         double dc_mid[FXAI_AI_MLP_HIDDEN];
         ...
         for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
         {
            ...
            dzo_ln[h] = FXAI_ClipSym((dh * tanh_c) * og[t][h] * (1.0 - og[t][h]), 4.0);
            dc_mid[h] = dc_next[h] + dh * og[t][h] * (1.0 - tanh_c * tanh_c);
         }

         double dzo_raw[FXAI_AI_MLP_HIDDEN];
         double zo_row[FXAI_AI_MLP_HIDDEN];
         for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++) zo_row[h] = zo_hat[t][h];
         ...
         double dc_vec[FXAI_AI_MLP_HIDDEN];
         ...
         for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
         {
            ...
            di_ln[h] = FXAI_ClipSym((dc * gg[t][h]) * ig[t][h] * (1.0 - ig[t][h]), 4.0);
            df_ln[h] = FXAI_ClipSym((dc * c_prev[t][h]) * fg[t][h] * (1.0 - fg[t][h]), 4.0);
            dg_ln[h] = FXAI_ClipSym((dc * ig[t][h]) * (1.0 - gg[t][h] * gg[t][h]), 4.0);
         }

         double dzi_raw[FXAI_AI_MLP_HIDDEN];
         double dzf_raw[FXAI_AI_MLP_HIDDEN];
         double dzg_raw[FXAI_AI_MLP_HIDDEN];
         double zi_row[FXAI_AI_MLP_HIDDEN];
         double zf_row[FXAI_AI_MLP_HIDDEN];
         double zg_row[FXAI_AI_MLP_HIDDEN];
         for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
         {
            zi_row[h] = zi_hat[t][h];
            zf_row[h] = zf_hat[t][h];
         ...
         for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
         {
            ...
            double dzi = FXAI_ClipSym(dzi_raw[h], 4.0);
            double dzf = FXAI_ClipSym(dzf_raw[h], 4.0);
            double dzo = FXAI_ClipSym(dzo_raw[h], 4.0);
            double dzg = FXAI_ClipSym(dzg_raw[h], 4.0);
      ...
      for(int c=0; c<FXAI_LSTMG_CLASS_COUNT; c++) gnorm2 += g_b_cls[c] * g_b_cls[c];
      ...
      for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
      {
         ...
         for(int c=0; c<FXAI_LSTMG_CLASS_COUNT; c++) gnorm2 += g_w_cls[c][h] * g_w_cls[c][h];
      ...
      double wd_gate = FXAI_Clamp(l2 * 0.18, 0.0, 0.20);
      double wd_peep = FXAI_Clamp(l2 * 0.10, 0.0, 0.12);
      double wd_ln = FXAI_Clamp(l2 * 0.04, 0.0, 0.08);
      double wd_cls = FXAI_Clamp(l2 * 0.08, 0.0, 0.12);
      double wd_move = FXAI_Clamp(l2 * 0.06, 0.0, 0.10);
      ...
      for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
      {
         ...
         m_pi[h] = FXAI_ClipSym(m_pi[h] * (1.0 - lr_peep * wd_peep) + lr_peep * gscale * g_pi[h], 4.0);
         m_pf[h] = FXAI_ClipSym(m_pf[h] * (1.0 - lr_peep * wd_peep) + lr_peep * gscale * g_pf[h], 4.0);
         m_po[h] = FXAI_ClipSym(m_po[h] * (1.0 - lr_peep * wd_peep) + lr_peep * gscale * g_po[h], 4.0);

         m_ln_gi[h] = FXAI_Clamp(m_ln_gi[h] * (1.0 - lr_ln * wd_ln) + lr_ln * gscale * g_ln_gi[h], 0.20, 3.00);
         m_ln_gf[h] = FXAI_Clamp(m_ln_gf[h] * (1.0 - lr_ln * wd_ln) + lr_ln * gscale * g_ln_gf[h], 0.20, 3.00);
         m_ln_go[h] = FXAI_Clamp(m_ln_go[h] * (1.0 - lr_ln * wd_ln) + lr_ln * gscale * g_ln_go[h], 0.20, 3.00);
         m_ln_gg[h] = FXAI_Clamp(m_ln_gg[h] * (1.0 - lr_ln * wd_ln) + lr_ln * gscale * g_ln_gg[h], 0.20, 3.00);

         m_ln_bi[h] = FXAI_ClipSym(m_ln_bi[h] + lr_ln * gscale * g_ln_bi[h], 3.0);
         m_ln_bf[h] = FXAI_ClipSym(m_ln_bf[h] + lr_ln * gscale * g_ln_bf[h], 3.0);
         m_ln_bo[h] = FXAI_ClipSym(m_ln_bo[h] + lr_ln * gscale * g_ln_bo[h], 3.0);
         m_ln_bg[h] = FXAI_ClipSym(m_ln_bg[h] + lr_ln * gscale * g_ln_bg[h], 3.0);
      ...
      for(int c=0; c<FXAI_LSTMG_CLASS_COUNT; c++)
      {
         m_b_cls[c] = FXAI_ClipSym(m_b_cls[c] + lr_cls * gscale * g_b_cls[c], 6.0);
      }

      m_b_mu += lr_move * gscale * g_b_mu;
      m_b_logv = FXAI_ClipSym(m_b_logv + lr_move * gscale * g_b_logv, 6.0);
      ...
      for(int t=0; t<len; t++)
      {
         ...
         double p_raw[FXAI_LSTMG_CLASS_COUNT];
         p_raw[0] = probs[t][0];
         p_raw[1] = probs[t][1];
         p_raw[2] = probs[t][2];
         ...
         double ev_raw = ExpectedMoveFromHeads(mu[t], logv[t], q25[t], q75[t], p_raw[(int)FXAI_LABEL_SKIP]);
         UpdateEVCalibration(ev_raw, edge_tgt, sw, base_lr);

         double den = p_raw[(int)FXAI_LABEL_BUY] + p_raw[(int)FXAI_LABEL_SELL];
         if(den < 1e-9) den = 1e-9;
         double p_dir = p_raw[(int)FXAI_LABEL_BUY] / den;
...
public:
                                        ...
                                        FXAIAIModelOutputV4 &out)
   {
      ...
      BuildStateWithPending(h_state, c_state, use_ema);
      double drop_mask[FXAI_AI_MLP_HIDDEN], zone_mask[FXAI_AI_MLP_HIDDEN];
