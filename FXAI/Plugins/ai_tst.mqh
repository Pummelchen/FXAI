...
#define FXAI_TST_SEQ 48
#define FXAI_TST_TBPTT 16
#define FXAI_TST_HEADS 2
...
private:
   ...
   int    m_seq_ptr;
   int    m_seq_len;
   double m_seq_h[FXAI_TST_SEQ][FXAI_AI_MLP_HIDDEN];

   // Session-aware conditioning + calibration.
   double m_session_bias[FXAI_TST_SESSIONS][FXAI_AI_MLP_HIDDEN];
   double m_sess_cal_a[FXAI_TST_SESSIONS];
   double m_sess_cal_b[FXAI_TST_SESSIONS];
   int    m_sess_cal_steps[FXAI_TST_SESSIONS];
   bool   m_sess_cal_ready[FXAI_TST_SESSIONS];
   ...
   double m_wq[FXAI_TST_HEADS][FXAI_TST_D_HEAD][FXAI_AI_MLP_HIDDEN];
   double m_wk[FXAI_TST_HEADS][FXAI_TST_D_HEAD][FXAI_AI_MLP_HIDDEN];
   double m_wv[FXAI_TST_HEADS][FXAI_TST_D_HEAD][FXAI_AI_MLP_HIDDEN];
   double m_wo[FXAI_TST_HEADS][FXAI_AI_MLP_HIDDEN][FXAI_TST_D_HEAD];
   double m_rel_decay[FXAI_TST_HEADS];
   ...
   double m_w_cls[FXAI_TST_CLASS_COUNT][FXAI_AI_MLP_HIDDEN];
   double m_b_cls[FXAI_TST_CLASS_COUNT];
   ...
   double m_b_q75;
   double m_w_mu_h[FXAI_TST_HORIZONS][FXAI_AI_MLP_HIDDEN];
   double m_b_mu_h[FXAI_TST_HORIZONS];

   // TBPTT buffers.
   int    m_train_len;
   double m_train_x[FXAI_TST_TBPTT][FXAI_AI_WEIGHTS];
   int    m_train_cls[FXAI_TST_TBPTT];
   double m_train_move[FXAI_TST_TBPTT];
   double m_train_cost[FXAI_TST_TBPTT];
   double m_train_w[FXAI_TST_TBPTT];

   // Forward caches for TBPTT updates.
   double m_cache_embed[FXAI_TST_TBPTT][FXAI_AI_MLP_HIDDEN];
   double m_cache_local[FXAI_TST_TBPTT][FXAI_AI_MLP_HIDDEN];
   double m_cache_attn[FXAI_TST_TBPTT][FXAI_AI_MLP_HIDDEN];
   double m_cache_final[FXAI_TST_TBPTT][FXAI_AI_MLP_HIDDEN];
   double m_cache_group[FXAI_TST_TBPTT][3];
   double m_cache_ctx[FXAI_TST_TBPTT][FXAI_TST_HEADS][FXAI_TST_D_HEAD];

   double m_cache_logits[FXAI_TST_TBPTT][FXAI_TST_CLASS_COUNT];
   double m_cache_probs[FXAI_TST_TBPTT][FXAI_TST_CLASS_COUNT];
   double m_cache_mu[FXAI_TST_TBPTT];
   double m_cache_logv[FXAI_TST_TBPTT];
   double m_cache_q25[FXAI_TST_TBPTT];
   double m_cache_q75[FXAI_TST_TBPTT];
   double m_cache_mu_h[FXAI_TST_TBPTT][FXAI_TST_HORIZONS];

   // Attention caches used by gradient-correct updates.
   int    m_cache_tok_count[FXAI_TST_TBPTT];
   double m_cache_tok[FXAI_TST_TBPTT][FXAI_TST_SEQ][FXAI_AI_MLP_HIDDEN];
   double m_cache_qv[FXAI_TST_TBPTT][FXAI_TST_HEADS][FXAI_TST_D_HEAD];
   double m_cache_kv[FXAI_TST_TBPTT][FXAI_TST_HEADS][FXAI_TST_SEQ][FXAI_TST_D_HEAD];
   double m_cache_vv[FXAI_TST_TBPTT][FXAI_TST_HEADS][FXAI_TST_SEQ][FXAI_TST_D_HEAD];
   double m_cache_aw[FXAI_TST_TBPTT][FXAI_TST_HEADS][FXAI_TST_SEQ];
   ...
   double ScheduledLR(const FXAIAIHyperParams &hp) const
   {
      ...
      double warm = FXAI_Clamp(st / 192.0, 0.10, 1.00);
                      ...
                      const double base_lr)
   {
      ...
      return FXAI_Clamp(base_lr * (0.60 + 0.40 * scale), 0.000002, 0.100000);
   }
   ...
   {
      for(int s=0; s<FXAI_TST_SESSIONS; s++)
      {
         m_sess_cal_a[s] = 1.0;
         m_sess_cal_b[s] = 0.0;
                   ...
                   const double hardness)
   {
      ...
      m_replay_ptr++;
      if(m_replay_ptr >= FXAI_TST_REPLAY) m_replay_ptr = 0;
      if(m_replay_len < FXAI_TST_REPLAY) m_replay_len++;
   }
   ...
   {
      int s = sess;
      if(s < 0 || s >= FXAI_TST_SESSIONS) s = FXAI_TST_SESSIONS - 1;

      double pr = FXAI_Clamp(p_raw, 0.001, 0.999);
      double z = FXAI_Logit(pr);
      double py = FXAI_Sigmoid(m_sess_cal_a[s] * z + m_sess_cal_b[s]);
      ...
      m_sess_cal_a[s] = FXAI_Clamp(m_sess_cal_a[s], 0.20, 5.00);
      m_sess_cal_b[s] = FXAI_Clamp(m_sess_cal_b[s], -4.0, 4.0);
                               ...
                               const double p_raw) const
   {
      ...
      double ps = FXAI_Sigmoid(m_sess_cal_a[s] * FXAI_Logit(FXAI_Clamp(p0, 0.001, 0.999)) + m_sess_cal_b[s]);
      return FXAI_Clamp(0.65 * p0 + 0.35 * ps, 0.001, 0.999);
   }
   ...
   {
      m_train_len = 0;
      for(int t=0; t<FXAI_TST_TBPTT; t++)
      {
         m_train_cls[t] = FXAI_TST_SKIP;
         m_train_move[t] = 0.0;
         m_train_cost[t] = 0.0;
         m_train_w[t] = 1.0;
         ...
         for(int hz=0; hz<FXAI_TST_HORIZONS; hz++)
         ...
         for(int hd=0; hd<FXAI_TST_HEADS; hd++)
         {
            for(int d=0; d<FXAI_TST_D_HEAD; d++)
            {
         ...
         for(int tok=0; tok<FXAI_TST_SEQ; tok++)
         {
            for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
         ...
         for(int c=0; c<FXAI_TST_CLASS_COUNT; c++)
         {
   ...
   int SeqIndexBack(const int back) const
   {
      ...
      while(idx < 0) idx += FXAI_TST_SEQ;
      while(idx >= FXAI_TST_SEQ) idx -= FXAI_TST_SEQ;
      return idx;
   }
   ...
   {
      if(y == FXAI_TST_SELL || y == FXAI_TST_BUY || y == FXAI_TST_SKIP)
         return y;

      double cost = InputCostProxyPoints(x);
      double edge = MathAbs(move_points) - cost;
      double skip_band = 0.10 + 0.25 * MathMax(cost, 0.0);
      if(edge <= skip_band) return FXAI_TST_SKIP;

      if(y > 0) return FXAI_TST_BUY;
      if(y == 0) return FXAI_TST_SELL;
      return (move_points >= 0.0 ? FXAI_TST_BUY : FXAI_TST_SELL);
   }
   ...
   void ConfigureFromHP(const FXAIAIHyperParams &hp)
   {
      int d = (int)MathRound(hp.tcn_layers);
      if(d < 2) d = 2;
      if(d > FXAI_TST_MAX_STACK) d = FXAI_TST_MAX_STACK;
      m_stack_depth = d;

      double k = FXAI_Clamp(hp.tcn_kernel, 2.0, 7.0);
      m_drop_rate = FXAI_Clamp(0.02 * k, 0.05, 0.22);
      m_stoch_rate = FXAI_Clamp(0.01 * k, 0.03, 0.18);
      m_cfg_ready = true;
   }
   ...
   {
      int keep = m_seq_len;
      if(keep > FXAI_TST_SEQ - 1) keep = FXAI_TST_SEQ - 1;
      ...
      for(int i=0; i<=keep; i++)
      {
         ...
         tokens[i][0] = FXAI_ClipSym(tokens[i][0] + e0, 8.0);
         if(FXAI_AI_MLP_HIDDEN > 1) tokens[i][1] = FXAI_ClipSym(tokens[i][1] + e1, 8.0);
         if(FXAI_AI_MLP_HIDDEN > 2) tokens[i][2] = FXAI_ClipSym(tokens[i][2] + e2, 8.0);
         if(FXAI_AI_MLP_HIDDEN > 3) tokens[i][3] = FXAI_ClipSym(tokens[i][3] + e3, 8.0);
      }
                           ...
                           const int cache_slot)
   {
      ...
      double inv_scale = 1.0 / MathSqrt((double)FXAI_TST_D_HEAD);
      double q[FXAI_TST_D_HEAD];
      double scores[FXAI_TST_SEQ];
      double v_cache[FXAI_TST_SEQ][FXAI_TST_D_HEAD];
      ...
      for(int hd=0; hd<FXAI_TST_HEADS; hd++)
      {
         ...
         for(int t=0; t<token_count; t++)
         {
            ...
            for(int d=0; d<FXAI_TST_D_HEAD; d++)
            {
               ...
               sc += q[d] * kv;
               v_cache[t][d] = vv;
               if(cache_train && cache_slot >= 0 && cache_slot < FXAI_TST_TBPTT)
               {
                    ...
                    double &head_ctx[][FXAI_TST_D_HEAD])
   {
      ...
      for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
         state_embed[h] = FXAI_ClipSym(state_embed[h] + 0.08 * m_session_bias[sess][h], 8.0);

      double layer_in[FXAI_AI_MLP_HIDDEN];
      for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++) layer_in[h] = state_embed[h];
      ...
      for(int layer=0; layer<m_stack_depth; layer++)
      {
         ...
         double tokens[FXAI_TST_SEQ][FXAI_AI_MLP_HIDDEN];
         int token_count = 0;
         BuildTokens(state_local, tokens, token_count);
         if(training && cache_slot >= 0 && cache_slot < FXAI_TST_TBPTT)
         {
         ...
         for(int o=0; o<FXAI_AI_MLP_HIDDEN; o++)
         {
            ...
            ff1[o] = FXAI_Tanh(z1) * DropMask(cache_slot + 1, layer, o, training);
         }
         ...
         for(int o=0; o<FXAI_AI_MLP_HIDDEN; o++)
         {
            ...
            state_final[o] = state_final[o] + stoch * (0.25 * FXAI_ClipSym(z2, 8.0));
         }
         ...
         if(layer < m_stack_depth - 1)
         {
            ...
            for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
               layer_in[h] = FXAI_ClipSym(blend * state_final[h] + (1.0 - blend) * layer_in[h], 8.0);
         }
      }
      ...
      {
         m_seq_ptr++;
         if(m_seq_ptr >= FXAI_TST_SEQ) m_seq_ptr = 0;
         for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
            m_seq_h[m_seq_ptr][h] = state_final[h];
         if(m_seq_len < FXAI_TST_SEQ) m_seq_len++;
      }
   }
                     ...
                     double &mu_h2) const
   {
      ...
      if(MathAbs(mh1) < MathAbs(mh0))
         mh1 = FXAI_Sign(mh1 == 0.0 ? mh0 : mh1) * MathAbs(mh0);
      if(MathAbs(mh2) < MathAbs(mh1))
         mh2 = FXAI_Sign(mh2 == 0.0 ? mh1 : mh2) * MathAbs(mh1);
                          ...
                          const double sample_w)
   {
      ...
      for(int t=1; t<FXAI_TST_TBPTT; t++)
      {
         for(int i=0; i<FXAI_AI_WEIGHTS; i++)
      ...
      for(int i=0; i<FXAI_AI_WEIGHTS; i++)
         m_train_x[FXAI_TST_TBPTT - 1][i] = x[i];
      m_train_cls[FXAI_TST_TBPTT - 1] = cls;
      m_train_move[FXAI_TST_TBPTT - 1] = move_points;
      m_train_cost[FXAI_TST_TBPTT - 1] = cost_points;
      m_train_w[FXAI_TST_TBPTT - 1] = sample_w;
   }
   ...
   void TrainTBPTT(const FXAIAIHyperParams &hp)
   {
      ...
      int n = m_train_len;
      if(n > FXAI_TST_TBPTT) n = FXAI_TST_TBPTT;
      if(n <= 0) return;

      double lr_base = ScheduledLR(hp);
      double l2 = FXAI_Clamp(hp.l2, 0.0000, 1.0000);
      double wd = FXAI_Clamp(0.25 * l2, 0.0, 0.10);

      int cls_count[FXAI_TST_CLASS_COUNT];
      for(int c=0; c<FXAI_TST_CLASS_COUNT; c++) cls_count[c] = 0;
      for(int t=0; t<n; t++)
      {
      ...
      for(int t=n - 1; t>=0; t--)
      {
         ...
         double ev_scale = FXAI_Clamp(0.60 + edge / MathMax(cost, 1.0), 0.25, 4.00);

         double w_cls = ClassWeight(cls, mv, cost, sw);
         double w_mv = MoveWeight(mv, cost, sw);
         w_cls = FXAI_Clamp(w_cls * ev_scale, 0.25, 8.00);
         w_mv = FXAI_Clamp(w_mv * ev_scale, 0.25, 10.00);

         int cc = cls;
         if(cc < 0 || cc >= FXAI_TST_CLASS_COUNT) cc = FXAI_TST_SKIP;
         double invf = (cls_count[cc] > 0 ? ((double)n / ((double)FXAI_TST_CLASS_COUNT * (double)cls_count[cc])) : 1.0);
         w_cls = FXAI_Clamp(w_cls * FXAI_Clamp(invf, 0.50, 2.50), 0.25, 10.0);

         double g_logits[FXAI_TST_CLASS_COUNT];
         for(int c=0; c<FXAI_TST_CLASS_COUNT; c++)
         {
         ...
         double target_h[FXAI_TST_HORIZONS];
         target_h[0] = target_move;
         target_h[1] = FXAI_ClipSym(1.40 * target_move, 80.0);
         target_h[2] = FXAI_ClipSym(1.90 * target_move, 120.0);
         double g_mu_h[FXAI_TST_HORIZONS];
         for(int hz=0; hz<FXAI_TST_HORIZONS; hz++)
            g_mu_h[hz] = 0.0;
         ...
         for(int hz=0; hz<FXAI_TST_HORIZONS; hz++)
         {
            double err_h = FXAI_ClipSym(m_cache_mu_h[t][hz] - target_h[hz], 30.0);
            double hw = (hz == 0 ? 0.60 : (hz == 1 ? 0.28 : 0.12));
            g_mu_h[hz] = FXAI_ClipSym(hw * w_mv * err_h / MathMax(MathSqrt(sigma2), 0.25), 4.0);
         }
         ...
         for(int c=0; c<FXAI_TST_CLASS_COUNT; c++) gn += g_logits[c] * g_logits[c];
         gn += g_mu * g_mu + g_logv * g_logv + g_q25 * g_q25 + g_q75 * g_q75;
         for(int hz=0; hz<FXAI_TST_HORIZONS; hz++) gn += g_mu_h[hz] * g_mu_h[hz];
         ...
         if(gn > 5.0)
         {
            ...
            for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
               delta[h] *= sc;
            for(int c=0; c<FXAI_TST_CLASS_COUNT; c++) g_logits[c] *= sc;
            g_mu *= sc;
            g_logv *= sc;
            g_q25 *= sc;
            g_q75 *= sc;
            for(int hz=0; hz<FXAI_TST_HORIZONS; hz++) g_mu_h[hz] *= sc;
         }

         // Head updates.
         double gmag_head = MathAbs(g_mu) + MathAbs(g_logv) + MathAbs(g_q25) + MathAbs(g_q75);
         for(int hz=0; hz<FXAI_TST_HORIZONS; hz++) gmag_head += MathAbs(g_mu_h[hz]);
         for(int c=0; c<FXAI_TST_CLASS_COUNT; c++) gmag_head += MathAbs(g_logits[c]);
         ...
         for(int c=0; c<FXAI_TST_CLASS_COUNT; c++)
         {
            ...
            for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
            {
               ...
               m_w_cls[c][h] -= lr_head * FXAI_ClipSym(g, 6.0);
         ...
         for(int hz=0; hz<FXAI_TST_HORIZONS; hz++)
            m_b_mu_h[hz] = FXAI_ClipSym(m_b_mu_h[hz], 30.0);

         for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
         {
            m_w_mu[h] -= lr_head * FXAI_ClipSym(g_mu * m_cache_final[t][h], 6.0);
            m_w_logv[h] -= lr_head * FXAI_ClipSym(g_logv * m_cache_final[t][h], 6.0);
            m_w_q25[h] -= lr_head * FXAI_ClipSym(g_q25 * m_cache_final[t][h], 6.0);
            m_w_q75[h] -= lr_head * FXAI_ClipSym(g_q75 * m_cache_final[t][h], 6.0);
            ...
            for(int hz=0; hz<FXAI_TST_HORIZONS; hz++)
            {
               m_w_mu_h[hz][h] -= lr_head * FXAI_ClipSym(g_mu_h[hz] * m_cache_final[t][h], 5.0);
         ...
         for(int o=0; o<FXAI_AI_MLP_HIDDEN; o++)
         {
            ...
            for(int i=0; i<FXAI_AI_MLP_HIDDEN; i++)
            {
               m_ff2_w[o][i] -= lr_ffn * (0.08 * FXAI_ClipSym(d * m_cache_final[t][i], 4.0));
               m_ff1_w[o][i] -= lr_ffn * (0.04 * FXAI_ClipSym(d * m_cache_local[t][i], 4.0));
         ...
         for(int o=0; o<FXAI_AI_MLP_HIDDEN; o++)
         {
            ...
            for(int i=0; i<FXAI_AI_MLP_HIDDEN; i++)
            {
               m_grn2_w2[o][i] -= lr_core * (0.18 * FXAI_ClipSym(d * m_cache_attn[t][i], 4.0));
               m_grn2_w1[o][i] -= lr_core * (0.10 * FXAI_ClipSym(d * m_cache_local[t][i], 4.0));
               m_grn2_wg[o][i] -= lr_core * (0.06 * FXAI_ClipSym(d * m_cache_attn[t][i], 4.0));
               m_attn_wg[o][i] -= lr_core * (0.05 * FXAI_ClipSym(d * m_cache_local[t][i], 4.0));
         ...
         double head_delta[FXAI_TST_HEADS][FXAI_TST_D_HEAD];
         int token_count = m_cache_tok_count[t];
         if(token_count < 1) token_count = 1;
         if(token_count > FXAI_TST_SEQ) token_count = FXAI_TST_SEQ;
         double inv_scale = 1.0 / MathSqrt((double)FXAI_TST_D_HEAD);
            ...
            for(int d=0; d<FXAI_TST_D_HEAD; d++)
            {
               ...
               for(int tok=0; tok<token_count; tok++)
               {
                  ...
                  for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
                  {
                     ...
                     m_wk[hd][d][h] -= lr_attn * FXAI_ClipSym(dk * tokh, 3.0);
                     m_wv[hd][d][h] -= lr_attn * FXAI_ClipSym((m_cache_aw[t][hd][tok] * dh) * tokh, 3.0);
               ...
               for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
               {
                  m_wo[hd][h][d] -= lr_attn * 0.08 * FXAI_ClipSym(delta[h] * m_cache_ctx[t][hd][d], 4.0);
                  m_wq[hd][d][h] -= lr_attn * 0.04 * FXAI_ClipSym(dq * m_cache_local[t][h], 4.0);
                  m_wo[hd][h][d] -= lr_attn * wd * m_wo[hd][h][d];
                  m_wq[hd][d][h] -= lr_attn * wd * m_wq[hd][d][h];
               }
            }

            m_rel_decay[hd] -= lr_attn * FXAI_ClipSym(rel_grad, 3.0);
            m_rel_decay[hd] = FXAI_Clamp(m_rel_decay[hd], -0.5, 0.5);
         }
         ...
         for(int o=0; o<FXAI_AI_MLP_HIDDEN; o++)
         {
            ...
            for(int i=0; i<FXAI_AI_MLP_HIDDEN; i++)
            {
               m_grn1_w2[o][i] -= lr_core * (0.08 * FXAI_ClipSym(d * m_cache_local[t][i], 4.0));
               m_grn1_w1[o][i] -= lr_core * (0.05 * FXAI_ClipSym(d * m_cache_embed[t][i], 4.0));
               m_grn1_wg[o][i] -= lr_core * (0.03 * FXAI_ClipSym(d * m_cache_local[t][i], 4.0));
         ...
         for(int g=0; g<3; g++)
         {
            ...
            m_v_gate_b[g] -= lr_core * 0.004 * FXAI_ClipSym(gs, 3.0);

            for(int i=0; i<FXAI_AI_WEIGHTS; i++)
            {
               double xg = xn[i] * m_group_mask[g][i];
               m_v_gate_w[g][i] -= lr_core * (0.002 * FXAI_ClipSym(gs * xg, 3.0));
         ...
         for(int o=0; o<FXAI_AI_MLP_HIDDEN; o++)
         {
            ...
            for(int i=0; i<FXAI_AI_WEIGHTS; i++)
            {
               ...
               m_w_obs[o][i] -= lr_core * (0.02 * FXAI_ClipSym(d * a0, 3.0));
               m_w_known[o][i] -= lr_core * (0.02 * FXAI_ClipSym(d * a1, 3.0));
               m_w_static[o][i] -= lr_core * (0.02 * FXAI_ClipSym(d * a2, 3.0));
         ...
         double den = m_cache_probs[t][FXAI_TST_BUY] + m_cache_probs[t][FXAI_TST_SELL];
         if(den < 1e-9) den = 1e-9;
         double p_dir_raw = m_cache_probs[t][FXAI_TST_BUY] / den;
         int sess = SessionBucketNow();
         if(cls == FXAI_TST_BUY)
         {
         ...
         for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
         {
            m_session_bias[sess][h] -= lr_core * 0.005 * FXAI_ClipSym(delta[h], 3.0);
            m_session_bias[sess][h] = FXAI_ClipSym(m_session_bias[sess][h], 4.0);
         }
...
public:
                                        ...
                                        double &expected_move_points)
   {
      ...
      double grp[3];
      double ctx[FXAI_TST_HEADS][FXAI_TST_D_HEAD];
      ForwardStep(xa, false, false, -1, emb, loc, att, fin, grp, ctx);
