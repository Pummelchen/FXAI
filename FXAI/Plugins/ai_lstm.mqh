...
private:
   ...
   double DotX(const double &w[][FXAI_AI_WEIGHTS], const int row, const double &x[]) const
   {
      double s = 0.0;
      for(int i=0; i<FXAI_AI_WEIGHTS; i++) s += w[row][i] * x[i];
      return s;
   }

   double DotH(const double &w[][FXAI_AI_MLP_HIDDEN], const int row, const double &hprev[]) const
   {
      double s = 0.0;
      for(int i=0; i<FXAI_AI_MLP_HIDDEN; i++) s += w[row][i] * hprev[i];
      return s;
   }
                   ...
                   double &h_new[]) const
   {
      ...
      for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
      {
         ig[h] = FXAI_Sigmoid(DotX(m_wi_x, h, x) + DotH(m_wi_h, h, h_in) + m_bi[h]);
         fg[h] = FXAI_Sigmoid(DotX(m_wf_x, h, x) + DotH(m_wf_h, h, h_in) + m_bf[h]);
         og[h] = FXAI_Sigmoid(DotX(m_wo_x, h, x) + DotH(m_wo_h, h, h_in) + m_bo[h]);
         gg[h] = FXAI_Tanh(DotX(m_wg_x, h, x) + DotH(m_wg_h, h, h_in) + m_bg[h]);
   ...
   void TrainBatch(const FXAIAIHyperParams &hp)
   {
      ...
      double base_lr0 = FXAI_Clamp(hp.lr, 0.0001, 0.5000);
      double warm = FXAI_Clamp((double)m_step / 256.0, 0.10, 1.00);
      double decay = 1.0 / MathSqrt(1.0 + 0.0010 * MathMax(0.0, (double)m_step - 256.0));
      double base_lr = FXAI_Clamp(base_lr0 * warm * decay, 0.00002, 0.05000);
      ...
      for(int t=len - 1; t>=0; t--)
      {
         ...
         for(int h=0; h<FXAI_AI_MLP_HIDDEN; h++)
         {
            ...
            di = FXAI_ClipSym(di, 4.0);
            df = FXAI_ClipSym(df, 4.0);
            doo = FXAI_ClipSym(doo, 4.0);
            dg = FXAI_ClipSym(dg, 4.0);
