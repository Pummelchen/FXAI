#ifndef __FXAI_AI_QUANTILE_MQH__
#define __FXAI_AI_QUANTILE_MQH__

#include "..\..\FXDataEngine\API\plugin_base.mqh"

#define FXAI_QT_Q      9
#define FXAI_QT_MID    4
#define FXAI_QT_SIDE   4
#define FXAI_QT_SESS   4
#define FXAI_QT_REG    3
#define FXAI_QT_ZF     8
#define FXAI_QT_CALF   8

class CFXAIAIQuantile : public CFXAIAIPlugin
{
private:
   bool   m_initialized;
   ...
   // Short-horizon head.
   double m_w_med_s[FXAI_AI_WEIGHTS];
   double m_w_up_s[FXAI_QT_SIDE][FXAI_AI_WEIGHTS];
   double m_w_dn_s[FXAI_QT_SIDE][FXAI_AI_WEIGHTS];

   // Medium-horizon head.
   double m_w_med_m[FXAI_AI_WEIGHTS];
   double m_w_up_m[FXAI_QT_SIDE][FXAI_AI_WEIGHTS];
   double m_w_dn_m[FXAI_QT_SIDE][FXAI_AI_WEIGHTS];

   // AdaGrad accumulators.
   double m_g2_med_s[FXAI_AI_WEIGHTS];
   double m_g2_up_s[FXAI_QT_SIDE][FXAI_AI_WEIGHTS];
   double m_g2_dn_s[FXAI_QT_SIDE][FXAI_AI_WEIGHTS];

   double m_g2_med_m[FXAI_AI_WEIGHTS];
   double m_g2_up_m[FXAI_QT_SIDE][FXAI_AI_WEIGHTS];
   double m_g2_dn_m[FXAI_QT_SIDE][FXAI_AI_WEIGHTS];

   // Session/regime conditioning.
   double m_sess_bias[2][FXAI_QT_SESS];
   double m_g2_sess_bias[2][FXAI_QT_SESS];

   double m_reg_scale[2][FXAI_QT_REG];
   double m_g2_reg_scale[2][FXAI_QT_REG];

   // Native 3-class head on quantile features.
   double m_cls_w[3][FXAI_QT_ZF];
   double m_cls_g2[3][FXAI_QT_ZF];

   // Learned direction calibrator over quantile features.
   double m_cal_w_q[FXAI_QT_CALF];
   double m_cal_g2_q[FXAI_QT_CALF];
   ...
   void Softmax3(const double &logits[], double &probs[]) const
   {
      ...
      for(int c=0; c<3; c++)
      {
         probs[c] = MathExp(FXAI_ClipSym(logits[c] - m, 30.0));
         den += probs[c];
      }
      if(den <= 0.0) den = 1.0;
                  ...
                  double &q[]) const
   {
      ...
      if(h == 0)
      {
         ...
         scale = FXAI_Clamp(m_reg_scale[0][reg], 0.40, 2.80);
      }
      else
      {
         med = DotVec(m_w_med_m, x) + m_sess_bias[1][sess];
         scale = FXAI_Clamp(m_reg_scale[1][reg], 0.40, 2.80);
      }
      ...
      for(int j=0; j<FXAI_QT_SIDE; j++)
      {
         ...
         q[FXAI_QT_MID + 1 + j] = q[FXAI_QT_MID + j] - 0.0 + gap_up;
         q[FXAI_QT_MID - 1 - j] = q[FXAI_QT_MID - j] - gap_dn;
      }

      // Hard monotonic guard for numerical safety.
      for(int k=1; k<FXAI_QT_Q; k++)
      {
         if(q[k] < q[k - 1] + 1e-4)
            q[k] = q[k - 1] + 1e-4;
      }
                       ...
                       const double wd)
   {
      for(int i=0; i<FXAI_AI_WEIGHTS; i++)
      {
         ...
         if(h == 0)
         {
            ...
            m_w_med_s[i] += step * g;
            m_w_med_s[i] = FXAI_ClipSym(m_w_med_s[i], 10.0);
         }
         else
         {
            m_g2_med_m[i] += g * g;
            ...
            m_w_med_m[i] += step * g;
            m_w_med_m[i] = FXAI_ClipSym(m_w_med_m[i], 10.0);
         }
      }
   }
                      ...
                      const double wd)
   {
      for(int i=0; i<FXAI_AI_WEIGHTS; i++)
      {
         ...
         if(h == 0)
         {
            ...
            m_w_up_s[j][i] += step * g;
            m_w_up_s[j][i] = FXAI_ClipSym(m_w_up_s[j][i], 10.0);
         }
         else
         {
            m_g2_up_m[j][i] += g * g;
            ...
            m_w_up_m[j][i] += step * g;
            m_w_up_m[j][i] = FXAI_ClipSym(m_w_up_m[j][i], 10.0);
         }
      }
   }
                      ...
                      const double wd)
   {
      for(int i=0; i<FXAI_AI_WEIGHTS; i++)
      {
         ...
         if(h == 0)
         {
            ...
            m_w_dn_s[j][i] += step * g;
            m_w_dn_s[j][i] = FXAI_ClipSym(m_w_dn_s[j][i], 10.0);
         }
         else
         {
            m_g2_dn_m[j][i] += g * g;
            ...
            m_w_dn_m[j][i] += step * g;
            m_w_dn_m[j][i] = FXAI_ClipSym(m_w_dn_m[j][i], 10.0);
         }
      }
   }
   ...
   void BuildClassFeatures(const double &q_short[],
                           ...
                           double &zf[]) const
   {
      double spread_s = q_short[FXAI_QT_Q - 1] - q_short[0];
      if(spread_s < 0.05) spread_s = 0.05;

      double spread_m = q_medium[FXAI_QT_Q - 1] - q_medium[0];
      if(spread_m < 0.05) spread_m = 0.05;

      double spread_b = q_blend[FXAI_QT_Q - 1] - q_blend[0];
      if(spread_b < 0.05) spread_b = 0.05;

      double ev_buy = 0.5 * (q_blend[FXAI_QT_MID] + q_blend[FXAI_QT_Q - 1]) - cost;
      double ev_sell = 0.5 * (-q_blend[FXAI_QT_MID] - q_blend[0]) - cost;

      zf[0] = 1.0;
      zf[1] = FXAI_ClipSym(q_short[FXAI_QT_MID] / spread_s, 8.0);
      zf[2] = FXAI_ClipSym(spread_s, 20.0);
      zf[3] = FXAI_ClipSym(q_medium[FXAI_QT_MID] / spread_m, 8.0);
      zf[4] = FXAI_ClipSym(spread_m, 20.0);
      zf[5] = FXAI_ClipSym((ev_buy - ev_sell) / spread_b, 8.0);
      zf[6] = FXAI_ClipSym(cost, 12.0);
      zf[7] = FXAI_ClipSym((ArraySize(x) > 6 ? MathAbs(x[6]) : 0.0), 8.0);
   }
                        ...
                        double &probs_out[])
   {
      ...
      double zf_norm2 = 0.0;
      for(int i=0; i<FXAI_QT_ZF; i++) zf_norm2 += zf[i] * zf[i];
      ...
      for(int c=0; c<3; c++)
      {
         ...
         for(int i=0; i<FXAI_QT_ZF; i++)
         {
            ...
            m_cls_w[c][i] += step * g;
            m_cls_w[c][i] = FXAI_ClipSym(m_cls_w[c][i], 10.0);
         }
      }
   }
                         ...
                         double &cf[]) const
   {
      ...
      cf[0] = 1.0;
      cf[1] = FXAI_ClipSym(q_blend[FXAI_QT_MID] / spread, 8.0);
      cf[2] = FXAI_ClipSym((ev_buy - ev_sell) / spread, 8.0);
      cf[3] = FXAI_ClipSym(class_probs[(int)FXAI_LABEL_BUY] - class_probs[(int)FXAI_LABEL_SELL], 1.0);
      cf[4] = FXAI_ClipSym(class_probs[(int)FXAI_LABEL_SKIP], 1.0);
      cf[5] = FXAI_ClipSym(cost, 12.0);
      cf[6] = FXAI_ClipSym((ArraySize(x) > 6 ? MathAbs(x[6]) : 0.0), 8.0);
      cf[7] = FXAI_ClipSym(1.0 / (spread + 0.10), 8.0);
   }

   double PredictDirectionCal(const double &cf[]) const
   {
      return FXAI_Sigmoid(DotCal(cf));
   }
                                  ...
                                  const double wd)
   {
      ...
      double cf_norm2 = 0.0;
      for(int i=0; i<FXAI_QT_CALF; i++) cf_norm2 += cf[i] * cf[i];
      ...
      for(int i=0; i<FXAI_QT_CALF; i++)
      {
         ...
         m_cal_w_q[i] += step * g;
         m_cal_w_q[i] = FXAI_ClipSym(m_cal_w_q[i], 10.0);
      }
   }
                    ...
                    const double &q[]) const
   {
      if(target <= q[0])
         return 0.01;
      if(target >= q[FXAI_QT_Q - 1])
         return 0.99;

      for(int k=0; k<FXAI_QT_Q - 1; k++)
      {
         if(target >= q[k] && target <= q[k + 1])
         {
            ...
            return FXAI_Clamp(pit, 0.001, 0.999);
         }
      }
      return 0.50;
   }
                          ...
                          const double &class_probs[])
   {
      ...
      int c = cls;
      if(c < 0 || c > 2) c = (int)FXAI_LABEL_SKIP;
      double p_true = FXAI_Clamp(class_probs[c], 0.0, 1.0);
      ...
      if(m_diag_n >= 25)
      {
         ...
         double target_rel = FXAI_Clamp(1.25 - drift, 0.35, 1.50);
         m_rel_weight = FXAI_Clamp((0.97 * m_rel_weight) + (0.03 * target_rel), 0.25, 1.75);
      }
   }
                          ...
                          const double wd)
   {
      ...
      double z_up[FXAI_QT_SIDE], z_dn[FXAI_QT_SIDE];
      double sp_up[FXAI_QT_SIDE], sp_dn[FXAI_QT_SIDE];
      double sig_up[FXAI_QT_SIDE], sig_dn[FXAI_QT_SIDE];
      double scale = FXAI_Clamp(m_reg_scale[h][reg], 0.40, 2.80);
      ...
      for(int k=0; k<FXAI_QT_Q; k++)
      {
         ...
         gq[k] = FXAI_ClipSym(gq[k], 2.0);
      }

      double g_med = 0.0;
      for(int k=0; k<FXAI_QT_Q; k++) g_med += gq[k];

      double g_up[FXAI_QT_SIDE];
      double g_dn[FXAI_QT_SIDE];
      double g_scale = 0.0;

      for(int j=0; j<FXAI_QT_SIDE; j++)
      {
         double sum_up = 0.0;
         for(int k=FXAI_QT_MID + 1 + j; k<FXAI_QT_Q; k++)
            sum_up += gq[k];

         double sum_dn = 0.0;
         for(int k=0; k<=FXAI_QT_MID - 1 - j; k++)
            sum_dn += gq[k];
      ...
      double x_norm2 = 0.0;
      for(int i=0; i<FXAI_AI_WEIGHTS; i++) x_norm2 += x[i] * x[i];
...
public:
   ...
   virtual void Update(const int y,
                       ...
                       const FXAIAIHyperParams &hp)
   {
      double pseudo_move = (y == (int)FXAI_LABEL_BUY ? 1.0 : (y == (int)FXAI_LABEL_SELL ? -1.0 : 0.0));
                               ...
                               const double move_points)
   {
      ...
      double lr = FXAI_Clamp(hp.quantile_lr, 0.00005, 0.05000);
      double wd = FXAI_Clamp(hp.quantile_l2, 0.00000, 0.05000);
      ...
      double den = class_probs[(int)FXAI_LABEL_BUY] + class_probs[(int)FXAI_LABEL_SELL];
      if(den < 1e-9) den = 1e-9;
      double p_dir_raw = class_probs[(int)FXAI_LABEL_BUY] / den;
   ...
   virtual double PredictProb(const double &x[], const FXAIAIHyperParams &hp)
   {
      ...
      p_dir = FXAI_Clamp(p_dir, 0.001, 0.999);
      ...
      p_up_raw = FXAI_Clamp(p_up_raw, 0.001, 0.999);
...
#endif // __FXAI_AI_QUANTILE_MQH__
