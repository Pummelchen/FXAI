...
#define FXAI_ENH_FIELDS 5
#define FXAI_ENH_FPAIRS (FXAI_ENH_FIELDS * FXAI_ENH_FIELDS)
#define FXAI_ENH_SESSIONS 4
#define FXAI_ENH_REGIMES 3
#define FXAI_ENH_REHASH_PERIOD 512
...
private:
   ...
   double m_w_lin[FXAI_ENH_CLASSES][FXAI_AI_WEIGHTS];

   // Field-aware hashed interactions with dual-table rehash windows.
   double m_z_hash[FXAI_ENH_CLASSES][FXAI_ENH_TABLES][FXAI_ENH_FPAIRS][FXAI_ENHASH_BUCKETS];
   double m_n_hash[FXAI_ENH_CLASSES][FXAI_ENH_TABLES][FXAI_ENH_FPAIRS][FXAI_ENHASH_BUCKETS];
   double m_w_hash[FXAI_ENH_CLASSES][FXAI_ENH_TABLES][FXAI_ENH_FPAIRS][FXAI_ENHASH_BUCKETS];

   // Continuous collision telemetry.
   double m_bucket_use_ema[FXAI_ENH_TABLES][FXAI_ENH_FPAIRS][FXAI_ENHASH_BUCKETS];
   double m_pair_collision_ema[FXAI_ENH_FPAIRS];

   // Session/regime conditioning.
   double m_bias[FXAI_ENH_CLASSES][FXAI_ENH_REGIMES][FXAI_ENH_SESSIONS];
   double m_bias_g2[FXAI_ENH_CLASSES][FXAI_ENH_REGIMES][FXAI_ENH_SESSIONS];
   ...
   {
      double m = logits[0];
      for(int c=1; c<FXAI_ENH_CLASSES; c++)
      ...
      if(den <= 0.0) den = 1.0;
      for(int c=0; c<FXAI_ENH_CLASSES; c++) probs[c] /= den;
   }
   ...
   {
      double logits[FXAI_ENH_CLASSES];
      for(int c=0; c<FXAI_ENH_CLASSES; c++)
      {
      ...
      for(int i=1; i<FXAI_AI_WEIGHTS; i++)
      {
         for(int j=i+1; j<FXAI_AI_WEIGHTS; j++)
         {
            ...
            for(int c=0; c<FXAI_ENH_CLASSES; c++)
            {
               ...
               logits[c] += c0 + c1;
               if(c == (int)FXAI_LABEL_BUY) d_buy = c0 + c1;
               if(c == (int)FXAI_LABEL_SELL) d_sell = c0 + c1;
            }
      ...
      p_skip = probs[(int)FXAI_LABEL_SKIP];
      double den = probs[(int)FXAI_LABEL_BUY] + probs[(int)FXAI_LABEL_SELL];
      if(den <= 1e-9) den = 1e-9;
      p_dir_raw = probs[(int)FXAI_LABEL_BUY] / den;
      ...
      if(pair_n > 0)
      {
         ...
         collision_metric = FXAI_Clamp(coll_sum / (double)pair_n, 0.0, 1.0);
      }
      else
      {
         inter_amp = 0.0;
   ...
   void AdaptHyperParams(const FXAIAIHyperParams &hp,
                         double &alpha,
                         double &beta,
                         double &l1,
                         double &l2) const
   {
      alpha = FXAI_Clamp(hp.enhash_lr, 0.0002, 0.1000);
      beta  = 1.0 + 4.0 * FXAI_Clamp(hp.enhash_l2, 0.0000, 0.1000);
      l1    = FXAI_Clamp(hp.enhash_l1, 0.0000, 0.1000);
      l2    = FXAI_Clamp(hp.enhash_l2, 0.0000, 0.1000);
      ...
      alpha *= FXAI_Clamp(1.0 - penalty + reward, 0.35, 1.50);
      beta  *= FXAI_Clamp(1.0 + 0.80 * m_diag_collision_ema, 0.80, 3.00);
      l1    *= FXAI_Clamp(1.0 + 1.50 * m_diag_collision_ema, 0.80, 3.00);
      l2    *= FXAI_Clamp(1.0 + m_diag_calerr_ema + 0.80 * m_diag_collision_ema, 0.80, 3.50);

      alpha = FXAI_Clamp(alpha, 0.00005, 0.1500);
      beta  = FXAI_Clamp(beta, 0.25000, 12.0000);
      l1    = FXAI_Clamp(l1, 0.00000, 0.2000);
      l2    = FXAI_Clamp(l2, 0.00000, 0.2000);
   }
   ...
   void RotateSecondaryTable(void)
   {
      ...
      for(int c=0; c<FXAI_ENH_CLASSES; c++)
      {
         for(int fp=0; fp<FXAI_ENH_FPAIRS; fp++)
         {
            for(int b=0; b<FXAI_ENHASH_BUCKETS; b++)
            {
...
public:
   CFXAIAIENHash(void) { Reset(); }

   virtual int AIId(void) const { return (int)AI_ENHASH; }
   virtual string AIName(void) const { return "lin_enhash"; }
   ...
   virtual void Reset(void)
   {
      ...
      for(int c=0; c<FXAI_ENH_CLASSES; c++)
      {
         for(int t=0; t<FXAI_ENH_TABLES; t++)
         {
            for(int fp=0; fp<FXAI_ENH_FPAIRS; fp++)
            {
               for(int b=0; b<FXAI_ENHASH_BUCKETS; b++)
      ...
      for(int c=0; c<FXAI_ENH_CLASSES; c++)
      {
         for(int r=0; r<FXAI_ENH_REGIMES; r++)
         {
            for(int s=0; s<FXAI_ENH_SESSIONS; s++)
            {
               m_bias[c][r][s] = 0.0;
               m_bias_g2[c][r][s] = 0.0;
            }
      ...
      for(int r=0; r<FXAI_ENH_REGIMES; r++)
         for(int s=0; s<FXAI_ENH_SESSIONS; s++)
            m_bias[(int)FXAI_LABEL_SKIP][r][s] = 0.20;
                               ...
                               const double move_points)
   {
      ...
      double sw = FXAI_Clamp(MoveSampleWeight(x, move_points), 0.10, 6.00);
      ...
      for(int c=0; c<FXAI_ENH_CLASSES; c++)
      {
         ...
         double gcls = FXAI_ClipSym(sw * (target - probs[c]), 4.0);
         ...
         m_bias[c][reg][sess] = FXAI_ClipSym(m_bias[c][reg][sess], 8.0);
      ...
      m_diag_uncert_ema = 0.98 * m_diag_uncert_ema + 0.02 * FXAI_Clamp(inter_std, 0.0, 2.0);

      int pred_cls = 0;
      for(int c=1; c<FXAI_ENH_CLASSES; c++) if(probs[c] > probs[pred_cls]) pred_cls = c;

      double edge = MathAbs(move_points) - cost;
      double edge_hit = 0.0;
      if(pred_cls == cls)
      {
         if(cls == (int)FXAI_LABEL_SKIP) edge_hit = (edge <= 0.0 ? 1.0 : 0.25);
      ...
      if((m_step % FXAI_ENH_REHASH_PERIOD) == 0 || m_diag_collision_ema > 0.65)
   ...
   virtual double PredictExpectedMovePoints(const double &x[], const FXAIAIHyperParams &hp)
   {
      ...
      double ev = (probs[(int)FXAI_LABEL_BUY] + probs[(int)FXAI_LABEL_SELL]) * (inter_amp + 0.35 * inter_std);
      if(m_inter_n > 1)
      {
         double var = m_inter_m2 / (double)(m_inter_n - 1);
         if(var > 0.0)
