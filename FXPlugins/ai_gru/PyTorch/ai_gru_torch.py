"""PyTorch/MPS reference backend for FXAI ai_gru.

This plugin-local backend implements a gated recurrent unit sequence model with real PyTorch layer
...
class AIGRUReferenceModel(nn.Module):
    """Reference-grade gated recurrent unit sequence model."""
...
@dataclass
class AIGRUReferenceState:
    model: AIGRUReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AIGRUReferenceState":
        dev = device or preferred_device()
        model = AIGRUReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AIGRUReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AIGRUReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AIGRUReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AIGRUReferenceState:
    state = state or AIGRUReferenceState.create(lr=lr)
    state.model.train()
