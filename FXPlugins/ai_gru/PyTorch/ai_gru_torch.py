...
@dataclass
class AIGRUTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AIGRUTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 58)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AIGRUTorchState] = None) -> torch.Tensor:
    state = state or AIGRUTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIGRUTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AIGRUTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIGRUTorchState] = None, lr: float = 0.01) -> AIGRUTorchState:
    state = state or AIGRUTorchState.create()
    x = _features(batch, state.input_weight.device)
