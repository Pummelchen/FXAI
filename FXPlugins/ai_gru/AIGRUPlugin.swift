...
public struct AIGRUPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.gru.rawValue,
        aiName: "ai_gru",
        family: .recurrent,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.gru.rawValue),
    ...
    private var cpu: AIGRUCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AIGRUAccelerated.descriptor }

    public init() {
        self.cpu = AIGRUCPUModel()
    }
