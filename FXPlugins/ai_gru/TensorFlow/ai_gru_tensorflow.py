"""TensorFlow/Keras reference backend for FXAI ai_gru."""
...
class AIGRUTensorFlowModel(tf.keras.Model):
    def __init__(self) -> None:
        super().__init__()
...
@dataclass
class AIGRUTensorFlowState:
    model: AIGRUTensorFlowModel
    optimizer: tf.keras.optimizers.Optimizer

    @classmethod
    def create(cls, lr: float = 3.0e-4) -> "AIGRUTensorFlowState":
        return cls(model=AIGRUTensorFlowModel(), optimizer=tf.keras.optimizers.AdamW(learning_rate=lr, weight_decay=1.0e-4))


def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIGRUTensorFlowState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AIGRUTensorFlowState.create()
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIGRUTensorFlowState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AIGRUTensorFlowState:
    state = state or AIGRUTensorFlowState.create(lr=lr)
