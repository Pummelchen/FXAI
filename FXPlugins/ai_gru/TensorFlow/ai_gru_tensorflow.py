...
class AIGRUTensorFlowState:
    ...
    @classmethod
    def create(cls) -> "AIGRUTensorFlowState":
        generator = tf.random.Generator.from_seed(20_000 + 58)
        return cls(
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AIGRUTensorFlowState] = None) -> tf.Tensor:
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIGRUTensorFlowState] = None) -> dict[str, object]:
    state = state or AIGRUTensorFlowState.create()
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIGRUTensorFlowState] = None, lr: float = 0.01) -> AIGRUTensorFlowState:
    state = state or AIGRUTensorFlowState.create()
