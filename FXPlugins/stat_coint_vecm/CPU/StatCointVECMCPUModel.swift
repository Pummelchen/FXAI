...
public struct StatCointVECMCPUModel: Sendable {
    private static let stateDimension = 16

    private var steps: Int
    private var residualMean: Double
    private var residualSpeed: Double
    private var residualVariance: Double
    private var previousResidual: Double
    private var previousResidualReady: Bool
    ...
    public init() {
        self.steps = 0
        self.residualMean = 0.0
        self.residualSpeed = 0.08
        self.residualVariance = 0.05
        self.previousResidual = 0.0
        self.previousResidualReady = false
        self.moveEMA = 0.0
    ...
    public mutating func train(_ request: TrainRequestV4, hyperParameters _: HyperParameters) {
        ...
        updateResidualState(z: z, sampleWeight: sampleWeight)
    ...
    public func predict(_ request: PredictRequestV4, hyperParameters _: HyperParameters) -> PredictionV4 {
        ...
        let window = Self.preparedWindow(request.xWindow, dataHasVolume: request.context.dataHasVolume)
        let z = buildState(x: x, window: window, horizonMinutes: request.context.horizonMinutes)
        let residual = cointegrationResidual(z: z)
        let zDeviation = (residual - residualMean) / sqrt(max(residualVariance, 1.0e-6))
        let volumeLift = request.context.dataHasVolume ? min(abs(z[7]) * 0.05, 0.15) : 0.0
        let modelConfidence = fxClamp(abs(zDeviation) / 2.5 * (1.0 + volumeLift), 0.0, 1.0)
        let margin = PluginSupportTools.clipSymmetric(-residualSpeed * zDeviation, limit: 8.0)
        let residualVolatility = sqrt(max(residualVariance, 1.0e-8)) * (1.0 + volumeLift)
        let forecastVolatility = max(windowVolatility(z: z), residualVolatility)
    ...
    private mutating func updateResidualState(z: [Double], sampleWeight: Double) {
        let residual = cointegrationResidual(z: z)
        if previousResidualReady {
            let delta = residual - previousResidual
            let deviation = previousResidual - residualMean
            let alpha = fxClamp(0.02 * sampleWeight, 0.001, 0.08)
            if abs(deviation) > 1.0e-6 {
                residualSpeed = fxClamp(
                    (1.0 - alpha) * residualSpeed +
                        alpha * fxClamp(-delta / deviation, 0.001, 1.0),
                    0.001,
                    1.0
                )
            }
            residualMean += alpha * (residual - residualMean)
            let error = residual - residualMean
            residualVariance = max((1.0 - alpha) * residualVariance + alpha * error * error, 1.0e-6)
        }
        previousResidual = residual
        previousResidualReady = true
    }

    private func cointegrationResidual(z: [Double]) -> Double {
        z[1] - 0.65 * z[6] - 0.35 * z[12]
    }
