...
class AIAttnCNNBiLSTMTensorFlowState:
    ...
    @classmethod
    def create(cls) -> "AIAttnCNNBiLSTMTensorFlowState":
        generator = tf.random.Generator.from_seed(20_000 + 47)
        return cls(
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AIAttnCNNBiLSTMTensorFlowState] = None) -> tf.Tensor:
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIAttnCNNBiLSTMTensorFlowState] = None) -> dict[str, object]:
    state = state or AIAttnCNNBiLSTMTensorFlowState.create()
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIAttnCNNBiLSTMTensorFlowState] = None, lr: float = 0.01) -> AIAttnCNNBiLSTMTensorFlowState:
    state = state or AIAttnCNNBiLSTMTensorFlowState.create()
