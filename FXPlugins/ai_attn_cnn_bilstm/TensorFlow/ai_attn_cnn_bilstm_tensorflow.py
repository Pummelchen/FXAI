"""TensorFlow/Keras reference backend for FXAI ai_attn_cnn_bilstm."""
...
class AIAttnCNNBiLSTMTensorFlowModel(tf.keras.Model):
    def __init__(self) -> None:
        super().__init__()
...
@dataclass
class AIAttnCNNBiLSTMTensorFlowState:
    model: AIAttnCNNBiLSTMTensorFlowModel
    optimizer: tf.keras.optimizers.Optimizer

    @classmethod
    def create(cls, lr: float = 3.0e-4) -> "AIAttnCNNBiLSTMTensorFlowState":
        return cls(model=AIAttnCNNBiLSTMTensorFlowModel(), optimizer=tf.keras.optimizers.AdamW(learning_rate=lr, weight_decay=1.0e-4))


def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIAttnCNNBiLSTMTensorFlowState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AIAttnCNNBiLSTMTensorFlowState.create()
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIAttnCNNBiLSTMTensorFlowState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AIAttnCNNBiLSTMTensorFlowState:
    state = state or AIAttnCNNBiLSTMTensorFlowState.create(lr=lr)
