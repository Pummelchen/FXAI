...
ARCHITECTURE_MODE = "attentionCNNBiLSTM"
FEATURE_COUNT = 32
HIDDEN_COUNT = 22
CLASS_COUNT = 3
...
@dataclass
class AIAttnCNNBiLSTMTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AIAttnCNNBiLSTMTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 47)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AIAttnCNNBiLSTMTorchState] = None) -> torch.Tensor:
    state = state or AIAttnCNNBiLSTMTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIAttnCNNBiLSTMTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AIAttnCNNBiLSTMTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIAttnCNNBiLSTMTorchState] = None, lr: float = 0.01) -> AIAttnCNNBiLSTMTorchState:
    state = state or AIAttnCNNBiLSTMTorchState.create()
    x = _features(batch, state.input_weight.device)
