"""PyTorch/MPS reference backend for FXAI ai_attn_cnn_bilstm.

This plugin-local backend implements a Conv1D BiLSTM with multi-head attention pooling with real PyTorch layer
...
PLUGIN_NAME = "ai_attn_cnn_bilstm"
ARCHITECTURE_MODE = "ATTN_CNN_BILSTM"
FEATURE_COUNT = 32
CLASS_COUNT = 3
HIDDEN_COUNT = 48
HEAD_COUNT = 4
...
class AIAttnCNNBiLSTMReferenceModel(nn.Module):
    """Reference-grade Conv1D BiLSTM with multi-head attention pooling."""
...
@dataclass
class AIAttnCNNBiLSTMReferenceState:
    model: AIAttnCNNBiLSTMReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AIAttnCNNBiLSTMReferenceState":
        dev = device or preferred_device()
        model = AIAttnCNNBiLSTMReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AIAttnCNNBiLSTMReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AIAttnCNNBiLSTMReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AIAttnCNNBiLSTMReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AIAttnCNNBiLSTMReferenceState:
    state = state or AIAttnCNNBiLSTMReferenceState.create(lr=lr)
    state.model.train()
