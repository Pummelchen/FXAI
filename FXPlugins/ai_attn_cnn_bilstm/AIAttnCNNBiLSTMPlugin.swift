...
public struct AIAttnCNNBiLSTMPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.attnCNNBiLSTM.rawValue,
        aiName: "ai_attn_cnn_bilstm",
        family: .convolutional,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.attnCNNBiLSTM.rawValue),
    ...
    private var cpu: AIAttnCNNBiLSTMCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AIAttnCNNBiLSTMAccelerated.descriptor }

    public init() {
        self.cpu = AIAttnCNNBiLSTMCPUModel()
    }
