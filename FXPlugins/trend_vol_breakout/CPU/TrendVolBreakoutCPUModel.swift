...
public struct TrendVolBreakoutCPUModel: Sendable {
    ...
    public func predict(_ request: PredictRequestV4, hyperParameters _: HyperParameters) -> PredictionV4 {
        ...
        let margin = volBreakoutMargin(z: z, window: window)
        let modelConfidence = volBreakoutConfidence(z: z, window: window, forecastVolatility: forecastVolatility, dataHasVolume: request.context.dataHasVolume)
    ...
    private func volBreakoutMargin(z: [Double], window: [[Double]]) -> Double {
        let expansion = volExpansion(window: window)
        let breakoutDirection = expansion >= 0.0 ? 1.0 : -1.0
        return PluginSupportTools.clipSymmetric(
            breakoutDirection * z[8] + 0.25 * linearMargin(z: z),
            limit: 8.0
        )
    }

    private func volBreakoutConfidence(z: [Double], window: [[Double]], forecastVolatility: Double, dataHasVolume: Bool) -> Double {
        let expansionStrength = fxClamp(abs(volExpansion(window: window)) / max(forecastVolatility, 0.05), 0.0, 1.0)
        let volatilityQuality = fxClamp(1.0 / (1.0 + max(forecastVolatility, 0.0)), 0.0, 1.0)
        let volumeBoost = dataHasVolume ? 0.06 * abs(z[7]) : 0.0
        return fxClamp(0.35 + 0.38 * expansionStrength + 0.15 * abs(z[8]) + 0.12 * volatilityQuality + volumeBoost, 0.0, 1.0)
    }

    private func volExpansion(window: [[Double]]) -> Double {
        guard !window.isEmpty else { return 0.0 }
        let recentRange = Self.windowRange(window, feature: 1, start: 0, count: 8)
        let baselineRange = Self.windowRange(window, feature: 1, start: 8, count: 24)
        return recentRange - baselineRange
    }
    ...
    private static func seedWeight(_ a: Int, _ b: Int) -> Double {
        let raw = sin(Double((AIModelID.trendVolBreakout.rawValue + 17) * (a + 3) * 37 + (b + 11) * 101))
        return 0.035 * raw
    }
    ...
    private static func windowRange(_ window: [[Double]], feature: Int, count: Int) -> Double {
        windowRange(window, feature: feature, start: 0, count: count)
    }

    private static func windowRange(_ window: [[Double]], feature: Int, start: Int, count: Int) -> Double {
