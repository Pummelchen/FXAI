"""PyTorch/MPS reference backend for FXAI wm_cfx.

This plugin-local backend implements a currency factor world model with real PyTorch layer
...
class WMCFXReferenceModel(nn.Module):
    """Reference-grade currency factor world model."""
...
@dataclass
class WMCFXReferenceState:
    model: WMCFXReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "WMCFXReferenceState":
        dev = device or preferred_device()
        model = WMCFXReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[WMCFXReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or WMCFXReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[WMCFXReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> WMCFXReferenceState:
    state = state or WMCFXReferenceState.create(lr=lr)
    state.model.train()
