...
PLUGIN_NAME = "wm_cfx"
ARCHITECTURE_MODE = "currencyFactorWorld"
FEATURE_COUNT = 32
HIDDEN_COUNT = 16
CLASS_COUNT = 3
...
@dataclass
class WMCFXTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "WMCFXTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 22)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[WMCFXTorchState] = None) -> torch.Tensor:
    state = state or WMCFXTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[WMCFXTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or WMCFXTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[WMCFXTorchState] = None, lr: float = 0.01) -> WMCFXTorchState:
    state = state or WMCFXTorchState.create()
    x = _features(batch, state.input_weight.device)
