...
public struct WMCFXCPUModel: Sendable {
    private static let featureCount = 32
    private static let hiddenCount = 16
    private static let architectureID = 22
    private static let architectureMode = "currencyFactorWorld"
    ...
    private static func seed(_ a: Int, _ b: Int) -> Double {
        sin(Double((AIModelID.cfxWorld.rawValue + 31) * (a + 3) * 67 + (b + 11) * 131 + Self.architectureID * 17))
    }
}
