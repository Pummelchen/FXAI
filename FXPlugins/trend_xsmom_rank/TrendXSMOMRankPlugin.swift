...
public struct TrendXSMOMRankPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.trendXSMOMRank.rawValue,
        aiName: "trend_xsmom_rank",
        family: .other,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.trendXSMOMRank.rawValue),
    ...
    private var cpu: TrendXSMOMRankCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { TrendXSMOMRankAccelerated.descriptor }

    public init() {
        self.cpu = TrendXSMOMRankCPUModel()
    }
