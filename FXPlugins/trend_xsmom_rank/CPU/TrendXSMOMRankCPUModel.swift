...
public struct TrendXSMOMRankCPUModel: Sendable {
    ...
    public func predict(_ request: PredictRequestV4, hyperParameters _: HyperParameters) -> PredictionV4 {
        ...
        let margin = xsmomRankMargin(z: z)
        let modelConfidence = xsmomConfidence(z: z, dataHasVolume: request.context.dataHasVolume)
    ...
    private func xsmomRankMargin(z: [Double]) -> Double {
        PluginSupportTools.clipSymmetric(
            0.55 * z[6] + 0.35 * z[8] + 0.10 * linearMargin(z: z),
            limit: 8.0
        )
    }

    private func xsmomConfidence(z: [Double], dataHasVolume: Bool) -> Double {
        let trendStructure = min(abs(z[6]) + abs(z[8]), 2.0) / 2.0
        let volumeBoost = dataHasVolume ? 0.06 * abs(z[7]) : 0.0
        return fxClamp(0.45 + 0.35 * trendStructure + volumeBoost, 0.0, 1.0)
    }
    ...
    private static func seedWeight(_ a: Int, _ b: Int) -> Double {
        let raw = sin(Double((AIModelID.trendXSMOMRank.rawValue + 17) * (a + 3) * 37 + (b + 11) * 101))
        return 0.035 * raw
    }
