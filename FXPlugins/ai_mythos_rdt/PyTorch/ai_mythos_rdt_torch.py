"""PyTorch/MPS reference backend for FXAI ai_mythos_rdt.

This plugin-local backend implements a recursive decision transformer with real PyTorch layer
...
PLUGIN_NAME = "ai_mythos_rdt"
ARCHITECTURE_MODE = "MYTHOS_RDT"
FEATURE_COUNT = 32
CLASS_COUNT = 3
HIDDEN_COUNT = 48
HEAD_COUNT = 4
...
@dataclass
class AIMythosRDTReferenceState:
    model: AIMythosRDTReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AIMythosRDTReferenceState":
        dev = device or preferred_device()
        model = AIMythosRDTReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AIMythosRDTReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AIMythosRDTReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AIMythosRDTReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AIMythosRDTReferenceState:
    state = state or AIMythosRDTReferenceState.create(lr=lr)
    state.model.train()
