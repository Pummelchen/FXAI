...
@dataclass
class AIMythosRDTTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AIMythosRDTTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 61)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AIMythosRDTTorchState] = None) -> torch.Tensor:
    state = state or AIMythosRDTTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIMythosRDTTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AIMythosRDTTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIMythosRDTTorchState] = None, lr: float = 0.01) -> AIMythosRDTTorchState:
    state = state or AIMythosRDTTorchState.create()
    x = _features(batch, state.input_weight.device)
