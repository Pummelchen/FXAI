...
public struct StatMSGARCHCPUModel: Sendable {
    ...
    private var garchH: [Double]
    private var garchOmega: [Double]
    private var garchAlpha: [Double]
    private var garchBeta: [Double]
    ...
    public init() {
        ...
        self.garchH = Array(repeating: 0.0, count: Self.regimes)
        self.garchOmega = Array(repeating: 0.0, count: Self.regimes)
        self.garchAlpha = Array(repeating: 0.0, count: Self.regimes)
        self.garchBeta = Array(repeating: 0.0, count: Self.regimes)
        self.moveEMA = 0.0
        ...
        self.policyOld = Array(repeating: 1.0 / Double(LabelClass.allCases.count), count: LabelClass.allCases.count)
        ...
        for regime in 0..<Self.regimes {
            ...
            garchH[regime] = 0.02 + 0.02 * Double(regime)
            garchOmega[regime] = 0.0005 + 0.0002 * Double(regime)
            garchAlpha[regime] = 0.07 + 0.02 * Double(regime)
            garchBeta[regime] = 0.86 - 0.03 * Double(regime)
    ...
    public func predict(_ request: PredictRequestV4, hyperParameters _: HyperParameters) -> PredictionV4 {
        ...
        let window = Self.preparedWindow(request.xWindow, dataHasVolume: request.context.dataHasVolume)
        let z = buildState(x: x, window: window, horizonMinutes: request.context.horizonMinutes)
        var forecastVolatility = 0.0
        var modelConfidence = 0.0
        let margin = msgarchMargin(z: z, forecastVolatility: &forecastVolatility, confidence: &modelConfidence)
    ...
    private func msgarchMargin(
        z: [Double],
        forecastVolatility: inout Double,
        confidence: inout Double
    ) -> Double {
        let pi = hmmForward(observation: z[8] + 0.15 * z[1], volatility: windowVolatility(z: z))
        let squaredReturn = z[8] * z[8]
        var drift = 0.0
        forecastVolatility = 0.0
        confidence = 0.0
        for regime in 0..<Self.regimes {
            let h = max(garchOmega[regime] + garchAlpha[regime] * squaredReturn + garchBeta[regime] * max(garchH[regime], 1.0e-6), 1.0e-8)
            forecastVolatility += pi[regime] * sqrt(h)
            drift += pi[regime] * regimeMu[regime] / sqrt(max(h, 1.0e-8))
            confidence = max(confidence, pi[regime])
        }
        let supervisedDrift = linearMargin(z: z)
        return PluginSupportTools.clipSymmetric(0.85 * drift + 0.15 * supervisedDrift, limit: 8.0)
    }
    ...
    private mutating func updateRegimeModels(z: [Double], sampleWeight: Double) {
        ...
        for regime in 0..<Self.regimes {
            ...
            garchH[regime] = max(garchOmega[regime] + garchAlpha[regime] * delta * delta + garchBeta[regime] * garchH[regime], 1.0e-8)
        ...
        for regime in 0..<Self.regimes {
            ...
            let rowSum = max(hmmTransition[regime].reduce(0.0, +), 1.0)
            for target in 0..<Self.regimes {
                hmmTransition[regime][target] /= rowSum
            }
        }
    }
    ...
    private static func seedWeight(_ a: Int, _ b: Int) -> Double {
        let raw = sin(Double((AIModelID.statMSGARCH.rawValue + 17) * (a + 3) * 37 + (b + 11) * 101))
        return 0.035 * raw
    }
