...
| `lin_enhash` | B+ | Dual field/hash online learner with FTRL-like behavior and Metal pair-product support. | Formalize ENHash reference spec, verify collision handling, and add golden replay fixtures from canonical sparse-hash cases. |
...
| `mix_moe_conformal` | A- | Functional MoE router with conformal score rings, calibration buckets, shared split-conformal finite-sample cutoff diagnostics, prediction-set fixtures, expert specialization tests, and move-interval validity coverage. | Add live held-out market coverage dashboards and accelerator parity evidence before treating empirical coverage as production-certified. |
| `rl_ppo` | B+ | Actor-critic PPO with rollout buffer, GAE, clipped policy loss, entropy/value losses, PyTorch/MPS runtime, and NF-010 net-of-cost truth decision for buy/sell/skip actions. | Add longer offline market-environment replay fixtures and checkpoint parity across CPU fallback and PyTorch MPS. |
