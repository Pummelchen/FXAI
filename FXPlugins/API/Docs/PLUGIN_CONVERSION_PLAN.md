...
- `API/Backends/Python/fxai_plugin_backend.py` provides the generic PyTorch/TensorFlow
...
- Shared implementation primitives now live in `FXDataEngine`, not in
  `FXPlugins`; each non-handwritten plugin owns its manifest, acceleration plan,
  and Swift adapter file under its own plugin folder.

Remaining parity work after this wave is plugin-specific: replace reference
...
- Added `PluginAccelerationPlan` metadata for backend suitability in
  `FXDataEngine`.
...
- `PythonMLBackendBridge` and `API/Backends/Python/fxai_plugin_backend.py` provide the
  process-level PyTorch/TensorFlow execution path for backend-owned plugins.

Remaining work:

- Replace reference surrogate prediction with full per-plugin native logic or
