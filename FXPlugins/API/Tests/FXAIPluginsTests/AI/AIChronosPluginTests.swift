...
final class AIChronosPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = AIChronosPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.chronos.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "ai_chronos")
        ...
        XCTAssertEqual(plan.pluginName, "ai_chronos")
    ...
    func testConvertedPluginOwnsExpectedAccelerators() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AIChronosCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AIChronosAccelerated.swift").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("ai_chronos")
    }
}
