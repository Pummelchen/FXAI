...
final class AILSTMTCNPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = AILSTMTCNPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.lstmTCN.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "ai_lstm_tcn")
        ...
        XCTAssertEqual(plan.pluginName, "ai_lstm_tcn")
    ...
    func testConvertedPluginOwnsExpectedAccelerators() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AILSTMTCNCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AILSTMTCNAccelerated.swift").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("ai_lstm_tcn")
    }
}
