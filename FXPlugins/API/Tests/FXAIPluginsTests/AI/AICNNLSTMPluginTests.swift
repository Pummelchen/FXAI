...
final class AICNNLSTMPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = AICNNLSTMPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.cnnLSTM.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "ai_cnn_lstm")
        ...
        XCTAssertEqual(plan.pluginName, "ai_cnn_lstm")
    ...
    func testConvertedPluginOwnsExpectedAccelerators() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AICNNLSTMCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AICNNLSTMAccelerated.swift").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("ai_cnn_lstm")
    }
}
