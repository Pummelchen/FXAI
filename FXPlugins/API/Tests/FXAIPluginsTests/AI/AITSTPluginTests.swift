...
final class AITSTPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = AITSTPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.tst.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "ai_tst")
        ...
        XCTAssertEqual(plan.pluginName, "ai_tst")
    ...
    func testConvertedPluginOwnsExpectedAccelerators() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AITSTCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AITSTAccelerated.swift").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("ai_tst")
    }
}
