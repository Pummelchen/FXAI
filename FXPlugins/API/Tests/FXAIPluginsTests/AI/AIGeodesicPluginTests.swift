...
final class AIGeodesicPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = AIGeodesicPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.geodesicAttention.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "ai_geodesic")
        ...
        XCTAssertEqual(plan.pluginName, "ai_geodesic")
    ...
    func testConvertedPluginOwnsExpectedAccelerators() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AIGeodesicCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AIGeodesicAccelerated.swift").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("ai_geodesic")
    }
}
