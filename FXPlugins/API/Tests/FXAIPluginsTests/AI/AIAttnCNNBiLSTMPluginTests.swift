...
final class AIAttnCNNBiLSTMPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = AIAttnCNNBiLSTMPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.attnCNNBiLSTM.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "ai_attn_cnn_bilstm")
        ...
        XCTAssertEqual(plan.pluginName, "ai_attn_cnn_bilstm")
    ...
    func testTrainingUpdatesReliabilityAndMoveState() throws {
        ...
        var plugin = AIAttnCNNBiLSTMPlugin()
    ...
    func testConvertedPluginOwnsExpectedAccelerators() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AIAttnCNNBiLSTMCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AIAttnCNNBiLSTMAccelerated.swift").path))
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("TensorFlow").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("ai_attn_cnn_bilstm")
    }
}
