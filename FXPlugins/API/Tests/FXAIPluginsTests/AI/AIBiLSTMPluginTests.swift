...
final class AIBiLSTMPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = AIBiLSTMPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.bilstm.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "ai_bilstm")
        XCTAssertEqual(plugin.manifest.family, .recurrent)
        ...
        XCTAssertEqual(plan.pluginName, "ai_bilstm")
    ...
    func testConvertedPluginOwnsExpectedAccelerators() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AIBiLSTMCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AIBiLSTMAccelerated.swift").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("ai_bilstm")
    }
}
