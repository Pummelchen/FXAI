...
final class AILSTMGPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = AILSTMGPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.lstmg.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "ai_lstmg")
        ...
        XCTAssertEqual(plan.pluginName, "ai_lstmg")
    ...
    func testConvertedPluginOwnsExpectedAccelerators() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AILSTMGCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AILSTMGAccelerated.swift").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("ai_lstmg")
    }
}
