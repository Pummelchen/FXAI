...
final class AIMythosRDTPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = AIMythosRDTPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.mythosRDT.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "ai_mythos_rdt")
        XCTAssertEqual(plugin.manifest.family, .transformer)
        ...
        XCTAssertEqual(plan.pluginName, "ai_mythos_rdt")
    ...
    func testConvertedPluginOwnsExpectedAccelerators() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AIMythosRDTCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/AIMythosRDTAccelerated.swift").path))
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("NLP").path))
    }
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("ai_mythos_rdt")
    }
}
