...
final class FactorPPPValuePluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = FactorPPPValuePlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.factorPPPValue.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "factor_ppp_value")
        ...
        XCTAssertEqual(plan.pluginName, "factor_ppp_value")
    ...
    func testPPPValueFactorSeparatesUndervaluedAndOvervaluedDirections() throws {
        let plugin = FactorPPPValuePlugin()
        let undervalued = try plugin.predict(Self.undervaluedRequest(dataHasVolume: true), hyperParameters: Self.hyperParameters())
        let overvalued = try plugin.predict(Self.overvaluedRequest(dataHasVolume: true), hyperParameters: Self.hyperParameters())

        XCTAssertGreaterThan(undervalued.classProbabilities[LabelClass.buy.rawValue], overvalued.classProbabilities[LabelClass.buy.rawValue])
        XCTAssertGreaterThan(overvalued.classProbabilities[LabelClass.sell.rawValue], undervalued.classProbabilities[LabelClass.sell.rawValue])
        XCTAssertNoThrow(try undervalued.validate())
        XCTAssertNoThrow(try overvalued.validate())
    }

    func testTrainingUpdatesMoveAndReliabilityState() throws {
        let request = Self.undervaluedRequest(dataHasVolume: true)
        var plugin = FactorPPPValuePlugin()
        let before = try plugin.predict(request, hyperParameters: Self.hyperParameters())

        for iteration in 0..<96 {
            try plugin.train(Self.trainRequest(from: request, label: .buy, movePoints: 2.0 + 0.01 * Double(iteration % 4)), hyperParameters: Self.hyperParameters())
        }
    ...
    func testVolumeAffectsPPPValueOnlyWhenDatasetHasVolume() throws {
        let plugin = FactorPPPValuePlugin()
        let withVolume = try plugin.predict(Self.undervaluedRequest(dataHasVolume: true), hyperParameters: Self.hyperParameters())
        let withoutVolume = try plugin.predict(Self.undervaluedRequest(dataHasVolume: false), hyperParameters: Self.hyperParameters())
    ...
    func testResetClearsPPPValueState() throws {
        let request = Self.undervaluedRequest(dataHasVolume: true)
        var plugin = FactorPPPValuePlugin()
        let before = try plugin.predict(request, hyperParameters: Self.hyperParameters())
    ...
    func testConvertedPluginOwnsCPUCodeOnly() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/FactorPPPValueCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/FactorPPPValueAccelerated.swift").path))
    ...
    private static func undervaluedRequest(dataHasVolume: Bool) -> PredictRequestV4 {
        PredictRequestV4(
            valid: true,
            context: context(dataHasVolume: dataHasVolume),
            x: features(valueGap: -0.72, macroPressure: -0.45, volume: 1.20)
        )
    }

    private static func overvaluedRequest(dataHasVolume: Bool) -> PredictRequestV4 {
        PredictRequestV4(
            valid: true,
            context: context(dataHasVolume: dataHasVolume),
            x: features(valueGap: 0.72, macroPressure: 0.45, volume: 1.10)
        )
    }
    ...
    private static func context(dataHasVolume: Bool) -> PluginContextV4 {
        PluginContextV4(
            ...
            minMovePoints: 0.50,
            pointValue: 1.0,
            sampleTimeUTC: 1_800_110_000,
            dataHasVolume: dataHasVolume
        )
    }

    private static func features(valueGap: Double, macroPressure: Double, volume: Double) -> [Double] {
        var values = Array(repeating: 0.0, count: FXDataEngineConstants.aiWeights)
        values[1] = -0.04 * valueGap
        values[4] = valueGap
        values[6] = volume
        values[7] = 0.03 * valueGap
        values[12] = -0.10 * valueGap
        values[40] = 0.0
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("factor_ppp_value")
    }
}
