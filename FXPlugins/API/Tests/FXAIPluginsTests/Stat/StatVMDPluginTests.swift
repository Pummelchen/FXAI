...
final class StatVMDPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = StatVMDPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.statVMD.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "stat_vmd")
        ...
        XCTAssertEqual(plan.pluginName, "stat_vmd")
    ...
    func testVMDSeparatesPositiveAndNegativeModeEMAs() throws {
        let plugin = StatVMDPlugin()
    ...
    func testModeEMAChangesPredictionVersusFlatWindow() throws {
        let plugin = StatVMDPlugin()
    ...
    func testVolumeAffectsVMDOnlyWhenDatasetHasVolume() throws {
        let plugin = StatVMDPlugin()
    ...
    func testResetClearsVMDState() throws {
        let request = Self.momentumRequest(direction: 1.0, dataHasVolume: true)
        var plugin = StatVMDPlugin()
        let before = try plugin.predict(request, hyperParameters: Self.hyperParameters())
    ...
    func testConvertedPluginOwnsCPUAndMetalCode() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/StatVMDCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/StatVMDAccelerated.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("Metal/StatVMDMetal.swift").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("stat_vmd")
    }
}
