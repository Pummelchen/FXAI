...
final class StatOUSpreadPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedOUSpreadContract() throws {
        let plugin = StatOUSpreadPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.statOUSpread.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "stat_ou_spread")
        ...
        XCTAssertEqual(plan.pluginName, "stat_ou_spread")
    ...
    func testFreshOUSpreadModelProducesValidMeanReversionDistribution() throws {
        let plugin = StatOUSpreadPlugin()
    ...
    func testResetClearsOUSpreadState() throws {
    ...
    func testConvertedPluginOwnsCPUCodeOnly() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/StatOUSpreadCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/StatOUSpreadAccelerated.swift").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("stat_ou_spread")
    }
}
