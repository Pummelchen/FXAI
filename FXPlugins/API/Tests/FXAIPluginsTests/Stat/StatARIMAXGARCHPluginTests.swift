...
final class StatARIMAXGARCHPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedARIMAXGARCHContract() throws {
        let plugin = StatARIMAXGARCHPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.statARIMAXGARCH.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "stat_arimax_garch")
        ...
        XCTAssertEqual(plan.pluginName, "stat_arimax_garch")
    ...
    func testFreshARIMAXGARCHModelProducesValidDistribution() throws {
        let plugin = StatARIMAXGARCHPlugin()
    ...
    func testTrainingUpdatesPolicyGARCHAndSeparatesDirections() throws {
        var plugin = StatARIMAXGARCHPlugin()
        ...
        for iteration in 0..<96 {
            try plugin.train(Self.trainRequest(from: buyRequest, label: .buy, movePoints: 3.0 + 0.01 * Double(iteration % 5)), hyperParameters: Self.hyperParameters())
            try plugin.train(Self.trainRequest(from: sellRequest, label: .sell, movePoints: -2.8 - 0.01 * Double(iteration % 5)), hyperParameters: Self.hyperParameters())
        }
    ...
    func testVolumeAffectsVolatilityOnlyWhenDatasetHasVolume() throws {
        var plugin = StatARIMAXGARCHPlugin()
    ...
    func testResetClearsARIMAXGARCHState() throws {
        ...
        for _ in 0..<80 {
            try plugin.train(Self.trainRequest(from: request, label: .buy, movePoints: 2.9), hyperParameters: Self.hyperParameters())
        }
    ...
    func testConvertedPluginOwnsCPUCodeOnly() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/StatARIMAXGARCHCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/StatARIMAXGARCHAccelerated.swift").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("stat_arimax_garch")
    }
}
