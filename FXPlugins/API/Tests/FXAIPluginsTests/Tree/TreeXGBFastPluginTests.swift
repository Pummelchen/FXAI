...
final class TreeXGBFastPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedXGBFastContract() throws {
        let plugin = TreeXGBFastPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.xgbFast.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "tree_xgb_fast")
        ...
        XCTAssertEqual(plan.pluginName, "tree_xgb_fast")
    ...
    func testFreshTreeModelProducesValidDistribution() throws {
        let plugin = TreeXGBFastPlugin()
    ...
    func testTrainingBuildsTreesSeparatesDirectionsAndLearnsMoveDistribution() throws {
        var plugin = TreeXGBFastPlugin()
        let buyRequest = Self.predictRequest(dataHasVolume: true)
        let sellRequest = Self.sellRequest(from: buyRequest)

        for iteration in 0..<120 {
            try plugin.train(Self.trainRequest(from: buyRequest, label: .buy, movePoints: 4.2 + 0.01 * Double(iteration % 5)), hyperParameters: Self.hyperParameters())
            try plugin.train(Self.trainRequest(from: sellRequest, label: .sell, movePoints: -3.9 - 0.01 * Double(iteration % 5)), hyperParameters: Self.hyperParameters())
        }
    ...
    func testResetClearsTreeState() throws {
        ...
        for _ in 0..<80 {
            try plugin.train(Self.trainRequest(from: request, label: .buy, movePoints: 3.6), hyperParameters: Self.hyperParameters())
        }
    ...
    func testConvertedPluginOwnsCPUAndMetalCodeFolders() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/TreeXGBFastCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("Metal/TreeXGBFastMetal.swift").path))
    ...
    private static func trainRequest(from request: PredictRequestV4, label: LabelClass, movePoints: Double) -> TrainRequestV4 {
        TrainRequestV4(
            ...
            maePoints: abs(movePoints) * 0.34,
            timeToHitFraction: 0.38,
            pathRisk: 0.18,
            fillRisk: 0.10,
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("tree_xgb_fast")
    }
}
