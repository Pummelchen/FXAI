...
final class TreeCatboostPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedCatboostContract() throws {
        let plugin = TreeCatboostPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.catboost.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "tree_catboost")
        ...
        XCTAssertEqual(plan.pluginName, "tree_catboost")
    ...
    func testFreshCatboostModelKeepsLegacySkipPriorAndValidQuantiles() throws {
        let plugin = TreeCatboostPlugin()
    ...
    func testTrainingBuildsOrderedTreesSeparatesDirectionsAndLearnsMoveDistribution() throws {
        var plugin = TreeCatboostPlugin()
        ...
        for iteration in 0..<180 {
            try plugin.train(Self.trainRequest(from: buyRequest, label: .buy, movePoints: 4.4 + 0.02 * Double(iteration % 9)), hyperParameters: Self.hyperParameters())
            try plugin.train(Self.trainRequest(from: sellRequest, label: .sell, movePoints: -4.1 - 0.02 * Double(iteration % 9)), hyperParameters: Self.hyperParameters())
        }
    ...
    func testVolumeAffectsOrderedCTRAndSplitsOnlyWhenDatasetHasVolume() throws {
        var plugin = TreeCatboostPlugin()
    ...
    func testResetClearsCatboostState() throws {
        ...
        for _ in 0..<288 {
            try plugin.train(Self.trainRequest(from: request, label: .buy, movePoints: 4.2), hyperParameters: Self.hyperParameters())
        }
    ...
    func testConvertedPluginOwnsCPUAndMetalCodeFolders() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/TreeCatboostCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("Metal/TreeCatboostMetal.swift").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("tree_catboost")
    }
}
