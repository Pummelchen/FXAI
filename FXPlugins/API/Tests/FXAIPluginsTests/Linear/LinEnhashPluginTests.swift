...
final class LinEnhashPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedENHashContract() throws {
        let plugin = LinEnhashPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.enhash.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "lin_enhash")
        ...
        XCTAssertEqual(plan.pluginName, "lin_enhash")
    ...
    func testFreshModelUsesLegacySkipPrior() throws {
        let plugin = LinEnhashPlugin()
    ...
    func testTrainingSeparatesBuyAndSellAndLearnsMoveEstimate() throws {
        var plugin = LinEnhashPlugin()
    ...
    func testConvertedPluginOwnsCPUAndMetalCodeFolders() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/LinEnhashCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("Metal/LinEnhashMetal.swift").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("lin_enhash")
    }
}
