...
final class LinElasticLogitPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedElasticLogitContract() throws {
        let plugin = LinElasticLogitPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.linElasticLogit.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "lin_elastic_logit")
        ...
        XCTAssertEqual(plan.pluginName, "lin_elastic_logit")
    ...
    func testTrainingSeparatesBuyAndSellAndLearnsMoveEMA() throws {
        var plugin = LinElasticLogitPlugin()
    ...
    func testConvertedPluginOwnsCPUAndMetalCodeFolders() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/LinElasticLogitCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("Metal/LinElasticLogitMetal.swift").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("lin_elastic_logit")
    }
}
