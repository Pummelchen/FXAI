...
final class LinFTRLPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedFTRLContract() throws {
        let plugin = LinFTRLPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.ftrlLogit.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "lin_ftrl")
        ...
        XCTAssertEqual(plan.pluginName, "lin_ftrl")
    ...
    func testConvertedPluginOwnsCPUAndMetalCodeFolders() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/LinFTRLCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("Metal/LinFTRLMetal.swift").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("lin_ftrl")
    }
}
