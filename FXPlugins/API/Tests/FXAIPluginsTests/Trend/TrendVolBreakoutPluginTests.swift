...
final class TrendVolBreakoutPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = TrendVolBreakoutPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.trendVolBreakout.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "trend_vol_breakout")
    ...
    func testVolBreakoutSeparatesPositiveAndNegativeExpandingWindows() throws {
        let plugin = TrendVolBreakoutPlugin()
        let positive = try plugin.predict(Self.breakoutRequest(direction: 1.0, dataHasVolume: true), hyperParameters: Self.hyperParameters())
        let negative = try plugin.predict(Self.breakoutRequest(direction: -1.0, dataHasVolume: true), hyperParameters: Self.hyperParameters())
    ...
    func testRangeExpansionChangesPredictionVersusFlatWindow() throws {
        let plugin = TrendVolBreakoutPlugin()
        let trending = try plugin.predict(Self.breakoutRequest(direction: 1.0, dataHasVolume: true), hyperParameters: Self.hyperParameters())
    ...
    func testTrainingUpdatesLinearBaseAndReliabilityState() throws {
        let request = Self.breakoutRequest(direction: 1.0, dataHasVolume: true)
        var plugin = TrendVolBreakoutPlugin()
    ...
    func testVolumeAffectsVolBreakoutOnlyWhenDatasetHasVolume() throws {
        let plugin = TrendVolBreakoutPlugin()
        let withVolume = try plugin.predict(Self.breakoutRequest(direction: 1.0, dataHasVolume: true), hyperParameters: Self.hyperParameters())
        let withoutVolume = try plugin.predict(Self.breakoutRequest(direction: 1.0, dataHasVolume: false), hyperParameters: Self.hyperParameters())
    ...
    func testResetClearsVolBreakoutState() throws {
        let request = Self.breakoutRequest(direction: 1.0, dataHasVolume: true)
        var plugin = TrendVolBreakoutPlugin()
        let before = try plugin.predict(request, hyperParameters: Self.hyperParameters())
    ...
    private static func breakoutRequest(direction: Double, dataHasVolume: Bool) -> PredictRequestV4 {
        let window = breakoutWindow(direction: direction, volume: 1.20)
    ...
    private static func flatRequest(dataHasVolume: Bool) -> PredictRequestV4 {
        let window = breakoutWindow(direction: 0.0, volume: 1.20)
        return PredictRequestV4(
            valid: true,
            context: context(dataHasVolume: dataHasVolume),
            windowSize: window.count,
    ...
    private static func breakoutWindow(direction: Double, volume: Double) -> [[Double]] {
        (0..<32).map { index in
            ...
            if direction == 0.0 {
                ...
                let recentPulse = (index % 2 == 0 ? 0.18 : -0.18)
                row[1] = direction * (0.95 - 0.055 * age + recentPulse)
                row[2] = direction * (0.45 - 0.012 * age)
            } else {
                row[1] = direction * (0.24 - 0.002 * age)
                row[2] = direction * (0.18 - 0.001 * age)
            }
            row[6] = volume
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("trend_vol_breakout")
    }
}
