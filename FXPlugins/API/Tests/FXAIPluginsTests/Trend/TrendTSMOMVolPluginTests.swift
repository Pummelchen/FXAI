...
final class TrendTSMOMVolPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = TrendTSMOMVolPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.trendTSMOMVol.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "trend_tsmom_vol")
    ...
    func testTSMOMVolSeparatesPositiveAndNegativeWindowMomentum() throws {
        let plugin = TrendTSMOMVolPlugin()
    ...
    func testWindowMomentumChangesPredictionVersusFlatWindow() throws {
        let plugin = TrendTSMOMVolPlugin()
    ...
    func testTrainingUpdatesLinearBaseAndReliabilityState() throws {
        ...
        var plugin = TrendTSMOMVolPlugin()
    ...
    func testVolumeAffectsTSMOMVolOnlyWhenDatasetHasVolume() throws {
        let plugin = TrendTSMOMVolPlugin()
    ...
    func testResetClearsTSMOMVolState() throws {
        let request = Self.momentumRequest(direction: 1.0, dataHasVolume: true)
        var plugin = TrendTSMOMVolPlugin()
        let before = try plugin.predict(request, hyperParameters: Self.hyperParameters())
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("trend_tsmom_vol")
    }
}
