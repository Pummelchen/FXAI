...
final class TrendXSMOMRankPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = TrendXSMOMRankPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.trendXSMOMRank.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "trend_xsmom_rank")
        ...
        XCTAssertEqual(plan.pluginName, "trend_xsmom_rank")
    ...
    func testXSMOMRankSeparatesPositiveAndNegativeRankStates() throws {
        let plugin = TrendXSMOMRankPlugin()
        let positive = try plugin.predict(Self.positiveRankRequest(dataHasVolume: true), hyperParameters: Self.hyperParameters())
        let negative = try plugin.predict(Self.negativeRankRequest(dataHasVolume: true), hyperParameters: Self.hyperParameters())
    ...
    func testTrainingUpdatesLinearBaseAndReliabilityState() throws {
        let request = Self.positiveRankRequest(dataHasVolume: true)
        var plugin = TrendXSMOMRankPlugin()
    ...
    func testVolumeAffectsXSMOMRankOnlyWhenDatasetHasVolume() throws {
        let plugin = TrendXSMOMRankPlugin()
        let withVolume = try plugin.predict(Self.positiveRankRequest(dataHasVolume: true), hyperParameters: Self.hyperParameters())
        let withoutVolume = try plugin.predict(Self.positiveRankRequest(dataHasVolume: false), hyperParameters: Self.hyperParameters())
    ...
    func testResetClearsXSMOMRankState() throws {
        let request = Self.positiveRankRequest(dataHasVolume: true)
        var plugin = TrendXSMOMRankPlugin()
        let before = try plugin.predict(request, hyperParameters: Self.hyperParameters())
    ...
    func testConvertedPluginOwnsCPUCodeOnly() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/TrendXSMOMRankCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/TrendXSMOMRankAccelerated.swift").path))
    ...
    private static func positiveRankRequest(dataHasVolume: Bool) -> PredictRequestV4 {
        PredictRequestV4(
            valid: true,
            context: context(dataHasVolume: dataHasVolume),
            x: features(rankSignal: 0.75, direction: 1.0, volume: 1.20)
        )
    }

    private static func negativeRankRequest(dataHasVolume: Bool) -> PredictRequestV4 {
        PredictRequestV4(
            valid: true,
            context: context(dataHasVolume: dataHasVolume),
            x: features(rankSignal: -0.75, direction: -1.0, volume: 1.10)
        )
    }
    ...
    private static func features(rankSignal: Double, direction: Double, volume: Double) -> [Double] {
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("trend_xsmom_rank")
    }
}
