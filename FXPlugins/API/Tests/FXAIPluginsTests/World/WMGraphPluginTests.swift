...
final class WMGraphPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = WMGraphPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.graphWM.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "wm_graph")
        XCTAssertEqual(plugin.manifest.family, .worldModel)
        ...
        XCTAssertEqual(plan.pluginName, "wm_graph")
    ...
    func testConvertedPluginOwnsExpectedAccelerators() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/WMGraphCPUModel.swift").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CPU/WMGraphAccelerated.swift").path))
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("wm_graph")
    }
}
