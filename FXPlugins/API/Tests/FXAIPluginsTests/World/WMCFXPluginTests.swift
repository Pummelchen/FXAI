...
final class WMCFXPluginTests: XCTestCase {
    func testManifestAndAccelerationPlanMatchConvertedContract() throws {
        let plugin = WMCFXPlugin()
        let plan = plugin.accelerationPlan

        XCTAssertEqual(plugin.manifest.aiID, AIModelID.cfxWorld.rawValue)
        XCTAssertEqual(plugin.manifest.aiName, "wm_cfx")
        ...
        XCTAssertEqual(plan.pluginName, "wm_cfx")
    ...
    private static func pluginRoot() -> URL {
        ...
        return url.appendingPathComponent("wm_cfx")
    }
}
