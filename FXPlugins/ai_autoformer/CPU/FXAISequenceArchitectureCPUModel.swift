...
public struct FXAISequenceArchitectureCPUConfiguration: Hashable, Sendable {
    public var hiddenCount: Int
    public var architectureID: Int
    public var modelID: Int
    public var architectureMode: String
    public var family: AIFamily

    public init(hiddenCount: Int, architectureID: Int, modelID: Int? = nil, architectureMode: String, family: AIFamily) {
        self.hiddenCount = max(1, hiddenCount)
        self.architectureID = architectureID
        self.modelID = modelID ?? architectureID
        self.architectureMode = architectureMode
        self.family = family
    }
}

public struct FXAISequenceArchitectureCPUModel: Sendable {
    private static let featureCount = 32
    private static let volumeFeatureIndexes = [6, 68, 69, 70, 71, 74, 75, 76, 77, 78, 80, 81, 82, 83]

    private let configuration: FXAISequenceArchitectureCPUConfiguration
    ...
    public init(configuration: FXAISequenceArchitectureCPUConfiguration) {
        self.configuration = configuration
        self.steps = 0
        self.hiddenWeights = Array(repeating: Array(repeating: 0.0, count: Self.featureCount), count: configuration.hiddenCount)
        self.hiddenBias = Array(repeating: 0.0, count: configuration.hiddenCount)
        self.classWeights = Array(repeating: Array(repeating: 0.0, count: configuration.hiddenCount + 1), count: LabelClass.allCases.count)
        self.moveWeights = Array(repeating: 0.0, count: configuration.hiddenCount + 1)
        self.valueWeights = Array(repeating: 0.0, count: configuration.hiddenCount + 1)
        self.classMass = Array(repeating: 1.0, count: LabelClass.allCases.count)
        self.moveEMA = 0.0
        self.moveReady = false
        self.recurrentState = Array(repeating: 0.0, count: configuration.hiddenCount)
    ...
    public mutating func reset() {
        self = FXAISequenceArchitectureCPUModel(configuration: configuration)
    }
    ...
    private mutating func seedWeights() {
        for h in 0..<configuration.hiddenCount {
            hiddenBias[h] = seed(11, h) * 0.08
            recurrentState[h] = 0.0
            for i in 0..<Self.featureCount {
                hiddenWeights[h][i] = seed(h + 1, i + 3) * 0.12
            }
        }
        for c in 0..<LabelClass.allCases.count {
            for h in 0..<(configuration.hiddenCount + 1) {
                classWeights[c][h] = seed(c + 23, h + 5) * 0.05
            }
        }
        for h in 0..<(configuration.hiddenCount + 1) {
            moveWeights[h] = 0.04 * abs(seed(41, h + 7))
            valueWeights[h] = 0.03 * seed(61, h + 13)
        }
    }
    ...
    private func architectureSignals(x: [Double], window: [[Double]], slope: Double, std: Double, delta: Double, emaFast: Double, emaSlow: Double, attention: Double, longDelta: Double, volumePulse: Double, context: PluginContextV4) -> (Double, Double, Double, Double, Double, Double, Double, Double) {
        ...
        switch configuration.architectureMode {
        case "recurrent":
    ...
    private func mythosRefinement(slope: Double, delta: Double, attention: Double, energy: Double, macro: Double, volumePulse: Double) -> (Double, Double, Double, Double, Double, Double, Double, Double) {
        ...
        for depth in 0..<loops {
            for i in 0..<latent.count {
                ...
                let injected = 0.58 * latent[i] + 0.24 * memory + 0.18 * seed(depth + 3, i + 5) * difficulty
    ...
    private func hiddenActivations(features: [Double], window: [[Double]]) -> [Double] {
        FXAISequenceReferenceEncoders.encode(
            architectureMode: configuration.architectureMode,
            features: features,
            window: window,
            ...
            architectureID: configuration.architectureID
        )
    }

    private mutating func updateRecurrentState(hidden: [Double]) {
        let decay = stateDecay()
        for h in 0..<min(configuration.hiddenCount, hidden.count) {
    ...
    private func applyArchitectureRiskGate(_ probabilities: [Double], features: [Double], hidden: [Double], context: PluginContextV4) -> [Double] {
        ...
        var skipBoost = 0.0
        if configuration.architectureMode == "ppoPolicy" {
        ...
        if configuration.architectureMode == "mythosRDT" {
    ...
    private func quantileSkew(features: [Double]) -> Double {
        if configuration.architectureMode == "qcew" { return 0.45 + 0.15 * fxClamp(abs(features[27]), 0.0, 1.0) }
        return 0.55
    }
    ...
    private func seed(_ a: Int, _ b: Int) -> Double {
        sin(Double((configuration.modelID + 31) * (a + 3) * 67 + (b + 11) * 131 + configuration.architectureID * 17))
    }
}
