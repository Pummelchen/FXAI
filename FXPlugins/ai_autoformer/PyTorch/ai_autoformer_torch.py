...
@dataclass
class AIAutoformerTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AIAutoformerTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 0)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AIAutoformerTorchState] = None) -> torch.Tensor:
    state = state or AIAutoformerTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIAutoformerTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AIAutoformerTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIAutoformerTorchState] = None, lr: float = 0.01) -> AIAutoformerTorchState:
    state = state or AIAutoformerTorchState.create()
    x = _features(batch, state.input_weight.device)
