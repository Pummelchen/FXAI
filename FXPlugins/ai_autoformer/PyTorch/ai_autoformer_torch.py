"""PyTorch/MPS reference backend for FXAI ai_autoformer.

This plugin-local backend implements a Autoformer decomposition and auto-correlation model with real PyTorch layer
...
PLUGIN_NAME = "ai_autoformer"
ARCHITECTURE_MODE = "AUTOFORMER"
FEATURE_COUNT = 32
CLASS_COUNT = 3
HIDDEN_COUNT = 48
HEAD_COUNT = 4
...
class AIAutoformerReferenceModel(nn.Module):
    """Reference-grade Autoformer decomposition and auto-correlation model."""
...
@dataclass
class AIAutoformerReferenceState:
    model: AIAutoformerReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AIAutoformerReferenceState":
        dev = device or preferred_device()
        model = AIAutoformerReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AIAutoformerReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AIAutoformerReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AIAutoformerReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AIAutoformerReferenceState:
    state = state or AIAutoformerReferenceState.create(lr=lr)
    state.model.train()
