...
public struct AITSTPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.tst.rawValue,
        aiName: "ai_tst",
        family: .transformer,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.tst.rawValue),
    ...
    private var cpu: AITSTCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AITSTAccelerated.descriptor }

    public init() {
        self.cpu = AITSTCPUModel()
    }
