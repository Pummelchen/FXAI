...
@dataclass
class AITSTTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AITSTTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 19)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AITSTTorchState] = None) -> torch.Tensor:
    state = state or AITSTTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AITSTTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AITSTTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AITSTTorchState] = None, lr: float = 0.01) -> AITSTTorchState:
    state = state or AITSTTorchState.create()
    x = _features(batch, state.input_weight.device)
