...
public struct AITimesfmPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.timesfm.rawValue,
        aiName: "ai_timesfm",
        family: .transformer,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.timesfm.rawValue),
    ...
    private var cpu: AITimesfmCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AITimesfmAccelerated.descriptor }

    public init() {
        self.cpu = AITimesfmCPUModel()
    }
