...
ARCHITECTURE_MODE = "foundationForecaster"
FEATURE_COUNT = 32
HIDDEN_COUNT = 20
CLASS_COUNT = 3
...
@dataclass
class AITimesfmTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AITimesfmTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 18)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AITimesfmTorchState] = None) -> torch.Tensor:
    state = state or AITimesfmTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AITimesfmTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AITimesfmTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AITimesfmTorchState] = None, lr: float = 0.01) -> AITimesfmTorchState:
    state = state or AITimesfmTorchState.create()
    x = _features(batch, state.input_weight.device)
