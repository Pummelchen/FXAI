"""PyTorch/MPS reference backend for FXAI ai_timesfm.

This plugin-local backend implements a TimesFM-style patch foundation forecaster with real PyTorch layer
...
PLUGIN_NAME = "ai_timesfm"
ARCHITECTURE_MODE = "TIMESFM"
FEATURE_COUNT = 32
CLASS_COUNT = 3
HIDDEN_COUNT = 48
HEAD_COUNT = 4
...
class AITimesFMReferenceModel(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        if self.architecture == "MLP":
            ...
            if self.architecture == "TIMESFM":
                self.patch_embedding = nn.Sequential(nn.Linear(FEATURE_COUNT * self.patch_size, HIDDEN_COUNT), nn.LayerNorm(HIDDEN_COUNT), nn.GELU(), nn.Linear(HIDDEN_COUNT, HIDDEN_COUNT))
                self.frequency_position = nn.Parameter(torch.zeros(1, 128, HIDDEN_COUNT))
            else:
            ...
            self.foundation_encoder = nn.TransformerEncoder(layer, num_layers=6 if self.architecture == "TIMESFM" else 3)
        ...
        self._apply_pretrained_init()

    def _apply_pretrained_init(self) -> None:
        """Pretrained-style initialization: Kaiming normal for weights, Xavier uniform for patch embedding, scaled attention output projections."""
        ...
        if hasattr(self, "patch_embedding"):
            if isinstance(self.patch_embedding, nn.Sequential):
                for module in self.patch_embedding:
        ...
        for module in self.modules():
            if isinstance(module, nn.MultiheadAttention):
                if hasattr(module, "out_proj") and module.out_proj.weight is not None:
                    nn.init.xavier_uniform_(module.out_proj.weight, gain=1.0 / (2.0 ** 0.5))
    ...
    def _patches(self, x: torch.Tensor) -> torch.Tensor:
        if self.architecture == "TIMESFM":
            ...
            x = (x - mean) / std
            self._revin_mean = mean.detach()
            self._revin_std = std.detach()
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if arch in {"PATCHTST", "TIMESFM"}:
            ...
            if arch == "TIMESFM" and hasattr(self, "frequency_position"):
                patch_embed = patch_embed + self.frequency_position[:, : patches.shape[1], :]
            encoded = self.foundation_encoder(patch_embed + self.position[:, : patches.shape[1], :])
...
@dataclass
class AITimesFMReferenceState:
    model: AITimesFMReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AITimesFMReferenceState":
        dev = device or preferred_device()
        model = AITimesFMReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AITimesFMReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AITimesFMReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AITimesFMReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AITimesFMReferenceState:
    state = state or AITimesFMReferenceState.create(lr=lr)
    state.model.train()
    ...
    step = getattr(state, '_step', 0)
    setattr(state, '_step', step + 1)
    base_lr = lr
    cosine_t = 0.5 * (1.0 + torch.cos(torch.tensor(torch.pi * (step % 1000) / 1000.0)))
    current_lr = max(base_lr * 0.01, base_lr * cosine_t)
