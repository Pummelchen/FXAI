...
public struct RlPPOPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.rlPPO.rawValue,
        aiName: "rl_ppo",
        family: .other,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.rlPPO.rawValue),
    ...
    private var cpu: RlPPOCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { RlPPOAccelerated.descriptor }

    public init() {
        self.cpu = RlPPOCPUModel()
    }
