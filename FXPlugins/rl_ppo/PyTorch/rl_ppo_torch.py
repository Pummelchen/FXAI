...
@dataclass
class RlPPOTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "RlPPOTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 41)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[RlPPOTorchState] = None) -> torch.Tensor:
    state = state or RlPPOTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[RlPPOTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or RlPPOTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[RlPPOTorchState] = None, lr: float = 0.01) -> RlPPOTorchState:
    state = state or RlPPOTorchState.create()
    x = _features(batch, state.input_weight.device)
