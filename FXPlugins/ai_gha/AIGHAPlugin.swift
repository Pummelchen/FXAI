...
public struct AIGHAPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.gha.rawValue,
        aiName: "ai_gha",
        family: .stateSpace,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.gha.rawValue),
    ...
    private var cpu: AIGHACPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AIGHAAccelerated.descriptor }

    public init() {
        self.cpu = AIGHACPUModel()
    }
