"""PyTorch/MPS reference backend for FXAI ai_gha.

This plugin-local backend implements a generalized Hebbian/Oja projection model with real PyTorch layer
...
class AIGHAReferenceModel(nn.Module):
    """Reference-grade generalized Hebbian/Oja projection model."""
...
@dataclass
class AIGHAReferenceState:
    model: AIGHAReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AIGHAReferenceState":
        dev = device or preferred_device()
        model = AIGHAReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AIGHAReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AIGHAReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AIGHAReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AIGHAReferenceState:
    state = state or AIGHAReferenceState.create(lr=lr)
    state.model.train()
