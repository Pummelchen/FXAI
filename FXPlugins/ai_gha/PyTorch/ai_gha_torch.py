...
@dataclass
class AIGHATorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AIGHATorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 34)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AIGHATorchState] = None) -> torch.Tensor:
    state = state or AIGHATorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIGHATorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AIGHATorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIGHATorchState] = None, lr: float = 0.01) -> AIGHATorchState:
    state = state or AIGHATorchState.create()
    x = _features(batch, state.input_weight.device)
