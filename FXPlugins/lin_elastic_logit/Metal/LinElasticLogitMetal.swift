...
public enum LinElasticLogitMetal {
    public static let kernelSource = """
    #include <metal_stdlib>
    using namespace metal;

    kernel void lin_elastic_logit_scores(
    ...
    kernel void lin_elastic_logit_update(
        device const float* state [[buffer(0)]],
    ...
    public static let descriptor = FXPluginAccelerationPlan(
        pluginName: "lin_elastic_logit",
        primaryBackends: [.swiftSIMD, .accelerate],
        candidateBackends: [.metal],
        usesVolumeWhenAvailable: true,
        notes: "Plugin-local Swift CPU port of the framework elastic-logit kind. Metal folder contains fixed 16-state batch scoring and elastic update kernels; PyTorch, TensorFlow, and NLP are not suitable for this online linear model."
    )
}
