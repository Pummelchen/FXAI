...
public struct StatVMDCPUModel: Sendable {
    ...
    public func predict(_ request: PredictRequestV4, hyperParameters _: HyperParameters) -> PredictionV4 {
        ...
        let margin = vmdMargin(z: z, window: window)
        let modelConfidence = vmdConfidence(z: z, window: window, forecastVolatility: forecastVolatility, dataHasVolume: request.context.dataHasVolume)
    ...
    private func vmdMargin(z: [Double], window: [[Double]]) -> Double {
        let fastMode = window.isEmpty ? z[8] : Self.windowEMAMean(window, feature: 1, alpha: 0.55)
        let slowMode = window.isEmpty ? z[12] : Self.windowEMAMean(window, feature: 2, alpha: 0.85)
        return PluginSupportTools.clipSymmetric(
            0.40 * linearMargin(z: z) + 0.30 * fastMode + 0.30 * slowMode,
            limit: 8.0
        )
    }

    private func vmdConfidence(z: [Double], window: [[Double]], forecastVolatility: Double, dataHasVolume: Bool) -> Double {
        let fastMode = window.isEmpty ? z[8] : Self.windowEMAMean(window, feature: 1, alpha: 0.55)
        let slowMode = window.isEmpty ? z[12] : Self.windowEMAMean(window, feature: 2, alpha: 0.85)
        let decompositionStrength = fxClamp((abs(fastMode) + abs(slowMode) + 0.5 * abs(z[8])) / max(forecastVolatility, 0.05), 0.0, 2.0) / 2.0
    ...
    private static func seedWeight(_ a: Int, _ b: Int) -> Double {
        let raw = sin(Double((AIModelID.statVMD.rawValue + 17) * (a + 3) * 37 + (b + 11) * 101))
        return 0.035 * raw
    }
