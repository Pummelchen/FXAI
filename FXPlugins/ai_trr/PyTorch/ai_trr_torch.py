...
PLUGIN_NAME = "ai_trr"
ARCHITECTURE_MODE = "trendReversalRecurrent"
FEATURE_COUNT = 32
HIDDEN_COUNT = 16
CLASS_COUNT = 3
...
@dataclass
class AITRRTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AITRRTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 24)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AITRRTorchState] = None) -> torch.Tensor:
    state = state or AITRRTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AITRRTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AITRRTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AITRRTorchState] = None, lr: float = 0.01) -> AITRRTorchState:
    state = state or AITRRTorchState.create()
    x = _features(batch, state.input_weight.device)
