"""PyTorch/MPS reference backend for FXAI ai_trr.

This plugin-local backend implements a trend-reversal recurrent regime model with real PyTorch layer
...
class AITRRReferenceModel(nn.Module):
    """Reference-grade trend-reversal recurrent regime model."""
...
@dataclass
class AITRRReferenceState:
    model: AITRRReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AITRRReferenceState":
        dev = device or preferred_device()
        model = AITRRReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AITRRReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AITRRReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AITRRReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AITRRReferenceState:
    state = state or AITRRReferenceState.create(lr=lr)
    state.model.train()
