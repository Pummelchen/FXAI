...
public struct AITRRPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.trr.rawValue,
        aiName: "ai_trr",
        family: .recurrent,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.trr.rawValue),
    ...
    private var cpu: AITRRCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AITRRAccelerated.descriptor }

    public init() {
        self.cpu = AITRRCPUModel()
    }
