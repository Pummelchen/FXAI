...
public struct AICNNLSTMPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.cnnLSTM.rawValue,
        aiName: "ai_cnn_lstm",
        family: .convolutional,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.cnnLSTM.rawValue),
    ...
    private var cpu: AICNNLSTMCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AICNNLSTMAccelerated.descriptor }

    public init() {
        self.cpu = AICNNLSTMCPUModel()
    }
