"""PyTorch/MPS reference backend for FXAI ai_cnn_lstm.

This plugin-local backend implements a Conv1D feature extractor with LSTM decoder with real PyTorch layer
...
class AICNNLSTMReferenceModel(nn.Module):
    """Reference-grade Conv1D feature extractor with LSTM decoder."""
...
@dataclass
class AICNNLSTMReferenceState:
    model: AICNNLSTMReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AICNNLSTMReferenceState":
        dev = device or preferred_device()
        model = AICNNLSTMReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AICNNLSTMReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AICNNLSTMReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AICNNLSTMReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AICNNLSTMReferenceState:
    state = state or AICNNLSTMReferenceState.create(lr=lr)
    state.model.train()
