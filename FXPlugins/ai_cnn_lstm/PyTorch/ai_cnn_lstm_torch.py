...
@dataclass
class AICNNLSTMTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AICNNLSTMTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 46)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AICNNLSTMTorchState] = None) -> torch.Tensor:
    state = state or AICNNLSTMTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AICNNLSTMTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AICNNLSTMTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AICNNLSTMTorchState] = None, lr: float = 0.01) -> AICNNLSTMTorchState:
    state = state or AICNNLSTMTorchState.create()
    x = _features(batch, state.input_weight.device)
