...
class AICNNLSTMTensorFlowState:
    ...
    @classmethod
    def create(cls) -> "AICNNLSTMTensorFlowState":
        generator = tf.random.Generator.from_seed(20_000 + 46)
        return cls(
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AICNNLSTMTensorFlowState] = None) -> tf.Tensor:
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AICNNLSTMTensorFlowState] = None) -> dict[str, object]:
    state = state or AICNNLSTMTensorFlowState.create()
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AICNNLSTMTensorFlowState] = None, lr: float = 0.01) -> AICNNLSTMTensorFlowState:
    state = state or AICNNLSTMTensorFlowState.create()
