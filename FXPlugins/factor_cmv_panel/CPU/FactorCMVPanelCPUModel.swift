...
public struct FactorCMVPanelCPUModel: Sendable {
    ...
    public func predict(_ request: PredictRequestV4, hyperParameters _: HyperParameters) -> PredictionV4 {
        ...
        let margin = cmvPanelMargin(z: z)
        let modelConfidence = cmvConfidence(z: z, dataHasVolume: request.context.dataHasVolume)
    ...
    private func cmvPanelMargin(z: [Double]) -> Double {
        PluginSupportTools.clipSymmetric(
            0.40 * z[13] + 0.35 * z[8] - 0.25 * z[4] + 0.25 * linearMargin(z: z),
            limit: 8.0
        )
    }

    private func cmvConfidence(z: [Double], dataHasVolume: Bool) -> Double {
        let structure = min(abs(z[13]) + abs(z[8]) + 0.5 * abs(z[4]), 2.0) / 2.0
        let volumeBoost = dataHasVolume ? 0.06 * abs(z[7]) : 0.0
        return fxClamp(0.45 + 0.30 * structure + volumeBoost, 0.0, 1.0)
    }
    ...
    private static func seedWeight(_ a: Int, _ b: Int) -> Double {
        let raw = sin(Double((AIModelID.factorCMVPanel.rawValue + 17) * (a + 3) * 37 + (b + 11) * 101))
        return 0.035 * raw
    }
