"""PyTorch/MPS reference backend for FXAI ai_stmn.

This plugin-local backend implements a spatio-temporal memory network with real PyTorch layer
...
class AISTMNReferenceModel(nn.Module):
    """Reference-grade spatio-temporal memory network."""
...
@dataclass
class AISTMNReferenceState:
    model: AISTMNReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AISTMNReferenceState":
        dev = device or preferred_device()
        model = AISTMNReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AISTMNReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AISTMNReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AISTMNReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AISTMNReferenceState:
    state = state or AISTMNReferenceState.create(lr=lr)
    state.model.train()
