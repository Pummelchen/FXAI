...
@dataclass
class AIS4TorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AIS4TorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 13)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AIS4TorchState] = None) -> torch.Tensor:
    state = state or AIS4TorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIS4TorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AIS4TorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIS4TorchState] = None, lr: float = 0.01) -> AIS4TorchState:
    state = state or AIS4TorchState.create()
    x = _features(batch, state.input_weight.device)
