"""PyTorch/MPS reference backend for FXAI ai_s4.

This plugin-local backend implements a S4D diagonal state-space sequence model with real PyTorch layer
...
class AIS4ReferenceModel(nn.Module):
    """Reference-grade S4D diagonal state-space sequence model."""
...
@dataclass
class AIS4ReferenceState:
    model: AIS4ReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AIS4ReferenceState":
        dev = device or preferred_device()
        model = AIS4ReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AIS4ReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AIS4ReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AIS4ReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AIS4ReferenceState:
    state = state or AIS4ReferenceState.create(lr=lr)
    state.model.train()
