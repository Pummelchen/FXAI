...
public struct FactorPPPValueCPUModel: Sendable {
    ...
    public mutating func train(_ request: TrainRequestV4, hyperParameters _: HyperParameters) {
        ...
        let volumeMultiplier = request.context.dataHasVolume ? (1.0 + 0.04 * abs(z[7])) : 1.0
    ...
    public func predict(_ request: PredictRequestV4, hyperParameters _: HyperParameters) -> PredictionV4 {
        ...
        let margin = pppValueMargin(z: z)
        let modelConfidence = pppConfidence(horizonMinutes: request.context.horizonMinutes, z: z, dataHasVolume: request.context.dataHasVolume)
    ...
    private func pppValueMargin(z: [Double]) -> Double {
        PluginSupportTools.clipSymmetric(-0.55 * z[4] - 0.35 * z[13], limit: 8.0)
    }

    private func pppConfidence(horizonMinutes: Int, z: [Double], dataHasVolume: Bool) -> Double {
        let horizonConfidence = 0.20 + 0.10 * Double(min(max(horizonMinutes, 0), 240)) / 60.0
        let volumeBoost = dataHasVolume ? 0.04 * abs(z[7]) : 0.0
        return fxClamp(horizonConfidence + volumeBoost, 0.0, 0.75)
    }
