"""PyTorch/MPS reference backend for FXAI ai_nhits.

This plugin-local backend implements an N-HiTS-style hierarchical interpolation forecaster with real PyTorch layer
...
class AINhitsReferenceModel(nn.Module):
    """Reference-grade N-HiTS hierarchical interpolation forecaster."""
    def __init__(self) -> None:
        ...
        if self.architecture == "MLP":
            ...
            self.nhits_pool_layers = nn.ModuleList([nn.MaxPool1d(kernel_size=max(1, p), stride=max(1, p), padding=0) for p in self.nhits_pool_sizes])
...
@dataclass
class AINhitsReferenceState:
    model: AINhitsReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AINhitsReferenceState":
        dev = device or preferred_device()
        model = AINhitsReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AINhitsReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AINhitsReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AINhitsReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AINhitsReferenceState:
    state = state or AINhitsReferenceState.create(lr=lr)
    state.model.train()
