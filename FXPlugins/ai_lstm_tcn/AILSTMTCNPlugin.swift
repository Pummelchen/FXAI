...
public struct AILSTMTCNPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.lstmTCN.rawValue,
        aiName: "ai_lstm_tcn",
        family: .convolutional,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.lstmTCN.rawValue),
    ...
    private var cpu: AILSTMTCNCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AILSTMTCNAccelerated.descriptor }

    public init() {
        self.cpu = AILSTMTCNCPUModel()
    }
