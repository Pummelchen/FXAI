...
class AILSTMTCNTensorFlowState:
    ...
    @classmethod
    def create(cls) -> "AILSTMTCNTensorFlowState":
        generator = tf.random.Generator.from_seed(20_000 + 60)
        return cls(
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AILSTMTCNTensorFlowState] = None) -> tf.Tensor:
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AILSTMTCNTensorFlowState] = None) -> dict[str, object]:
    state = state or AILSTMTCNTensorFlowState.create()
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AILSTMTCNTensorFlowState] = None, lr: float = 0.01) -> AILSTMTCNTensorFlowState:
    state = state or AILSTMTCNTensorFlowState.create()
