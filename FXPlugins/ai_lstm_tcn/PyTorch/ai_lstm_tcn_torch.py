"""PyTorch/MPS reference backend for FXAI ai_lstm_tcn.

This plugin-local backend implements a LSTM encoder with dilated temporal convolutional network with real PyTorch layer
...
PLUGIN_NAME = "ai_lstm_tcn"
ARCHITECTURE_MODE = "LSTM_TCN"
FEATURE_COUNT = 32
CLASS_COUNT = 3
HIDDEN_COUNT = 48
HEAD_COUNT = 4
...
class AILSTMTCNReferenceModel(nn.Module):
    """Reference-grade LSTM encoder with dilated temporal convolutional network."""
...
@dataclass
class AILSTMTCNReferenceState:
    model: AILSTMTCNReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AILSTMTCNReferenceState":
        dev = device or preferred_device()
        model = AILSTMTCNReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AILSTMTCNReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AILSTMTCNReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AILSTMTCNReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AILSTMTCNReferenceState:
    state = state or AILSTMTCNReferenceState.create(lr=lr)
    state.model.train()
