...
@dataclass
class AILSTMTCNTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AILSTMTCNTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 60)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AILSTMTCNTorchState] = None) -> torch.Tensor:
    state = state or AILSTMTCNTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AILSTMTCNTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AILSTMTCNTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AILSTMTCNTorchState] = None, lr: float = 0.01) -> AILSTMTCNTorchState:
    state = state or AILSTMTCNTorchState.create()
    x = _features(batch, state.input_weight.device)
