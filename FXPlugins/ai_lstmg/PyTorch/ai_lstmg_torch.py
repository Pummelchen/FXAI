...
ARCHITECTURE_MODE = "gatedRecurrent"
FEATURE_COUNT = 32
HIDDEN_COUNT = 18
CLASS_COUNT = 3
...
@dataclass
class AILSTMGTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AILSTMGTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 8)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AILSTMGTorchState] = None) -> torch.Tensor:
    state = state or AILSTMGTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AILSTMGTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AILSTMGTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AILSTMGTorchState] = None, lr: float = 0.01) -> AILSTMGTorchState:
    state = state or AILSTMGTorchState.create()
    x = _features(batch, state.input_weight.device)
