"""PyTorch/MPS reference backend for FXAI ai_lstmg.

This plugin-local backend implements a LSTM with feature gate with real PyTorch layer
...
PLUGIN_NAME = "ai_lstmg"
ARCHITECTURE_MODE = "LSTMG"
FEATURE_COUNT = 32
CLASS_COUNT = 3
HIDDEN_COUNT = 48
HEAD_COUNT = 4
...
class AILSTMGReferenceModel(nn.Module):
    """Reference-grade LSTM with feature gate."""
...
@dataclass
class AILSTMGReferenceState:
    model: AILSTMGReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AILSTMGReferenceState":
        dev = device or preferred_device()
        model = AILSTMGReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AILSTMGReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AILSTMGReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AILSTMGReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AILSTMGReferenceState:
    state = state or AILSTMGReferenceState.create(lr=lr)
    state.model.train()
