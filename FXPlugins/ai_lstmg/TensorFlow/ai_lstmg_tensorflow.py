"""TensorFlow/Keras reference backend for FXAI ai_lstmg."""
...
class AILSTMGTensorFlowModel(tf.keras.Model):
    def __init__(self) -> None:
        super().__init__()
...
@dataclass
class AILSTMGTensorFlowState:
    model: AILSTMGTensorFlowModel
    optimizer: tf.keras.optimizers.Optimizer

    @classmethod
    def create(cls, lr: float = 3.0e-4) -> "AILSTMGTensorFlowState":
        return cls(model=AILSTMGTensorFlowModel(), optimizer=tf.keras.optimizers.AdamW(learning_rate=lr, weight_decay=1.0e-4))


def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AILSTMGTensorFlowState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AILSTMGTensorFlowState.create()
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AILSTMGTensorFlowState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AILSTMGTensorFlowState:
    state = state or AILSTMGTensorFlowState.create(lr=lr)
