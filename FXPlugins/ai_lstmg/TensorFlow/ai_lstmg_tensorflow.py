...
class AILSTMGTensorFlowState:
    ...
    @classmethod
    def create(cls) -> "AILSTMGTensorFlowState":
        generator = tf.random.Generator.from_seed(20_000 + 8)
        return cls(
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AILSTMGTensorFlowState] = None) -> tf.Tensor:
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AILSTMGTensorFlowState] = None) -> dict[str, object]:
    state = state or AILSTMGTensorFlowState.create()
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AILSTMGTensorFlowState] = None, lr: float = 0.01) -> AILSTMGTensorFlowState:
    state = state or AILSTMGTensorFlowState.create()
