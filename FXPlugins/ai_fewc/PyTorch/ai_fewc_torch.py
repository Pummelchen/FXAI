"""PyTorch/MPS reference backend for FXAI ai_fewc.

This plugin-local backend implements a feature ensemble with elastic weight consolidation with real PyTorch layer
...
class AIFEWCReferenceModel(nn.Module):
    """Reference-grade feature ensemble with elastic weight consolidation."""
...
@dataclass
class AIFEWCReferenceState:
    model: AIFEWCReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AIFEWCReferenceState":
        dev = device or preferred_device()
        model = AIFEWCReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AIFEWCReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AIFEWCReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AIFEWCReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AIFEWCReferenceState:
    state = state or AIFEWCReferenceState.create(lr=lr)
    state.model.train()
