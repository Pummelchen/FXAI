...
@dataclass
class AIFEWCTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AIFEWCTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 33)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AIFEWCTorchState] = None) -> torch.Tensor:
    state = state or AIFEWCTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIFEWCTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AIFEWCTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIFEWCTorchState] = None, lr: float = 0.01) -> AIFEWCTorchState:
    state = state or AIFEWCTorchState.create()
    x = _features(batch, state.input_weight.device)
