...
public struct AIFEWCPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.fewc.rawValue,
        aiName: "ai_fewc",
        family: .stateSpace,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.fewc.rawValue),
    ...
    private var cpu: AIFEWCCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AIFEWCAccelerated.descriptor }

    public init() {
        self.cpu = AIFEWCCPUModel()
    }
