"""PyTorch/MPS reference backend for FXAI ai_qcew.

This plugin-local backend implements a quantile cross-entropy window model with real PyTorch layer
...
class AIQCEWReferenceModel(nn.Module):
    """Reference-grade quantile cross-entropy window model."""
...
@dataclass
class AIQCEWReferenceState:
    model: AIQCEWReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AIQCEWReferenceState":
        dev = device or preferred_device()
        model = AIQCEWReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AIQCEWReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AIQCEWReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AIQCEWReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AIQCEWReferenceState:
    state = state or AIQCEWReferenceState.create(lr=lr)
    state.model.train()
