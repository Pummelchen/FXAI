...
@dataclass
class AIQCEWTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AIQCEWTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 32)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AIQCEWTorchState] = None) -> torch.Tensor:
    state = state or AIQCEWTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIQCEWTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AIQCEWTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIQCEWTorchState] = None, lr: float = 0.01) -> AIQCEWTorchState:
    state = state or AIQCEWTorchState.create()
    x = _features(batch, state.input_weight.device)
