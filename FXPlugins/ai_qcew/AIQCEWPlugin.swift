...
public struct AIQCEWPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.qcew.rawValue,
        aiName: "ai_qcew",
        family: .distributional,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.qcew.rawValue),
    ...
    private var cpu: AIQCEWCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AIQCEWAccelerated.descriptor }

    public init() {
        self.cpu = AIQCEWCPUModel()
    }
