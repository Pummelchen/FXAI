...
- 8 full plugin-owned native conversions: `lin_sgd`, `lin_ftrl`, `lin_enhash`, `lin_pa`,
  `lin_elastic_logit`, `lin_profit_logit`, `dist_quantile`, and `mem_retrdiff`,
  with Swift CPU code under each plugin's `CPU/` folder and Metal kernel sources under `Metal/`.
- 51 Swift reference adapters for the remaining legacy plugins, each in its own plugin folder with
