...
public struct FactorCarryCPUModel: Sendable {
    ...
    public func predict(_ request: PredictRequestV4, hyperParameters _: HyperParameters) -> PredictionV4 {
        ...
        let margin = carryMargin(z: z, forecastVolatility: forecastVolatility)
        let modelConfidence = carryConfidence(z: z, dataHasVolume: request.context.dataHasVolume)
    ...
    private func carryMargin(z: [Double], forecastVolatility: Double) -> Double {
        PluginSupportTools.clipSymmetric(
            0.75 * z[13] - 0.30 * forecastVolatility + 0.25 * linearMargin(z: z),
            limit: 8.0
        )
    }

    private func carryConfidence(z: [Double], dataHasVolume: Bool) -> Double {
        let volumeBoost = dataHasVolume ? 0.06 * abs(z[7]) : 0.0
        return fxClamp(0.55 + volumeBoost, 0.0, 1.0)
    }
    ...
    private static func seedWeight(_ a: Int, _ b: Int) -> Double {
        let raw = sin(Double((AIModelID.factorCarry.rawValue + 17) * (a + 3) * 37 + (b + 11) * 101))
        return 0.035 * raw
    }
