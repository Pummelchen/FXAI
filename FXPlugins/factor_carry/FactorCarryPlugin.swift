...
public struct FactorCarryPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.factorCarry.rawValue,
        aiName: "factor_carry",
        family: .other,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.factorCarry.rawValue),
    ...
    private var cpu: FactorCarryCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { FactorCarryAccelerated.descriptor }

    public init() {
        self.cpu = FactorCarryCPUModel()
    }
