"""TensorFlow/Keras reference backend for FXAI ai_bilstm."""
...
class AIBiLSTMTensorFlowModel(tf.keras.Model):
    def __init__(self) -> None:
        super().__init__()
...
class AIBiLSTMTensorFlowState:
    model: AIBiLSTMTensorFlowModel
    optimizer: tf.keras.optimizers.Optimizer

    @classmethod
    def create(cls, lr: float = 3.0e-4) -> "AIBiLSTMTensorFlowState":
        return cls(model=AIBiLSTMTensorFlowModel(), optimizer=tf.keras.optimizers.AdamW(learning_rate=lr, weight_decay=1.0e-4))


def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIBiLSTMTensorFlowState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIBiLSTMTensorFlowState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AIBiLSTMTensorFlowState:
    state = state or AIBiLSTMTensorFlowState.create(lr=lr)
