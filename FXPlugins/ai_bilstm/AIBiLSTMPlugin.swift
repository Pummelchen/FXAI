...
public struct AIBiLSTMPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.bilstm.rawValue,
        aiName: "ai_bilstm",
        family: .recurrent,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.bilstm.rawValue),
    ...
    private var cpu: AIBiLSTMCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AIBiLSTMAccelerated.descriptor }

    public init() {
        self.cpu = AIBiLSTMCPUModel()
    }
