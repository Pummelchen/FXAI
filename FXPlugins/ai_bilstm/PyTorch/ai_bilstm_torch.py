"""PyTorch/MPS reference backend for FXAI ai_bilstm.

This plugin-local backend implements a bidirectional LSTM sequence model with real PyTorch layer
...
class AIBiLSTMReferenceModel(nn.Module):
    """Reference-grade bidirectional LSTM sequence model."""
...
@dataclass
class AIBiLSTMReferenceState:
    model: AIBiLSTMReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AIBiLSTMReferenceState":
        dev = device or preferred_device()
        model = AIBiLSTMReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AIBiLSTMReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AIBiLSTMReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AIBiLSTMReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AIBiLSTMReferenceState:
    state = state or AIBiLSTMReferenceState.create(lr=lr)
    state.model.train()
