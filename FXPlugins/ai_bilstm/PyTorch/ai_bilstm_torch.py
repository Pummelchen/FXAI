...
@dataclass
class AIBiLSTMTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AIBiLSTMTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 59)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AIBiLSTMTorchState] = None) -> torch.Tensor:
    state = state or AIBiLSTMTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIBiLSTMTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AIBiLSTMTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIBiLSTMTorchState] = None, lr: float = 0.01) -> AIBiLSTMTorchState:
    state = state or AIBiLSTMTorchState.create()
    x = _features(batch, state.input_weight.device)
