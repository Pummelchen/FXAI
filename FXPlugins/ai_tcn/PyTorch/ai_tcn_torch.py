...
@dataclass
class AITCNTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AITCNTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 16)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AITCNTorchState] = None) -> torch.Tensor:
    state = state or AITCNTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AITCNTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AITCNTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AITCNTorchState] = None, lr: float = 0.01) -> AITCNTorchState:
    state = state or AITCNTorchState.create()
    x = _features(batch, state.input_weight.device)
