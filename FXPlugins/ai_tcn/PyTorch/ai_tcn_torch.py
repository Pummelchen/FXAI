"""PyTorch/MPS reference backend for FXAI ai_tcn.

This plugin-local backend implements a dilated causal temporal convolutional network with real PyTorch layer
...
class AITCNReferenceModel(nn.Module):
    """Reference-grade dilated causal temporal convolutional network."""
...
@dataclass
class AITCNReferenceState:
    model: AITCNReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AITCNReferenceState":
        dev = device or preferred_device()
        model = AITCNReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AITCNReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AITCNReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AITCNReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AITCNReferenceState:
    state = state or AITCNReferenceState.create(lr=lr)
    state.model.train()
