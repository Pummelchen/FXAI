...
class AITCNTensorFlowState:
    ...
    @classmethod
    def create(cls) -> "AITCNTensorFlowState":
        generator = tf.random.Generator.from_seed(20_000 + 16)
        return cls(
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AITCNTensorFlowState] = None) -> tf.Tensor:
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AITCNTensorFlowState] = None) -> dict[str, object]:
    state = state or AITCNTensorFlowState.create()
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AITCNTensorFlowState] = None, lr: float = 0.01) -> AITCNTensorFlowState:
    state = state or AITCNTensorFlowState.create()
