"""TensorFlow/Keras reference backend for FXAI ai_tcn."""
...
class AITCNTensorFlowModel(tf.keras.Model):
    def __init__(self) -> None:
        super().__init__()
...
@dataclass
class AITCNTensorFlowState:
    model: AITCNTensorFlowModel
    optimizer: tf.keras.optimizers.Optimizer

    @classmethod
    def create(cls, lr: float = 3.0e-4) -> "AITCNTensorFlowState":
        return cls(model=AITCNTensorFlowModel(), optimizer=tf.keras.optimizers.AdamW(learning_rate=lr, weight_decay=1.0e-4))


def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AITCNTensorFlowState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AITCNTensorFlowState.create()
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AITCNTensorFlowState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AITCNTensorFlowState:
    state = state or AITCNTensorFlowState.create(lr=lr)
