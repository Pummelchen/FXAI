...
public struct LinProfitLogitPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.linProfitLogit.rawValue,
        aiName: "lin_profit_logit",
        family: .linear,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.linProfitLogit.rawValue),
    ...
    private var cpu: LinProfitLogitCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { LinProfitLogitMetal.descriptor }

    public init() {
        self.cpu = LinProfitLogitCPUModel()
    }
