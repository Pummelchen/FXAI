...
public struct LinProfitLogitCPUModel: Sendable {
    private static let classCount = 3
    private static let stateDimension = 16
    private static let aiID = AIModelID.linProfitLogit.rawValue
    ...
    public func predict(_ request: PredictRequestV4, hyperParameters: HyperParameters) -> PredictionV4 {
        ...
        let baseMargin = linearMargin(state)
        let profitScale = 1.0 + 0.25 * fxClamp(moveEMA / max(cost, 0.10), 0.0, 2.0)
        let margin = PluginSupportTools.clipSymmetric(baseMargin * profitScale, limit: 8.0)
