...
public struct AITFTPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.tft.rawValue,
        aiName: "ai_tft",
        family: .transformer,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.tft.rawValue),
    ...
    private var cpu: AITFTCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AITFTAccelerated.descriptor }

    public init() {
        self.cpu = AITFTCPUModel()
    }
