...
PLUGIN_NAME = "ai_tft"
ARCHITECTURE_MODE = "temporalFusionTransformer"
FEATURE_COUNT = 32
HIDDEN_COUNT = 22
CLASS_COUNT = 3
...
@dataclass
class AITFTTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AITFTTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 17)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AITFTTorchState] = None) -> torch.Tensor:
    state = state or AITFTTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AITFTTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AITFTTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AITFTTorchState] = None, lr: float = 0.01) -> AITFTTorchState:
    state = state or AITFTTorchState.create()
    x = _features(batch, state.input_weight.device)
