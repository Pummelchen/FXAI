"""PyTorch/MPS reference backend for FXAI ai_tft.

This plugin-local backend implements a temporal fusion transformer with real PyTorch layer
...
class AITFTReferenceModel(nn.Module):
    """Reference-grade temporal fusion transformer."""
...
@dataclass
class AITFTReferenceState:
    model: AITFTReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AITFTReferenceState":
        dev = device or preferred_device()
        model = AITFTReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AITFTReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AITFTReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AITFTReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AITFTReferenceState:
    state = state or AITFTReferenceState.create(lr=lr)
    state.model.train()
