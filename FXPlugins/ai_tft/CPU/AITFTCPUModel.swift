...
public struct AITFTCPUModel: Sendable {
    private static let featureCount = 32
    private static let hiddenCount = 22
    private static let architectureID = 17
    private static let architectureMode = "temporalFusionTransformer"
    ...
    private static func seed(_ a: Int, _ b: Int) -> Double {
        sin(Double((AIModelID.tft.rawValue + 31) * (a + 3) * 67 + (b + 11) * 131 + Self.architectureID * 17))
    }
}
