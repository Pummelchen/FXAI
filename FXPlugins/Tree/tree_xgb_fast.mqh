#ifndef __FXAI_AI_XGB_FAST_MQH__
#define __FXAI_AI_XGB_FAST_MQH__

#include "..\..\FXDataEngine\API\plugin_base.mqh"

#define FXAI_XGBF_CLASS_COUNT 3
#define FXAI_XGBF_SELL 0
...
#define FXAI_XGBF_MIN_SPLIT_SAMPLES_MAX 32
#define FXAI_XGBF_MIN_CHILD_WEIGHT_MIN  0.05
#define FXAI_XGBF_MIN_CHILD_WEIGHT_MAX  1.50

struct FXAIXGBFastNode
{
   bool   is_leaf;
   int    feature;
   double threshold;
...
struct FXAIXGBFastTree
{
   int    node_count;
   int    age;
   double weight;
   FXAIXGBFastNode nodes[FXAI_XGBF_MAX_NODES];
};

class CFXAIAIXGBFast : public CFXAIAIPlugin
{
private:
   bool   m_initialized;
   int    m_step;

   double m_bias[FXAI_XGBF_CLASS_COUNT];

   FXAIXGBFastTree m_trees[FXAI_XGBF_CLASS_COUNT][FXAI_XGBF_MAX_TREES];
   int            m_tree_count[FXAI_XGBF_CLASS_COUNT];

   // Ring buffer.
   double m_buf_x[FXAI_XGBF_BUFFER][FXAI_AI_WEIGHTS];
   int    m_buf_class[FXAI_XGBF_BUFFER];
   double m_buf_move[FXAI_XGBF_BUFFER];
   double m_buf_cost[FXAI_XGBF_BUFFER];
   double m_buf_w[FXAI_XGBF_BUFFER];
   int    m_buf_head;
   int    m_buf_size;
   ...
   void InitTree(FXAIXGBFastTree &tree) const
   {
      tree.node_count = 1;
      tree.age = 0;
      tree.weight = 1.0;
      for(int n=0; n<FXAI_XGBF_MAX_NODES; n++)
      {
   ...
   int BufPos(const int logical_idx) const
   {
      ...
      while(start < 0) start += FXAI_XGBF_BUFFER;
      int p = start + logical_idx;
      while(p >= FXAI_XGBF_BUFFER) p -= FXAI_XGBF_BUFFER;
      return p;
   }
                   ...
                   const double sample_w)
   {
      ...
      m_buf_head++;
      if(m_buf_head >= FXAI_XGBF_BUFFER) m_buf_head = 0;
      if(m_buf_size < FXAI_XGBF_BUFFER) m_buf_size++;
   }
   ...
   {
      if(y == FXAI_XGBF_SELL || y == FXAI_XGBF_BUY || y == FXAI_XGBF_SKIP)
         return y;
      ...
      if(edge <= skip_band) return FXAI_XGBF_SKIP;

      if(y > 0) return FXAI_XGBF_BUY;
      if(y == 0) return FXAI_XGBF_SELL;
      return (move_points >= 0.0 ? FXAI_XGBF_BUY : FXAI_XGBF_SELL);
   }
   ...
   void ConfigureRuntime(const FXAIAIHyperParams &hp)
   {
      ...
      m_cfg_colsample = FXAI_Clamp(0.60 + (0.35 * complexity), 0.50, 1.00);

      // 4) Regularization controls.
      m_cfg_lambda = FXAI_Clamp(hp.xgb_l2, 0.0001, 20.0);
      m_cfg_alpha = FXAI_Clamp(0.02 + 0.80 * hp.l2 + 0.08 * complexity, 0.0, 2.0);
      m_cfg_gamma = FXAI_Clamp(0.005 + 0.035 * complexity + 0.02 * hp.l2, 0.0, 0.25);
      m_cfg_min_child_weight = FXAI_Clamp(0.06 + 0.35 * hp.l2, FXAI_XGBF_MIN_CHILD_WEIGHT_MIN, FXAI_XGBF_MIN_CHILD_WEIGHT_MAX);
      m_cfg_min_split_samples = ClampInt(10 + (int)MathRound(10.0 * hp.l2), FXAI_XGBF_MIN_SPLIT_SAMPLES_MIN, FXAI_XGBF_MIN_SPLIT_SAMPLES_MAX);
      m_cfg_max_delta = FXAI_Clamp(0.80 + 2.40 * complexity, 0.25, 4.00);
      m_cfg_leaf_clip = FXAI_Clamp(3.00 + 3.00 * complexity, 2.00, 8.00);

      // 8) Drift controls.
      m_cfg_tree_decay = FXAI_Clamp(0.9985 - 0.0020 * complexity, 0.9900, 0.9999);
      m_cfg_refresh_every = ClampInt(256 - (int)MathRound(128.0 * complexity), 64, 512);
      m_cfg_refresh_lr = FXAI_Clamp(0.03 + 0.08 * complexity, 0.01, 0.25);
   }
   ...
   int TraverseLeafIndex(const FXAIXGBFastTree &tree, const double &x[]) const
   {
      int node = 0;
      int guard = 0;
      while(node >= 0 && node < tree.node_count && guard < FXAI_XGBF_MAX_NODES)
      {
   ...
   double TreeOutput(const FXAIXGBFastTree &tree, const double &x[]) const
   {
   ...
   {
      double margins[FXAI_XGBF_CLASS_COUNT];
      for(int c=0; c<FXAI_XGBF_CLASS_COUNT; c++)
         margins[c] = ClassMargin(c, x);

      double m = margins[0];
      for(int c=1; c<FXAI_XGBF_CLASS_COUNT; c++)
         if(margins[c] > m) m = margins[c];

      double sum = 0.0;
      for(int c=0; c<FXAI_XGBF_CLASS_COUNT; c++)
      {
         probs[c] = MathExp(FXAI_Clamp(margins[c] - m, -30.0, 30.0));
         sum += probs[c];
      }

      if(sum <= 0.0)
      {
         for(int c=0; c<FXAI_XGBF_CLASS_COUNT; c++) probs[c] = 1.0 / 3.0;
         return;
      }
   ...
   void BuildCuts(const int &sidx[],
                  ...
                  const int bins,
                  double &cuts[][FXAI_XGBF_MAX_BINS - 1]) const
   {
      for(int f=0; f<FXAI_AI_WEIGHTS; f++)
         for(int b=0; b<FXAI_XGBF_MAX_BINS - 1; b++)
            cuts[f][b] = DBL_MAX;
   ...
   bool FindBestSplit(const int node_idx,
                      ...
                      const double &h[],
                      const double &x_all[][FXAI_AI_WEIGHTS],
                      const double &cuts[][FXAI_XGBF_MAX_BINS - 1],
                      int &best_feature,
                      double &best_thr,
                      ...
                      double &best_gain) const
   {
      ...
      for(int f=1; f<FXAI_AI_WEIGHTS; f++)
      {
         ...
         double gbin[FXAI_XGBF_MAX_BINS];
         double hbin[FXAI_XGBF_MAX_BINS];
         int    cbin[FXAI_XGBF_MAX_BINS];
   ...
   void SetLeaf(FXAIXGBFastTree &tree,
                const int node_idx,
                const int &sidx[],
                const double &g[],
                const double &h[],
                ...
                const double eta)
   {
      ...
      leaf = FXAI_ClipSym(leaf, m_cfg_max_delta);
      tree.nodes[node_idx].leaf_value = FXAI_ClipSym(leaf, m_cfg_leaf_clip);
   ...
   void BuildNode(FXAIXGBFastTree &tree,
                  const int node_idx,
                  const int depth,
                  const int &sidx[],
                  const double &g[],
                  ...
                  const double eta)
   {
      if(node_idx < 0 || node_idx >= FXAI_XGBF_MAX_NODES) return;
      ...
      if(ArraySize(left_idx) < m_cfg_min_split_samples ||
         ...
         tree.node_count + 2 > FXAI_XGBF_MAX_NODES)
      {
         SetLeaf(tree, node_idx, sidx, g, h, mv, eta);
         return;
      }
   ...
   void CompactClassTrees(const int cls)
   {
      ...
      FXAIXGBFastTree keep[FXAI_XGBF_MAX_TREES];
      int k = 0;
      for(int i=0; i<cnt; i++)
      {
      ...
      if(lim > FXAI_XGBF_MAX_TREES) lim = FXAI_XGBF_MAX_TREES;
      if(k > lim)
      {
         int drop = k - lim;
         for(int i=0; i<lim; i++)
   ...
   bool BuildOneClassTree(const int cls, const FXAIAIHyperParams &hp)
   {
      ...
      for(int i=0; i<n; i++)
      {
         ...
         double edge_w = FXAI_MoveEdgeWeight(mv_all[i], cost_all[i]);

         if(cls == FXAI_XGBF_SKIP)
         {
         ...
         else
         {
            ...
            else            edge_w *= FXAI_Clamp(1.0 + 0.05 * MathMin(edge, 20.0), 1.0, 2.0);
         }

         double wi = FXAI_Clamp(w_all[i] * edge_w, 0.10, 8.00);
         g[i] = ((double)yk - p) * wi;
         h[i] = FXAI_Clamp(p * (1.0 - p) * wi, 0.01, 8.0);
      }
      ...
      if(Hbatch > 1e-9)
      {
         ...
         bstep = FXAI_ClipSym(bstep, m_cfg_max_delta);
         m_bias[cls] += bstep;
         m_bias[cls] = FXAI_Clamp(m_bias[cls], -8.0, 8.0);
      }

      // 5) Histogram cuts reused through node growth.
      double cuts[][FXAI_XGBF_MAX_BINS - 1];
      ArrayResize(cuts, FXAI_AI_WEIGHTS);
      BuildCuts(root_idx, x_all, m_cfg_bins, cuts);

      FXAIXGBFastTree tree;
      InitTree(tree);
      tree.weight = 1.0;
      tree.age = 0;

      BuildNode(tree, 0, 0, root_idx, g, h, x_all, mv_all, cuts, FXAI_Clamp(hp.xgb_lr, 0.0001, 1.0000));
   ...
   void RefreshLeaves(void)
   {
      ...
      for(int cls=0; cls<FXAI_XGBF_CLASS_COUNT; cls++)
      {
         for(int t=0; t<m_tree_count[cls]; t++)
         {
            double cnt[FXAI_XGBF_MAX_NODES];
            double pos[FXAI_XGBF_MAX_NODES];
            double sum_mv[FXAI_XGBF_MAX_NODES];
            double sum_sq[FXAI_XGBF_MAX_NODES];
            double sum_abs[FXAI_XGBF_MAX_NODES];
            ...
            for(int li=start; li<m_buf_size; li++)
            {
               int p = BufPos(li);
               double xloc[FXAI_AI_WEIGHTS];
               for(int k=0; k<FXAI_AI_WEIGHTS; k++) xloc[k] = m_buf_x[p][k];
            ...
            for(int n=0; n<m_trees[cls][t].node_count; n++)
            {
               ...
               double target_logit = FXAI_Logit(FXAI_Clamp(p_cls, 0.01, 0.99));
               double delta = FXAI_ClipSym(target_logit - m_trees[cls][t].nodes[n].leaf_value, m_cfg_max_delta);
               m_trees[cls][t].nodes[n].leaf_value += m_cfg_refresh_lr * delta;
               m_trees[cls][t].nodes[n].leaf_value = FXAI_ClipSym(m_trees[cls][t].nodes[n].leaf_value, m_cfg_leaf_clip);
            }
         }
      }
   }
...
public:
   ...
   virtual void Reset(void)
   {
      ...
      for(int c=0; c<FXAI_XGBF_CLASS_COUNT; c++)
      {
         m_bias[c] = 0.0;
         m_tree_count[c] = 0;
         for(int t=0; t<FXAI_XGBF_MAX_TREES; t++)
            InitTree(m_trees[c][t]);
      }

      m_buf_head = 0;
      m_buf_size = 0;
      for(int i=0; i<FXAI_XGBF_BUFFER; i++)
      {
         m_buf_class[i] = FXAI_XGBF_SKIP;
         m_buf_move[i] = 0.0;
         m_buf_cost[i] = 0.0;
         m_buf_w[i] = 1.0;
         for(int k=0; k<FXAI_AI_WEIGHTS; k++)
                               ...
                               const double move_points)
   {
      ...
      double den = probs_now[FXAI_XGBF_BUY] + probs_now[FXAI_XGBF_SELL];
      if(den < 1e-9) den = 1e-9;
      double p_dir = probs_now[FXAI_XGBF_BUY] / den;
      if(cls != FXAI_XGBF_SKIP)
      {
         int y_dir = (cls == FXAI_XGBF_BUY ? 1 : 0);
   ...
   virtual double PredictProb(const double &x[], const FXAIAIHyperParams &hp)
   {
      ...
      double p_up = p_dir_cal * FXAI_Clamp(1.0 - probs[FXAI_XGBF_SKIP], 0.0, 1.0);
   ...
   virtual double PredictExpectedMovePoints(const double &x[], const FXAIAIHyperParams &hp)
   {
      ...
      double qmix = probs[FXAI_XGBF_BUY] * q_buy + probs[FXAI_XGBF_SELL] * q_sell;
...
#endif // __FXAI_AI_XGB_FAST_MQH__
