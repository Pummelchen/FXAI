"""PyTorch/MPS reference backend for FXAI ai_mlp.

This plugin-local backend implements a deep multilayer perceptron with real PyTorch layer
...
class AIMLPReferenceModel(nn.Module):
    """Reference-grade deep multilayer perceptron."""
...
@dataclass
class AIMLPReferenceState:
    model: AIMLPReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AIMLPReferenceState":
        dev = device or preferred_device()
        model = AIMLPReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AIMLPReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AIMLPReferenceState.create()
    state.model.eval()
