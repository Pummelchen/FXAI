...
public struct StatARIMAXGARCHCPUModel: Sendable {
    ...
    private var arResidualEMA: Double
    ...
    public init() {
        ...
        self.garchVariance = 0.04
        self.arResidualEMA = 0.0
        self.moveEMA = 0.0
    ...
    public mutating func train(_ request: TrainRequestV4, hyperParameters: HyperParameters) {
        ...
        updateGARCH(z: z, signedMove: signedMove, sampleWeight: sampleWeight)
    ...
    public func predict(_ request: PredictRequestV4, hyperParameters _: HyperParameters) -> PredictionV4 {
        ...
        let window = Self.preparedWindow(request.xWindow, dataHasVolume: request.context.dataHasVolume)
        let z = buildState(x: x, window: window, horizonMinutes: request.context.horizonMinutes)
        let forecastVolatility = max(windowVolatility(z: z), sqrt(max(garchVariance, 1.0e-8)))
        let margin = arimaxGarchMargin(z: z, forecastVolatility: forecastVolatility)
        let modelConfidence = fxClamp(1.0 / (1.0 + forecastVolatility + abs(arResidualEMA)), 0.0, 1.0)
    ...
    private func arimaxGarchMargin(z: [Double], forecastVolatility: Double) -> Double {
        let base = linearMargin(z: z)
        let exogenous = (z[8] + 0.35 * z[13]) / max(forecastVolatility, 0.05)
        return PluginSupportTools.clipSymmetric(0.55 * base + 0.45 * exogenous, limit: 8.0)
    }
    ...
    private mutating func updateGARCH(z: [Double], signedMove: Double, sampleWeight: Double) {
        let predicted = linearMargin(z: z)
        let residual = fxClamp(signedMove * 0.01 - predicted * 0.02, -8.0, 8.0)
        let alpha = fxClamp(0.07 + 0.01 * sampleWeight, 0.03, 0.18)
        let beta = fxClamp(0.88 - 0.02 * sampleWeight, 0.70, 0.94)
        let omega = 0.0005
        garchVariance = max(omega + alpha * residual * residual + beta * garchVariance, 1.0e-8)
        arResidualEMA = 0.96 * arResidualEMA + 0.04 * residual
    }
    ...
    private static func seedWeight(_ a: Int, _ b: Int) -> Double {
        let raw = sin(Double((AIModelID.statARIMAXGARCH.rawValue + 17) * (a + 3) * 37 + (b + 11) * 101))
        return 0.035 * raw
    }
