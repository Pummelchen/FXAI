...
public struct StatARIMAXGARCHPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.statARIMAXGARCH.rawValue,
        aiName: "stat_arimax_garch",
        family: .stateSpace,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.statARIMAXGARCH.rawValue),
    ...
    private var cpu: StatARIMAXGARCHCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { StatARIMAXGARCHAccelerated.descriptor }

    public init() {
        self.cpu = StatARIMAXGARCHCPUModel()
    }
