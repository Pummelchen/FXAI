...
public struct AIChronosPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.chronos.rawValue,
        aiName: "ai_chronos",
        family: .transformer,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.chronos.rawValue),
    ...
    private var cpu: AIChronosCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AIChronosAccelerated.descriptor }

    public init() {
        self.cpu = AIChronosCPUModel()
    }
