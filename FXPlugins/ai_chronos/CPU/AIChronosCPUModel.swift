...
public struct AIChronosCPUModel: Sendable {
    private static let featureCount = 32
    private static let hiddenCount = 20
    private static let architectureID = 2
    private static let architectureMode = "causalTokenForecaster"
    ...
    private static func seed(_ a: Int, _ b: Int) -> Double {
        sin(Double((AIModelID.chronos.rawValue + 31) * (a + 3) * 67 + (b + 11) * 131 + Self.architectureID * 17))
    }
}
