"""PyTorch/MPS reference backend for FXAI ai_chronos.

This plugin-local backend implements a Chronos-style causal token forecaster with real PyTorch layer
...
class AIChronosReferenceModel(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        if self.architecture == "MLP":
            ...
            self.token_scale = nn.Parameter(torch.ones(1, 1, HIDDEN_COUNT) * 0.02)
            self.input_ln = nn.LayerNorm(HIDDEN_COUNT)
            self.position = nn.Parameter(torch.zeros(1, 128, HIDDEN_COUNT))
            layer = nn.TransformerEncoderLayer(d_model=HIDDEN_COUNT, nhead=HEAD_COUNT, dim_feedforward=HIDDEN_COUNT * 4, batch_first=True, activation="gelu")
            self.causal_transformer = nn.TransformerEncoder(layer, num_layers=6)
    ...
    def _apply_pretrained_init(self) -> None:
        """Pretrained-style initialization: Xavier uniform for weights, zero for biases, scaled attention output projections."""
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if arch == "CHRONOS":
            ...
            embedded = self.input_ln(self.token_embedding(tokens) * (1.0 + self.token_scale) + self.position[:, : tokens.shape[1], :])
...
@dataclass
class AIChronosReferenceState:
    model: AIChronosReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AIChronosReferenceState":
        dev = device or preferred_device()
        model = AIChronosReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AIChronosReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AIChronosReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AIChronosReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AIChronosReferenceState:
    state = state or AIChronosReferenceState.create(lr=lr)
    state.model.train()
