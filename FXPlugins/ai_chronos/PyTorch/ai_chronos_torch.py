...
@dataclass
class AIChronosTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AIChronosTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 2)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AIChronosTorchState] = None) -> torch.Tensor:
    state = state or AIChronosTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIChronosTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AIChronosTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIChronosTorchState] = None, lr: float = 0.01) -> AIChronosTorchState:
    state = state or AIChronosTorchState.create()
    x = _features(batch, state.input_weight.device)
