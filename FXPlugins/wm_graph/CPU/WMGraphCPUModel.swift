...
public struct WMGraphCPUModel: Sendable {
    private static let featureCount = 32
    private static let hiddenCount = 18
    private static let architectureID = 25
    private static let architectureMode = "graphWorld"
    ...
    private static func seed(_ a: Int, _ b: Int) -> Double {
        sin(Double((AIModelID.graphWM.rawValue + 31) * (a + 3) * 67 + (b + 11) * 131 + Self.architectureID * 17))
    }
}
