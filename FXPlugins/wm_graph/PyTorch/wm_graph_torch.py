...
@dataclass
class WMGraphTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "WMGraphTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 25)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[WMGraphTorchState] = None) -> torch.Tensor:
    state = state or WMGraphTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[WMGraphTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or WMGraphTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[WMGraphTorchState] = None, lr: float = 0.01) -> WMGraphTorchState:
    state = state or WMGraphTorchState.create()
    x = _features(batch, state.input_weight.device)
