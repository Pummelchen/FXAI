"""PyTorch/MPS reference backend for FXAI wm_graph.

This plugin-local backend implements a FX graph message-passing world model with real PyTorch layer
...
PLUGIN_NAME = "wm_graph"
ARCHITECTURE_MODE = "WM_GRAPH"
FEATURE_COUNT = 32
CLASS_COUNT = 3
HIDDEN_COUNT = 48
HEAD_COUNT = 4
...
class WMGraphReferenceModel(nn.Module):
    """Reference-grade FX graph message-passing world model."""
...
@dataclass
class WMGraphReferenceState:
    model: WMGraphReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "WMGraphReferenceState":
        dev = device or preferred_device()
        model = WMGraphReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[WMGraphReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or WMGraphReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[WMGraphReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> WMGraphReferenceState:
    state = state or WMGraphReferenceState.create(lr=lr)
    state.model.train()
