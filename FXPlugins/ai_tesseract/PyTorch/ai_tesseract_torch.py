"""PyTorch/MPS reference backend for FXAI ai_tesseract.

This plugin-local backend implements a tensor factorization network with real PyTorch layer
...
PLUGIN_NAME = "ai_tesseract"
ARCHITECTURE_MODE = "TESSERACT"
FEATURE_COUNT = 32
CLASS_COUNT = 3
HIDDEN_COUNT = 48
HEAD_COUNT = 4
...
class AITesseractReferenceModel(nn.Module):
    """Reference-grade tensor factorization network."""
...
@dataclass
class AITesseractReferenceState:
    model: AITesseractReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AITesseractReferenceState":
        dev = device or preferred_device()
        model = AITesseractReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AITesseractReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AITesseractReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AITesseractReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AITesseractReferenceState:
    state = state or AITesseractReferenceState.create(lr=lr)
    state.model.train()
