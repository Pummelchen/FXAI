...
ARCHITECTURE_MODE = "tensorTesseract"
FEATURE_COUNT = 32
HIDDEN_COUNT = 20
CLASS_COUNT = 3
...
@dataclass
class AITesseractTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AITesseractTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 35)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AITesseractTorchState] = None) -> torch.Tensor:
    state = state or AITesseractTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AITesseractTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AITesseractTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AITesseractTorchState] = None, lr: float = 0.01) -> AITesseractTorchState:
    state = state or AITesseractTorchState.create()
    x = _features(batch, state.input_weight.device)
