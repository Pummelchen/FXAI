...
public struct AITesseractPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.tesseract.rawValue,
        aiName: "ai_tesseract",
        family: .stateSpace,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.tesseract.rawValue),
    ...
    private var cpu: AITesseractCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AITesseractAccelerated.descriptor }

    public init() {
        self.cpu = AITesseractCPUModel()
    }
