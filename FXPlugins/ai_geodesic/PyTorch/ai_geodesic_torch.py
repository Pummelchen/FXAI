...
ARCHITECTURE_MODE = "geodesicAttention"
FEATURE_COUNT = 32
HIDDEN_COUNT = 20
CLASS_COUNT = 3
...
@dataclass
class AIGeodesicTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AIGeodesicTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 5)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AIGeodesicTorchState] = None) -> torch.Tensor:
    state = state or AIGeodesicTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIGeodesicTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AIGeodesicTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIGeodesicTorchState] = None, lr: float = 0.01) -> AIGeodesicTorchState:
    state = state or AIGeodesicTorchState.create()
    x = _features(batch, state.input_weight.device)
