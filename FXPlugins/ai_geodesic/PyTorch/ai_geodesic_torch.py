"""PyTorch/MPS reference backend for FXAI ai_geodesic.

This plugin-local backend implements a geodesic/RBF attention over feature manifolds with real PyTorch layer
...
PLUGIN_NAME = "ai_geodesic"
ARCHITECTURE_MODE = "GEODESIC"
FEATURE_COUNT = 32
CLASS_COUNT = 3
HIDDEN_COUNT = 48
HEAD_COUNT = 4
...
class AIGeodesicReferenceModel(nn.Module):
    """Reference-grade geodesic/RBF attention over feature manifolds."""
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if arch == "GEODESIC":
            current = x[:, -1, :]
            delta = current.unsqueeze(1) - self.landmarks.to(current.device).unsqueeze(0)
            distances = torch.sqrt(torch.sum(delta * delta, dim=-1) + 1.0e-8)
...
@dataclass
class AIGeodesicReferenceState:
    model: AIGeodesicReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AIGeodesicReferenceState":
        dev = device or preferred_device()
        model = AIGeodesicReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AIGeodesicReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AIGeodesicReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AIGeodesicReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AIGeodesicReferenceState:
    state = state or AIGeodesicReferenceState.create(lr=lr)
    state.model.train()
