...
public struct AIGeodesicPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.geodesicAttention.rawValue,
        aiName: "ai_geodesic",
        family: .transformer,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.geodesicAttention.rawValue),
    ...
    private var cpu: AIGeodesicCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AIGeodesicAccelerated.descriptor }

    public init() {
        self.cpu = AIGeodesicCPUModel()
    }
