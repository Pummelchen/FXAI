...
public struct FactorPCAPanelPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.factorPCAPanel.rawValue,
        aiName: "factor_pca_panel",
        family: .stateSpace,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.factorPCAPanel.rawValue),
    ...
    private var cpu: FactorPCAPanelCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { FactorPCAPanelAccelerated.descriptor }

    public init() {
        self.cpu = FactorPCAPanelCPUModel()
    }
