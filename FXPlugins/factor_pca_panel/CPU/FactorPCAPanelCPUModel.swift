...
public struct FactorPCAPanelCPUModel: Sendable {
    private static let stateDimension = 16
    private static let componentCount = 3

    private var steps: Int
    private var pcaMean: [Double]
    private var pcaLoading: [[Double]]
    private var pcaVariance: [Double]
    ...
    public init() {
        self.steps = 0
        self.pcaMean = Array(repeating: 0.0, count: Self.stateDimension)
        self.pcaLoading = Array(
            repeating: Array(repeating: 0.0, count: Self.stateDimension),
            count: Self.componentCount
        )
        self.pcaVariance = Array(repeating: 1.0, count: Self.componentCount)
        self.moveEMA = 0.0
        self.moveReady = false
        self.classMass = Array(repeating: 1.0, count: LabelClass.allCases.count)
        self.qualityBank = PluginQualityBank()

        for component in 0..<Self.componentCount {
            for index in 0..<Self.stateDimension {
                pcaLoading[component][index] = (component == index % Self.componentCount && index != 0) ? 1.0 : 0.0
            }
        }
    }

    public mutating func reset() {
        self = FactorPCAPanelCPUModel()
    }
    ...
    public mutating func train(_ request: TrainRequestV4, hyperParameters _: HyperParameters) {
        ...
        updatePCA(z: z, sampleWeight: sampleWeight)
    ...
    public func predict(_ request: PredictRequestV4, hyperParameters _: HyperParameters) -> PredictionV4 {
        ...
        let (margin, modelConfidence) = pcaMargin(z: z, dataHasVolume: request.context.dataHasVolume)
    ...
    private func pcaMargin(z: [Double], dataHasVolume: Bool) -> (margin: Double, confidence: Double) {
        var score = Array(repeating: 0.0, count: Self.componentCount)
        var totalVariance = 1.0e-6
        for component in 0..<Self.componentCount {
            for index in 0..<Self.stateDimension {
                score[component] += (z[index] - pcaMean[index]) * pcaLoading[component][index]
            }
            totalVariance += max(pcaVariance[component], 0.0)
        }
        let volumeBoost = dataHasVolume ? 0.05 * abs(z[7]) : 0.0
        let confidence = fxClamp((pcaVariance[0] + pcaVariance[1]) / totalVariance + volumeBoost, 0.0, 1.0)
        let margin = PluginSupportTools.clipSymmetric(0.55 * score[0] + 0.30 * score[1] - 0.15 * abs(score[2]), limit: 8.0)
        return (margin, confidence)
    }

    private mutating func updatePCA(z: [Double], sampleWeight: Double) {
        let alpha = fxClamp(0.01 * sampleWeight, 0.001, 0.05)
        for index in 0..<Self.stateDimension {
            let centered = z[index] - pcaMean[index]
            pcaMean[index] += alpha * centered
            for component in 0..<Self.componentCount {
                let projection = centered * pcaLoading[component][index]
                pcaLoading[component][index] += alpha * projection * centered
                pcaLoading[component][index] = fxClamp(pcaLoading[component][index], -4.0, 4.0)
                pcaVariance[component] = 0.995 * pcaVariance[component] + 0.005 * projection * projection
            }
        }
    }
