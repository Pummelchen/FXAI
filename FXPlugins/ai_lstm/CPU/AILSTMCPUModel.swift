...
public struct AILSTMCPUModel: Sendable {
    private static let featureCount = 32
    private static let hiddenCount = 18
    private static let architectureID = 7
    private static let architectureMode = "recurrent"
    ...
    private static func seed(_ a: Int, _ b: Int) -> Double {
        sin(Double((AIModelID.lstm.rawValue + 31) * (a + 3) * 67 + (b + 11) * 131 + Self.architectureID * 17))
    }
}
