...
class AILSTMTensorFlowState:
    ...
    @classmethod
    def create(cls) -> "AILSTMTensorFlowState":
        generator = tf.random.Generator.from_seed(20_000 + 7)
        return cls(
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AILSTMTensorFlowState] = None) -> tf.Tensor:
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AILSTMTensorFlowState] = None) -> dict[str, object]:
    state = state or AILSTMTensorFlowState.create()
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AILSTMTensorFlowState] = None, lr: float = 0.01) -> AILSTMTensorFlowState:
    state = state or AILSTMTensorFlowState.create()
