"""TensorFlow/Keras reference backend for FXAI ai_lstm."""
...
class AILSTMTensorFlowModel(tf.keras.Model):
    def __init__(self) -> None:
        super().__init__()
...
@dataclass
class AILSTMTensorFlowState:
    model: AILSTMTensorFlowModel
    optimizer: tf.keras.optimizers.Optimizer

    @classmethod
    def create(cls, lr: float = 3.0e-4) -> "AILSTMTensorFlowState":
        return cls(model=AILSTMTensorFlowModel(), optimizer=tf.keras.optimizers.AdamW(learning_rate=lr, weight_decay=1.0e-4))


def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AILSTMTensorFlowState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AILSTMTensorFlowState.create()
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AILSTMTensorFlowState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AILSTMTensorFlowState:
    state = state or AILSTMTensorFlowState.create(lr=lr)
