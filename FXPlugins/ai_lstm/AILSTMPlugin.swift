...
public struct AILSTMPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.lstm.rawValue,
        aiName: "ai_lstm",
        family: .recurrent,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.lstm.rawValue),
    ...
    private var cpu: AILSTMCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AILSTMAccelerated.descriptor }

    public init() {
        self.cpu = AILSTMCPUModel()
    }
