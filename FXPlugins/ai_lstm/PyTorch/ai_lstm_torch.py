"""PyTorch/MPS reference backend for FXAI ai_lstm.

This plugin-local backend implements a long short-term memory sequence model with real PyTorch layer
...
class AILSTMReferenceModel(nn.Module):
    """Reference-grade long short-term memory sequence model."""
...
@dataclass
class AILSTMReferenceState:
    model: AILSTMReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AILSTMReferenceState":
        dev = device or preferred_device()
        model = AILSTMReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AILSTMReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AILSTMReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AILSTMReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AILSTMReferenceState:
    state = state or AILSTMReferenceState.create(lr=lr)
    state.model.train()
