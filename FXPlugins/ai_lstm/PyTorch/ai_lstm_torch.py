...
@dataclass
class AILSTMTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AILSTMTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 7)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AILSTMTorchState] = None) -> torch.Tensor:
    state = state or AILSTMTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AILSTMTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AILSTMTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AILSTMTorchState] = None, lr: float = 0.01) -> AILSTMTorchState:
    state = state or AILSTMTorchState.create()
    x = _features(batch, state.input_weight.device)
