...
public struct StatEMDHHTCPUModel: Sendable {
    ...
    public func predict(_ request: PredictRequestV4, hyperParameters _: HyperParameters) -> PredictionV4 {
        ...
        let margin = emdHHTMargin(z: z, window: window)
        let modelConfidence = emdHHTConfidence(z: z, window: window, forecastVolatility: forecastVolatility, dataHasVolume: request.context.dataHasVolume)
    ...
    private func emdHHTMargin(z: [Double], window: [[Double]]) -> Double {
        let recentDelta = window.isEmpty ? z[11] : Self.windowRecentDelta(window, feature: 1, count: 8)
        let recentMean = window.isEmpty ? z[1] : Self.windowRecentMean(window, feature: 1, count: 8)
        let modeMean = window.isEmpty ? 0.0 : Self.windowFeatureMean(window, feature: 1)
        return PluginSupportTools.clipSymmetric(
            0.45 * linearMargin(z: z) + 0.35 * recentDelta + 0.20 * (recentMean - modeMean),
            limit: 8.0
        )
    }

    private func emdHHTConfidence(z: [Double], window: [[Double]], forecastVolatility: Double, dataHasVolume: Bool) -> Double {
        let recentDelta = abs(window.isEmpty ? z[11] : Self.windowRecentDelta(window, feature: 1, count: 8))
        let meanShift = abs((window.isEmpty ? z[1] : Self.windowRecentMean(window, feature: 1, count: 8)) - (window.isEmpty ? 0.0 : Self.windowFeatureMean(window, feature: 1)))
        let decompositionStrength = fxClamp((recentDelta + meanShift + abs(z[8])) / max(forecastVolatility, 0.05), 0.0, 2.0) / 2.0
        let volatilityQuality = fxClamp(1.0 / (1.0 + max(forecastVolatility, 0.0)), 0.0, 1.0)
        let volumeBoost = dataHasVolume ? 0.06 * abs(z[7]) : 0.0
        return fxClamp(0.36 + 0.36 * decompositionStrength + 0.16 * volatilityQuality + volumeBoost, 0.0, 1.0)
    }
    ...
    private static func seedWeight(_ a: Int, _ b: Int) -> Double {
        let raw = sin(Double((AIModelID.statEMDHHT.rawValue + 17) * (a + 3) * 37 + (b + 11) * 101))
        return 0.035 * raw
    }
    ...
    private static func windowFeatureMean(_ window: [[Double]], feature: Int) -> Double {
        guard !window.isEmpty else { return 0.0 }
        let full = windowMean(window, feature: feature, start: 0, count: window.count)
        let halfCount = max(window.count / 2, 1)
        let quarterCount = max(window.count / 4, 1)
        let half = windowMean(window, feature: feature, start: max(0, window.count - halfCount), count: halfCount)
        let quarter = windowMean(window, feature: feature, start: max(0, window.count - quarterCount), count: quarterCount)
        return 0.40 * full + 0.35 * half + 0.25 * quarter
    }

    private static func windowRecentMean(_ window: [[Double]], feature: Int, count: Int) -> Double {
        windowMean(window, feature: feature, start: 0, count: min(max(count, 1), window.count))
    }

    private static func windowMean(_ window: [[Double]], feature: Int, start: Int, count: Int) -> Double {
        let values = windowValues(window, feature: feature, start: start, count: count)
        guard !values.isEmpty else { return 0.0 }
    ...
    private static func windowValues(_ window: [[Double]], feature: Int, count: Int) -> [Double] {
        windowValues(window, feature: feature, start: 0, count: count)
    }

    private static func windowValues(_ window: [[Double]], feature: Int, start: Int, count: Int) -> [Double] {
        guard feature >= 0, count > 0 else { return [] }
        return window.dropFirst(max(0, start)).prefix(count).map { row in
