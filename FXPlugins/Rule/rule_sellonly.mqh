#ifndef __FXAI_RULE_SELLONLY_MQH__
#define __FXAI_RULE_SELLONLY_MQH__

#include "..\..\FXDataEngine\API\plugin_base.mqh"

class CFXAIAIRuleSellOnly : public CFXAIAIPlugin
{
public:
   CFXAIAIRuleSellOnly(void) : CFXAIAIPlugin()
   {
      Reset();
   }

   virtual int AIId(void) const { return (int)AI_SELL_ONLY; }
   virtual string AIName(void) const { return "rule_sellonly"; }
...
protected:
   ...
   {
      class_probs[(int)FXAI_LABEL_SELL] = 0.999;
      class_probs[(int)FXAI_LABEL_BUY]  = 0.001;
...
#endif // __FXAI_RULE_SELLONLY_MQH__
