...
public struct AIPatchtstPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.patchTST.rawValue,
        aiName: "ai_patchtst",
        family: .transformer,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.patchTST.rawValue),
    ...
    private var cpu: AIPatchtstCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AIPatchtstAccelerated.descriptor }

    public init() {
        self.cpu = AIPatchtstCPUModel()
    }
