...
ARCHITECTURE_MODE = "patchTransformer"
FEATURE_COUNT = 32
HIDDEN_COUNT = 20
CLASS_COUNT = 3
...
@dataclass
class AIPatchtstTorchState:
    input_weight: torch.Tensor
    class_weight: torch.Tensor
    move_weight: torch.Tensor

    @classmethod
    def create(cls, device: Optional[torch.device] = None) -> "AIPatchtstTorchState":
        dev = device or _device()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(10_000 + 11)
...
def encode(batch: Iterable[Iterable[float]], state: Optional[AIPatchtstTorchState] = None) -> torch.Tensor:
    state = state or AIPatchtstTorchState.create()
...
def predict_batch(batch: Iterable[Iterable[float]], state: Optional[AIPatchtstTorchState] = None) -> dict[str, list[list[float]] | list[float]]:
    state = state or AIPatchtstTorchState.create()
    with torch.no_grad():
...
def train_step(batch: Iterable[Iterable[float]], labels: Iterable[int], moves: Iterable[float], state: Optional[AIPatchtstTorchState] = None, lr: float = 0.01) -> AIPatchtstTorchState:
    state = state or AIPatchtstTorchState.create()
    x = _features(batch, state.input_weight.device)
