"""PyTorch/MPS reference backend for FXAI ai_patchtst.

This plugin-local backend implements a PatchTST channel-independent patch transformer with real PyTorch layer
...
PLUGIN_NAME = "ai_patchtst"
ARCHITECTURE_MODE = "PATCHTST"
FEATURE_COUNT = 32
CLASS_COUNT = 3
HIDDEN_COUNT = 48
HEAD_COUNT = 4
...
class AIPatchTSTReferenceModel(nn.Module):
    """Reference-grade PatchTST channel-independent patch transformer."""
...
@dataclass
class AIPatchTSTReferenceState:
    model: AIPatchTSTReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AIPatchTSTReferenceState":
        dev = device or preferred_device()
        model = AIPatchTSTReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AIPatchTSTReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AIPatchTSTReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AIPatchTSTReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AIPatchTSTReferenceState:
    state = state or AIPatchTSTReferenceState.create(lr=lr)
    state.model.train()
