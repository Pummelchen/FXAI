...
public struct AINbeatsPlugin: FXAIPlannedPlugin {
    private static let pluginManifest = PluginManifestV4(
        aiID: AIModelID.nbeats.rawValue,
        aiName: "ai_nbeats",
        family: .transformer,
        referenceTier: PluginPersistenceTools.defaultReferenceTier(aiID: AIModelID.nbeats.rawValue),
    ...
    private var cpu: AINbeatsCPUModel

    public var manifest: PluginManifestV4 { Self.pluginManifest }
    public var accelerationPlan: FXPluginAccelerationPlan { AINbeatsAccelerated.descriptor }

    public init() {
        self.cpu = AINbeatsCPUModel()
    }
