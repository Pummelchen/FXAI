"""PyTorch/MPS reference backend for FXAI ai_nbeats.

This plugin-local backend implements an N-BEATS-style doubly residual stack forecaster with real PyTorch layer
...
PLUGIN_NAME = "ai_nbeats"
ARCHITECTURE_MODE = "NBEATS"
FEATURE_COUNT = 32
CLASS_COUNT = 3
HIDDEN_COUNT = 48
HEAD_COUNT = 4
...
class AINbeatsReferenceModel(nn.Module):
    """Reference-grade N-BEATS doubly residual stack forecaster."""
    ...
    def _nbeats_trend_basis(self, length: int, device: torch.device) -> torch.Tensor:
        t = torch.linspace(0.0, 1.0, length, device=device)
        powers = torch.arange(self.nbeats_poly_degree + 1, dtype=t.dtype, device=device)
        return t.unsqueeze(-1).pow(powers.unsqueeze(0))

    def _nbeats_fourier_basis(self, length: int, device: torch.device) -> torch.Tensor:
        t = torch.linspace(0.0, 1.0, length, device=device)
        harmonics = torch.arange(1, self.nbeats_fourier_harmonics + 1, dtype=t.dtype, device=device)
        cos_basis = torch.cos(2.0 * torch.pi * harmonics.unsqueeze(0) * t.unsqueeze(-1))
        sin_basis = torch.sin(2.0 * torch.pi * harmonics.unsqueeze(0) * t.unsqueeze(-1))
        return torch.cat([cos_basis, sin_basis], dim=-1)
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if arch == "NBEATS":
            ...
            trend_basis = self._nbeats_trend_basis(HIDDEN_COUNT, x.device)
            fourier_basis = self._nbeats_fourier_basis(HIDDEN_COUNT, x.device)
            trend_residual = h
            season_residual = h
            block_idx = 0
...
@dataclass
class AINbeatsReferenceState:
    model: AINbeatsReferenceModel
    optimizer: torch.optim.Optimizer

    @classmethod
    def create(cls, device: Optional[torch.device] = None, lr: float = 3.0e-4) -> "AINbeatsReferenceState":
        dev = device or preferred_device()
        model = AINbeatsReferenceModel().to(dev)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.0e-4)
        return cls(model=model, optimizer=optimizer)


def predict_batch(batch: Iterable[Iterable[float]] | torch.Tensor, state: Optional[AINbeatsReferenceState] = None, data_has_volume: bool = True) -> dict[str, list[list[float]] | list[float] | str]:
    state = state or AINbeatsReferenceState.create()
    state.model.eval()
...
def train_step(batch: Iterable[Iterable[float]] | torch.Tensor, labels: Iterable[int], moves: Iterable[float], state: Optional[AINbeatsReferenceState] = None, lr: float = 3.0e-4, data_has_volume: bool = True) -> AINbeatsReferenceState:
    state = state or AINbeatsReferenceState.create(lr=lr)
    state.model.train()
