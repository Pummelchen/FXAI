...
public enum AINbeatsMetal {
    public static let kernelSource = """
    #include <metal_stdlib>
    using namespace metal;

    kernel void ai_nbeats_sequence_logits(
    ...
    kernel void ai_nbeats_softmax3(
    ...
    public static let descriptor = FXPluginAccelerationPlan(
        ...
        notes: "Plugin-local Metal projection kernel source for ai_nbeats. Metal kernels perform a simple linear projection and 3-class softmax for runtime compilation and parity verification. The real N-BEATS inference path uses the Swift CPU or PyTorch MPS accelerator."
    )
}
