...
public struct StatXrateConsistencyCPUModel: Sendable {
    private static let stateDimension = 16

    private var steps: Int
    private var policyWeights: [[Double]]
    private var consistencyEMA: Double
    ...
    public mutating func reset() {
        self = StatXrateConsistencyCPUModel()
    }
    ...
    public mutating func train(_ request: TrainRequestV4, hyperParameters: HyperParameters) {
        ...
        updateConsistencyState(z: z, signedMove: signedMove, sampleWeight: sampleWeight)
    ...
    public func predict(_ request: PredictRequestV4, hyperParameters _: HyperParameters) -> PredictionV4 {
        ...
        let margin = xrateConsistencyMargin(z: z, dataHasVolume: request.context.dataHasVolume)
        let modelConfidence = xrateConfidence(z: z, dataHasVolume: request.context.dataHasVolume)
    ...
    private func xrateConsistencyMargin(z: [Double], dataHasVolume: Bool) -> Double {
        let base = linearMargin(z: z)
        let divergence = z[6] - z[13]
        let volumeTilt = dataHasVolume ? 0.06 * z[7] : 0.0
        return PluginSupportTools.clipSymmetric(
            -0.60 * divergence + 0.25 * base + 0.10 * consistencyEMA + volumeTilt,
            limit: 8.0
        )
    }

    private func xrateConfidence(z: [Double], dataHasVolume: Bool) -> Double {
        let volumeBoost = dataHasVolume ? 0.08 * abs(z[7]) : 0.0
        return fxClamp(abs(z[6] - z[13]) / 2.0 + volumeBoost, 0.0, 1.0)
    }
    ...
    private mutating func updateConsistencyState(z: [Double], signedMove: Double, sampleWeight: Double) {
        let divergence = z[6] - z[13]
        let realizedDirection = signedMove > 0.0 ? 1.0 : (signedMove < 0.0 ? -1.0 : 0.0)
        let observedCorrection = fxClamp(realizedDirection * abs(divergence), -8.0, 8.0)
        let alpha = fxClamp(0.02 + 0.01 * sampleWeight, 0.01, 0.08)
        consistencyEMA = (1.0 - alpha) * consistencyEMA + alpha * observedCorrection
    }
    ...
    private static func seedWeight(_ a: Int, _ b: Int) -> Double {
        let raw = sin(Double((AIModelID.statXRateConsistency.rawValue + 17) * (a + 3) * 37 + (b + 11) * 101))
        return 0.035 * raw
    }
