...
struct FXAIFamilyPluginRuntime: Sendable {
    ...
    init(definition: FXAIGeneratedPluginDefinition) {
        let featureCount = FXDataEngineConstants.aiWeights
        self.steps = 0
        self.classBias = Self.seededClassBias(definition: definition)
        self.classWeights = Self.seededClassWeights(definition: definition)
        self.moveEMA = 1.0
    ...
    mutating func train(
        ...
        definition: FXAIGeneratedPluginDefinition,
        ...
        let rawProbabilities = rawPrediction(features: features, context: request.context, definition: definition)
    ...
    func predict(
        ...
        let features = Self.engineeredFeatures(request.x, window: request.xWindow, windowSize: request.windowSize, context: request.context)
        var probabilities = rawPrediction(features: features, context: request.context, definition: definition)
        ...
        let edge = classEdge(features) + definition.profile.edge(request)
        ...
        let output = PluginModelOutputV4(
            ...
            reliability: reliability(probabilities: probabilities, definition: definition),
            hasQuantiles: true,
            hasConfidence: true,
            hasPathQuality: true
        )
    ...
    private func rawPrediction(
        ...
        let profile = definition?.profile ?? .linear
    ...
    private func familyEdge(features: [Double], context: PluginContextV4, profile: FXAIGeneratedPluginProfile) -> Double {
    ...
    private func reliability(probabilities: [Double], definition: FXAIGeneratedPluginDefinition) -> Double {
        ...
        return fxClamp(definition.profile.baseReliability + 0.20 * trained + 0.12 * confidence, 0.0, 1.0)
    }
    ...
    private static func seededClassBias(definition: FXAIGeneratedPluginDefinition) -> [Double] {
        let id = Double(definition.aiID.rawValue + 1)
        return [
            0.02 * sin(id * 0.73),
            0.02 * cos(id * 0.61),
    ...
    private static func seededClassWeights(definition: FXAIGeneratedPluginDefinition) -> [[Double]] {
        var output = Array(
