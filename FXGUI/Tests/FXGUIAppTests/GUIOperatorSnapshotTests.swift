...
struct GUIOperatorSnapshotTests {
    ...
    private func snapshotOutputDirectory() -> URL? {
        ...
        guard environment["FXGUI_WRITE_SNAPSHOTS"] == "1" else {
            return nil
        }
        if let explicit = environment["FXGUI_SNAPSHOT_DIR"], !explicit.isEmpty {
            return URL(fileURLWithPath: explicit, isDirectory: true)
        }
        return URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("fxgui-snapshots", isDirectory: true)
    }
}
