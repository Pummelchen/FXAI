...
FXDatabase is a macOS terminal Swift project for exporting MetaTrader 5 historical M1 OHLCV data into ClickHouse, verifying it against MT5, repairing canonical ranges only when safe, and serving verified history to FXBacktest through a dedicated HTTP API v1.
...
2. `FXDatabase`
   ...
   - Writes raw audit rows and canonical OHLCV rows to ClickHouse.
...
5. At the `>` prompt, run the database and EA preflight. This applies idempotent migrations, verifies required tables, runs DB-only integrity checks, and compiles `FXDatabase.mq5` through MetaEditor from the terminal:

```text
> startcheck --config-dir Config --migrations-dir Migrations --skip-bridge
```

6. Attach the compiled `FXDatabase` EA in MT5. Set `SwiftHost = 127.0.0.1` and `SwiftPort = 5055`, then allow localhost sockets in MT5/Wine if prompted.
...
Leave this command running while FXBacktest loads M1 OHLCV data. The API server is the only supported external data path for FXBacktest.
...
Edit the local files before production use. There is intentionally no `broker_time.json`; FXDatabase discovers the connected MT5 company/server/account through the EA and registers the broker source in ClickHouse.
...
"logging": {
  ...
  "log_file_path": "Logs/FXDatabase.log",
...
`Config/symbols.json` explicitly maps logical symbols to data sources and MT5 broker symbols:

```json
{ "source_origin": "MT5", "logical_symbol": "EURUSD", "mt5_symbol": "EURUSD.a", "digits": 5 }
```
...
For the current live MT5 server day, FXDatabase can automatically record a verified offset segment from the EA's `GET_SERVER_TIME_SNAPSHOT` when no verified segment covers the live server timestamp. For known IC Markets MT5 servers, FXDatabase can also auto-create non-overlapping historical GMT+2/GMT+3 broker-policy segments for the discovered MT5 history range, but only after the EA live snapshot matches that policy. Unknown brokers, broker policy mismatches, overlapping DB authority, or uncovered ranges stop canonical ingestion.
...
FXDatabase is now a resident terminal app. Start it without launch-time input:

```bash
swift run FXDatabase
```
...
When stdout is a TTY and `NO_COLOR` is not set, FXDatabase uses ANSI colors compatible with the macOS Bash 3 terminal. The logger applies a black background to colored terminal lines. Red is reserved for actual error lines and failed agent statuses.

During `supervise`, every production agent prints a timestamped status line with its own non-red color:

```text
2026-05-15 13:34:16 - Agent M1 Updater - Checking for newly closed M1 OHLCV bars
...
FXDatabase only prints "clean" after the relevant safety step has succeeded: validation plus canonical readback for ingestion, or MT5 source-of-truth comparison for verification. MT5 source gaps are reported as source gaps instead of being hidden as calendar-minute gaps.
...
| --- | ---: | --- |
| `schema_drift_guard` | max(900s, 10x health interval) | Confirms required ClickHouse tables and critical columns still match FXDatabase migrations before data agents run. |
| `bridge_version_guard` | max(900s, 10x health interval) | Confirms the attached MT5 EA is the FXDatabase bridge, uses the expected protocol schema, and matches the configured terminal identity. |
...
It serves `GET /v1/health` with ClickHouse reachability, broker source count, canonical row count, unfinished ingest operation count, warning/failed agent counts, valid data certificate count, and latest canonical UTC. This endpoint is operational status only; it does not serve OHLCV data.
