...
final class IngestionTests: XCTestCase {
    ...
    func testCheckpointStoreRoundTrip() async throws {
        ...
        let loaded = try await store.latestState(brokerSourceId: state.brokerSourceId, sourceOrigin: state.sourceOrigin, logicalSymbol: state.logicalSymbol)
        XCTAssertEqual(loaded, state)
    }

    func testClickHouseCheckpointReadUsesHighestIngestedServerTime() async throws {
        let client = MockClickHouseClient(body: """
        demo\tMT5\tEURUSD\tEURUSD\t60\t180\t120\tbackfilling\tb\t1
        ...
        _ = try await store.latestState(brokerSourceId: try BrokerSourceId("demo"), sourceOrigin: .mt5, logicalSymbol: try LogicalSymbol("EURUSD"))
    ...
    func testCanonicalInsertVerifierComparesReadbackHashes() async throws {
        ...
        let client = SequenceClickHouseClient(bodies: [
            "1\t1\t1\t1\t1\t1\t1\t0\n",
            "MT5\tEURUSD\tM1\t120\t60\t60\tmanual\tverified\t110000\t110000\t110000\t110000\t0\t5\t\(bar.barHash.description)\n"
        ])
    ...
    func testCanonicalInsertVerifierRejectsHashMismatch() async throws {
        ...
        let client = SequenceClickHouseClient(bodies: [
            "1\t1\t1\t1\t1\t1\t1\t0\n",
            "MT5\tEURUSD\tM1\t120\t60\t60\tmanual\tverified\t110000\t110000\t110000\t110000\t0\t5\tbad-hash\n"
        ])
    ...
    func testCanonicalInsertVerifierRejectsMetadataMismatch() async throws {
        ...
        let client = SequenceClickHouseClient(bodies: [
            "1\t1\t1\t1\t1\t1\t1\t0\n",
            "MT5\tEURUSD.a\tM1\t120\t60\t60\tmanual\tverified\t110000\t110000\t110000\t110000\t0\t5\t\(bar.barHash.description)\n"
        ])
    ...
    func testCanonicalConflictRecorderWritesConflictBeforeReplace() async throws {
        let bar = try validatedBar(mt5: 120, utc: 60)
        let client = SequenceClickHouseClient(bodies: [
            "60\toldhash\t109000\t110000\t108000\t109500\t0\n",
            ""
        ])
