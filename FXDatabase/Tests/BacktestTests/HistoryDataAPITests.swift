...
final class HistoryDataAPITests: XCTestCase {
    ...
    func testClickHouseProviderBuildsValidatedColumnarSeries() async throws {
        ...
        XCTAssertEqual(series.volume, [0, 0])
        ...
        XCTAssertTrue(sql.contains("source_origin = 'MT5'"))
    ...
    func testClickHouseProviderAcceptsGenericClosedM1SourceStatus() async throws {
        let client = MockHistoryClickHouse(body: try historyRow(
            utc: 1_577_836_800,
            mt5: 1_577_844_000,
            open: 108_342,
        ...
        let series = try await provider.loadM1Ohlc(try request())

        XCTAssertEqual(series.count, 1)
        XCTAssertEqual(series.volume, [0])
    }
    ...
    func testSineWaveAgentGeneratesHourlyM1SineTestSecurity() async throws {
        ...
        XCTAssertEqual(series.open[30], 1_000)
        XCTAssertEqual(series.low[30], 1_000)
