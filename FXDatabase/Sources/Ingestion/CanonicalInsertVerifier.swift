...
public struct CanonicalInsertVerifier: Sendable {
    ...
    public func verify(
        ...
        guard readback.uniqueSourceOrigins == 1 else {
            throw IngestError.canonicalInsertVerificationFailed("expected one source origin in canonical readback, got \(readback.uniqueSourceOrigins)")
        }
        ...
        for (index, pair) in zip(rows, bars).enumerated() {
            let (row, bar) = pair
            guard row.sourceOrigin == bar.sourceOrigin,
                  row.mt5Symbol == bar.mt5Symbol,
                  ...
                  row.volume == bar.volume.rawValue,
    ...
    private func parseReadback(_ body: String) throws -> CanonicalRangeReadback {
        ...
        guard fields.count == 8,
              ...
              let uniqueSourceOrigins = Int(fields[3]),
              let uniqueMT5Symbols = Int(fields[4]),
              let uniqueTimeframes = Int(fields[5]),
              let uniqueDigits = Int(fields[6]),
              let nonVerifiedOffsetRows = Int(fields[7]) else {
        ...
        return CanonicalRangeReadback(
            ...
            uniqueSourceOrigins: uniqueSourceOrigins,
