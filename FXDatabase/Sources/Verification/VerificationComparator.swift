...
public enum VerificationMismatch: Equatable, Sendable {
    ...
    case sourceOrigin(index: Int, mt5: DataSourceOrigin, database: DataSourceOrigin)
    ...
    case ohlc(index: Int)
    case volume(index: Int, mt5: M1Volume, database: M1Volume)
...
public struct VerificationComparator: Sendable {
    ...
    public func compare(mt5SourceBars: [VerificationBar], databaseBars: [VerificationBar]) -> VerificationResult {
        ...
        for index in 0..<count {
            ...
            if mt5.sourceOrigin != db.sourceOrigin {
                mismatches.append(.sourceOrigin(index: index, mt5: mt5.sourceOrigin, database: db.sourceOrigin))
            }
            ...
            if mt5.volume != db.volume {
                mismatches.append(.volume(index: index, mt5: mt5.volume, database: db.volume))
            }
