...
public struct MetalBufferManager {
    ...
    public func makeReadOnlyBuffers(
        ...
        return MetalOhlcBuffers(
            ...
            count: series.count,
            utcTimestamps: try makeBuffer(series.utcTimestamps, label: "fxdatabase.utc_timestamps", device: device),
            open: try makeBuffer(series.open, label: "fxdatabase.open", device: device),
            high: try makeBuffer(series.high, label: "fxdatabase.high", device: device),
            low: try makeBuffer(series.low, label: "fxdatabase.low", device: device),
            close: try makeBuffer(series.close, label: "fxdatabase.close", device: device),
            volume: try makeUInt64Buffer(series.volume, label: "fxdatabase.volume", device: device)
        )
    }
    ...
    private func makeUInt64Buffer(_ values: [UInt64], label: String, device: MTLDevice) throws -> MTLBuffer {
        guard values.count <= Int.max / MemoryLayout<UInt64>.stride else {
            throw MetalAccelerationError.setupFailed("Metal buffer \(label) is too large.")
        }
        let byteCount = max(values.count * MemoryLayout<UInt64>.stride, 1)
        guard let buffer = device.makeBuffer(length: byteCount, options: [.storageModeShared]) else {
            throw MetalAccelerationError.setupFailed("Could not allocate Metal buffer \(label).")
        }
        buffer.label = label
