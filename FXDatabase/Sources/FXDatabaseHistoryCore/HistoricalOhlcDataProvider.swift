...
public struct ClickHouseHistoricalOhlcDataProvider: HistoricalOhlcDataProviding {
    ...
    private func sql(for request: HistoricalOhlcRequest, limit: Int) throws -> String {
        ...
        return """
        SELECT source_origin, mt5_symbol, ts_utc, mt5_server_ts_raw, open_scaled, high_scaled, low_scaled, close_scaled, volume,
        ...
        WHERE broker_source_id = '\(Self.sqlLiteral(request.brokerSourceId.rawValue))'
          AND source_origin = '\(Self.sqlLiteral(request.sourceOrigin.rawValue))'
    ...
    private func validateVerifiedCoverage(_ request: HistoricalOhlcRequest) async throws {
        ...
        WHERE broker_source_id = '\(Self.sqlLiteral(request.brokerSourceId.rawValue))'
          ...
          AND logical_symbol = '\(Self.sqlLiteral(request.logicalSymbol.rawValue))'
          AND timeframe = 'M1'
          AND hash_schema_version = '\(Self.sqlLiteral(ChunkHashSchemaVersion.current))'
    ...
    private func parseRow(_ row: String, request: HistoricalOhlcRequest) throws -> CanonicalOhlcRow {
        ...
        guard fields.count == 14,
              ...
              let tsUtc = Int64(fields[2]),
              let mt5ServerTime = Int64(fields[3]),
              let open = Int64(fields[4]),
              let high = Int64(fields[5]),
              let low = Int64(fields[6]),
              let close = Int64(fields[7]),
              let volume = UInt64(fields[8]),
              let digitsRaw = Int(fields[9]) else {
            throw HistoryDataError.invalidCanonicalRow(row)
        }
        guard sourceOrigin == request.sourceOrigin else {
            throw HistoryDataError.invalidCanonicalRow("source origin mismatch: expected \(request.sourceOrigin.rawValue), got \(sourceOrigin.rawValue)")
        }
        guard let timeframe = Timeframe(rawValue: fields[10]) else {
            throw HistoryDataError.invalidCanonicalRow("unknown timeframe '\(fields[10])'")
        }
        guard let confidence = OffsetConfidence(rawValue: fields[11]) else {
            throw HistoryDataError.invalidCanonicalRow("unknown offset confidence '\(fields[11])'")
        }
        guard let sourceStatus = SourceStatus(rawValue: fields[12]) else {
            throw HistoryDataError.invalidCanonicalRow("unknown source status '\(fields[12])'")
        }
        guard let storedHashValue = UInt64(fields[13], radix: 16) else {
            throw HistoryDataError.invalidCanonicalRow("invalid bar hash '\(fields[13])'")
        }
        ...
        guard sourceStatus.isCanonicalClosedM1Bar else {
            throw HistoryDataError.invalidCanonicalRow("canonical row source_status is \(sourceStatus.rawValue), expected a closed M1 bar status")
        }
