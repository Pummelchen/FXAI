...
public struct DataCertificationAgent: ProductionAgent {
    ...
    public func run(context: AgentRuntimeContext, startedAt: Date) async throws -> AgentOutcome {
        ...
        for range in ranges {
            try await store.certify(
                ...
                sourceOrigin: range.sourceOrigin,
    ...
    private func pendingRanges(context: AgentRuntimeContext, limit: Int) async throws -> [CertificationRange] {
        ...
        SELECT coverage.broker_source_id, coverage.source_origin, coverage.logical_symbol,
        ...
        LEFT JOIN
        (
            SELECT broker_source_id, source_origin, logical_symbol, utc_range_start, utc_range_end_exclusive,
                   argMax(certificate_status, created_at_utc) AS latest_status
            FROM \(context.config.clickHouse.database).data_certificates
            GROUP BY broker_source_id, source_origin, logical_symbol, utc_range_start, utc_range_end_exclusive
        ) AS certs
        ON coverage.broker_source_id = certs.broker_source_id
           AND coverage.source_origin = certs.source_origin
        ...
        WHERE coverage.broker_source_id = '\(SQLText.literal(context.config.brokerTime.brokerSourceId.rawValue))'
          AND coverage.hash_schema_version = '\(SQLText.literal(ChunkHashSchemaVersion.current))'
        ...
        GROUP BY coverage.broker_source_id, coverage.source_origin, coverage.logical_symbol, coverage.utc_range_start, coverage.utc_range_end_exclusive
...
private struct CertificationRange {
    ...
    static func parse(_ row: String) throws -> CertificationRange {
        ...
        guard fields.count == 5,
              let sourceOrigin = DataSourceOrigin(rawValue: String(fields[1])),
              let start = Int64(fields[3]),
              let end = Int64(fields[4]) else {
        ...
        return CertificationRange(
            ...
            logicalSymbol: try LogicalSymbol(String(fields[2])),
