...
public struct VerificationCoveragePlannerAgent: ProductionAgent {
    ...
    public func run(context: AgentRuntimeContext, startedAt: Date) async throws -> AgentOutcome {
        ...
        for mapping in context.config.symbols.symbols {
            let canonical = try await canonicalSummary(context: context, sourceOrigin: mapping.sourceOrigin, logicalSymbol: mapping.logicalSymbol.rawValue)
            let coverage = try await coverageIntervals(context: context, sourceOrigin: mapping.sourceOrigin, logicalSymbol: mapping.logicalSymbol.rawValue)
            if canonical.rows == 0 {
            ...
            let cleanVerifications = mapping.sourceOrigin == .mt5
                ? try await cleanVerificationCount(context: context, sourceOrigin: mapping.sourceOrigin, logicalSymbol: mapping.logicalSymbol.rawValue)
                : 1
    ...
    private func canonicalSummary(context: AgentRuntimeContext, sourceOrigin: DataSourceOrigin, logicalSymbol: String) async throws -> CanonicalSummary {
        ...
        WHERE broker_source_id = '\(SQLText.literal(context.config.brokerTime.brokerSourceId.rawValue))'
          AND source_origin = '\(SQLText.literal(sourceOrigin.rawValue))'
    ...
    private func coverageIntervals(context: AgentRuntimeContext, sourceOrigin: DataSourceOrigin, logicalSymbol: String) async throws -> [CoverageInterval] {
        let body = try await context.clickHouse.execute(.select("""
    ...
    private func cleanVerificationCount(context: AgentRuntimeContext, sourceOrigin: DataSourceOrigin, logicalSymbol: String) async throws -> Int64 {
        let body = try await context.clickHouse.execute(.select("""
        SELECT count()
        FROM (
            ...
            WHERE broker_source_id = '\(SQLText.literal(context.config.brokerTime.brokerSourceId.rawValue))'
              AND source_origin = '\(SQLText.literal(sourceOrigin.rawValue))'
              AND logical_symbol = '\(SQLText.literal(logicalSymbol))'
            GROUP BY source_origin, range_start_mt5_server_ts, range_end_mt5_server_ts
        )
