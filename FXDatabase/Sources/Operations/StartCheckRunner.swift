...
public struct StartCheckRunner: Sendable {
    ...
    public func run() async -> Bool {
        ...
        await step("2/7 Required ClickHouse tables", result: &result) {
            ...
            try await syncConfiguredSymbolDataSources()
        }
        ...
        if options.compileEA {
            await step("4/7 MetaEditor EA compile check", result: &result) {
                ...
                let compile = try await compiler.compileFXDatabaseEA(timeoutSeconds: options.compileTimeoutSeconds)
        ...
        await step("5/7 MT5 bridge connection and terminal identity", result: &result) {
            logger.info("Action needed if this waits: start MT5, attach FXDatabase, allow localhost sockets, and set SwiftHost/SwiftPort from Config/mt5_bridge.json")
    ...
    private func ensureAutomaticHistoricalOffsetAuthority(
        ...
        for mapping in config.symbols.symbols where mapping.sourceOrigin == .mt5 {
    ...
    private func syncConfiguredSymbolDataSources() async throws {
        guard !config.symbols.symbols.isEmpty else { return }
        let configuredAt = Int64(Date().timeIntervalSince1970)
        let rows = config.symbols.symbols.map { mapping in
            [
                Self.tsv(config.brokerTime.brokerSourceId.rawValue),
                Self.tsv(mapping.logicalSymbol.rawValue),
                Self.tsv(mapping.sourceOrigin.rawValue),
                Self.tsv(mapping.mt5Symbol.rawValue),
                "1",
                Self.tsv("active"),
                String(configuredAt)
            ].joined(separator: "\t")
        ...
        let sql = """
        INSERT INTO \(config.clickHouse.database).symbol_data_sources
        (broker_source_id, logical_symbol, source_origin, source_symbol, priority, status, configured_at_utc)
        FORMAT TabSeparated
        \(rows)
        """
        _ = try await clickHouse.execute(.mutation(sql, idempotent: false))
        logger.ok("Symbol data source map synced to ClickHouse: \(config.symbols.symbols.count) configured source route(s)")
    }
    ...
    private func verifySymbolsAndBridgeCommands(bridge: MT5BridgeClient, config: ConfigBundle) throws {
        ...
        guard let first = mt5Mappings.first else {
            throw StartCheckError.invalidBridge("no MT5 symbols configured")
        }
        for mapping in mt5Mappings {
            let info = try bridge.prepareSymbol(mapping.mt5Symbol)
...
private enum StartCheckGuidance {
    static func guidance(for error: Error) -> String {
        if case StartCheckError.missingTables = error {
            return "Next action: at the FXDatabase prompt run `migrate --config-dir Config --migrations-dir Migrations`, then run `startcheck --config-dir Config`."
        }
        ...
        if error is MT5BridgeError || error is ProtocolError {
            return "Next action: start MT5, attach the compiled FXDatabase EA to a chart, enable localhost sockets, then rerun startcheck."
        }
...
public struct MetaEditorCompiler: Sendable {
    ...
    public func compileFXDatabaseEA(timeoutSeconds: TimeInterval) async throws -> MetaEditorCompileResult {
        let source = try locateFXDatabaseSource()
        ...
        let stdoutPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("FXDatabase-EA-stdout-\(UUID().uuidString).log")
        ...
        throw StartCheckError.eaCompileFailed("MetaEditor compile did not produce a clean FXDatabase.ex5. Compile log: \(logPath.path). \(detail)")
    }
    ...
    private func locateFXDatabaseSource() throws -> URL {
        ...
        throw StartCheckError.eaSourceNotFound("Could not find FXDatabase.mq5 from \(workingDirectory.path)")
    }

    private static func createCompileStage(for source: URL) throws -> (directory: URL, source: URL) {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("FXDatabase-EA-stage-\(UUID().uuidString)")
...
public struct MetaEditorToolchain: Sendable {
    ...
    public static func locate(from source: URL) throws -> MetaEditorToolchain {
        ...
        let wine = URL(fileURLWithPath: environment["FXDATABASE_WINE"] ?? environment["FXAI_WINE"] ?? "/Applications/MetaTrader 5.app/Contents/SharedSupport/wine/bin/wine64")
        ...
        let metaEditor = URL(fileURLWithPath: environment["FXDATABASE_METAEDITOR"] ?? environment["FXAI_METAEDITOR"] ?? mt5Root.appendingPathComponent("MetaEditor64.exe").path)
        ...
        let winePrefix = URL(fileURLWithPath: environment["WINEPREFIX"] ?? environment["FXDATABASE_WINEPREFIX"] ?? mt5Root.deletingLastPathComponent().deletingLastPathComponent().deletingLastPathComponent().path)
