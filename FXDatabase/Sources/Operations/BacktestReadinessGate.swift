...
public struct BacktestReadinessGate: Sendable {
    ...
    private func validateRequest(_ request: BacktestReadinessRequest) throws {
        ...
        guard config.symbols.mapping(for: request.logicalSymbol, sourceOrigin: request.sourceOrigin) != nil else {
    ...
    private func validateIngestIsComplete(_ request: BacktestReadinessRequest) async throws {
        ...
        for mapping in config.symbols.symbols where mapping.sourceOrigin == request.sourceOrigin {
    ...
    private func validateNoUnfinishedIngestOperations(_ request: BacktestReadinessRequest) async throws {
        ...
        FROM (
            ...
            WHERE broker_source_id = '\(SQLText.literal(request.brokerSourceId.rawValue))'
              AND source_origin = '\(SQLText.literal(request.sourceOrigin.rawValue))'
    ...
    private func validateVerificationAndRepairState(_ request: BacktestReadinessRequest) async throws {
        ...
        FROM (
            ...
            WHERE broker_source_id = '\(SQLText.literal(request.brokerSourceId.rawValue))'
              AND source_origin = '\(SQLText.literal(request.sourceOrigin.rawValue))'
            GROUP BY source_origin, logical_symbol, range_start_mt5_server_ts, range_end_mt5_server_ts
        )
