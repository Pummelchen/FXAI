...
public final class SupervisorLock: @unchecked Sendable {
    ...
    public static func acquireRuntime(brokerSourceId: String, owner: String) throws -> SupervisorLock {
        ...
        let path = "/tmp/fxdatabase-\(String(safeId))-runtime.lock"
        let fd = open(path, O_CREAT | O_RDWR, S_IRUSR | S_IWUSR)
        guard fd >= 0 else {
            throw SupervisorError.lockUnavailable("Cannot open FXDatabase runtime lock at \(path): errno=\(errno)")
        }
        guard flock(fd, LOCK_EX | LOCK_NB) == 0 else {
            _ = close(fd)
            throw SupervisorError.lockUnavailable("Another FXDatabase writer/supervisor is already running for broker source \(brokerSourceId); \(owner) must wait")
        }
