...
public enum ClickHouseInsertError: Error, CustomStringConvertible, Sendable {
    ...
    public var description: String {
        switch self {
        case .mixedCanonicalRange:
            return "Canonical range contains multiple broker, source origin, symbol, timeframe, or digit identities."
        case .unsortedCanonicalRange:
...
public struct ClickHouseInsertBuilder: Sendable {
    ...
    public func rawBarsInsert(_ bars: [ValidatedBar]) -> ClickHouseQuery {
        ...
        INSERT INTO \(database).mt5_ohlc_m1_raw (
            broker_source_id, source_origin, logical_symbol, mt5_symbol, timeframe, mt5_server_ts_raw, ts_utc,
            server_utc_offset_seconds, offset_source, offset_confidence,
            open_scaled, high_scaled, low_scaled, close_scaled, volume, digits,
    ...
    public func canonicalRangeDelete(
        ...
        WHERE broker_source_id = '\(sqlLiteral(first.brokerSourceId.rawValue))'
          AND source_origin = '\(sqlLiteral(first.sourceOrigin.rawValue))'
    ...
    public func canonicalRangeIntegrityCheck(
        ...
        let range = try canonicalRangeIdentity(bars)
        guard let first = range.first, let last = range.last else {
            return .select("SELECT 0, 0, 0, 0, 0, 0, 0, 0 FORMAT TabSeparated")
        }
    ...
    public func canonicalRangeReadbackRows(
        ...
        let range = try canonicalRangeIdentity(bars)
        guard let first = range.first, let last = range.last else {
            return .select("SELECT source_origin, mt5_symbol, timeframe, mt5_server_ts_raw, ts_utc, server_utc_offset_seconds, offset_source, offset_confidence, open_scaled, high_scaled, low_scaled, close_scaled, volume, digits, bar_hash FROM \(database).ohlc_m1_canonical WHERE 0 FORMAT TabSeparated")
        }
        let predicate: String
        if let mt5Start, let mt5EndExclusive {
            predicate = """
        ...
        let sql = """
        SELECT source_origin, mt5_symbol, timeframe, mt5_server_ts_raw, ts_utc,
               server_utc_offset_seconds, offset_source, offset_confidence,
               open_scaled, high_scaled, low_scaled, close_scaled, volume, digits, bar_hash
        FROM \(database).ohlc_m1_canonical
        WHERE broker_source_id = '\(sqlLiteral(first.brokerSourceId.rawValue))'
          AND source_origin = '\(sqlLiteral(first.sourceOrigin.rawValue))'
          AND logical_symbol = '\(sqlLiteral(first.logicalSymbol.rawValue))'
    ...
    public func canonicalMT5RangeReadbackRows(
        ...
        WHERE broker_source_id = '\(sqlLiteral(brokerSourceId.rawValue))'
          AND source_origin = '\(sqlLiteral(sourceOrigin.rawValue))'
    ...
    public func canonicalConflictCandidates(_ bars: [ValidatedBar]) throws -> ClickHouseQuery {
        let range = try canonicalRangeIdentity(bars)
        guard let first = range.first, let last = range.last else {
            return .select("SELECT ts_utc, bar_hash, open_scaled, high_scaled, low_scaled, close_scaled, volume FROM \(database).ohlc_m1_canonical WHERE 0 FORMAT TabSeparated")
        }
        let sql = """
        SELECT ts_utc, bar_hash, open_scaled, high_scaled, low_scaled, close_scaled, volume
        FROM \(database).ohlc_m1_canonical
        WHERE broker_source_id = '\(sqlLiteral(first.brokerSourceId.rawValue))'
          AND source_origin = '\(sqlLiteral(first.sourceOrigin.rawValue))'
          AND logical_symbol = '\(sqlLiteral(first.logicalSymbol.rawValue))'
    ...
    public func conflictRowsInsert(_ conflicts: [CanonicalConflictRow]) -> ClickHouseQuery {
        ...
        INSERT INTO \(database).ohlc_m1_conflicts (
            broker_source_id, source_origin, logical_symbol, mt5_symbol, ts_utc,
            existing_bar_hash, incoming_bar_hash,
            existing_open_scaled, existing_high_scaled, existing_low_scaled, existing_close_scaled, existing_volume,
            incoming_open_scaled, incoming_high_scaled, incoming_low_scaled, incoming_close_scaled, incoming_volume,
    ...
    public func ingestStateUpsert(
        ...
        INSERT INTO \(database).ingest_state (
            broker_source_id, source_origin, logical_symbol, mt5_symbol, oldest_mt5_server_ts_raw,
    ...
    private func conflictRow(_ conflict: CanonicalConflictRow) -> String {
        [
            ...
            tsv(conflict.sourceOrigin.rawValue),
            ...
            String(conflict.existingVolume.rawValue),
            ...
            String(conflict.incomingVolume.rawValue),
    ...
    private func canonicalRangeIdentity(_ bars: [ValidatedBar]) throws -> (first: ValidatedBar?, last: ValidatedBar?) {
        ...
        for bar in bars.dropFirst() {
            guard bar.brokerSourceId == first.brokerSourceId,
                  bar.sourceOrigin == first.sourceOrigin,
...
public struct CanonicalConflictRow: Sendable, Hashable {
    ...
    public let existingVolume: M1Volume
    ...
    public let incomingVolume: M1Volume
