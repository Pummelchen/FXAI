...
private actor InteractiveCommandCoordinator {
    ...
    func printBanner() {
        info("FXDatabase interactive command shell started")
        info("Type an FXDatabase command at the prompt, for example: supervise --with-backfill")
    ...
    func handle(_ line: String) async -> Bool {
        ...
        case "exit", "quit":
            ...
            ok("FXDatabase interactive command shell stopped")
            return false
        case "help", "?":
            printHelp()
            return true
        ...
        case "shell", "interactive", "console":
            info("Already running inside the FXDatabase interactive command shell")
            return true
        default:
    ...
    func stopActiveCommand(reason: String) async {
        guard let active = activeCommand else {
            info("No active FXDatabase command is running")
            return
        }
    ...
    private func startAppCommand(_ arguments: [String]) async {
        ...
        let task = Task.detached(priority: .userInitiated) { () -> ExitCode in
            let result = await FXDatabaseCLI.run(arguments: arguments)
    ...
    private func printStatus() {
        guard let active = activeCommand else {
            ok("No active FXDatabase command is running; prompt is ready")
            return
        }
    ...
    private func printHelp() {
        print("""
        FXDatabase interactive command shell
    ...
    private func stripBinaryPrefix(_ tokens: [String]) -> [String] {
        ...
        if lower == "fxdatabase" || lower == "./fxdatabase" {
